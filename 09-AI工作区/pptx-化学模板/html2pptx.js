    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        liElements.forEach((li, idx) => {
          ...
          if (runs.length > 0) {
            runs[0].text = runs[0].text.replace(/^[•\-\*▪▸]\s*/, '');
            // Apply bullet to ALL runs: setting it only on runs[0] makes
            // PptxGenJS emit a second <a:pPr> (buNone, indent=0) for the
            // remaining runs, which suppresses the bullet in renderers.
            runs.forEach(r => { r.options.bullet = { indent: textIndent }; });
          }
