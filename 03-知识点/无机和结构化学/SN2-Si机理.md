...
## 十五、待完善项
- [x] 补充 P 上 SN2 的立体化学（Walden 保留 vs 反转）（2026-07-28 补充）
- [x] 补充溶剂效应对 SN2-Si 速率的影响（2026-07-28 补充）
- [x] 补充酶催化反应中的 SN2-P 机理（2026-07-28 补充）

### §十六 SN2-Si/P 的立体化学、溶剂效应与酶催化

**P 上 SN2 立体化学——Walden 保留 vs 反转**：
- 碳上 SN2：严格反转（背面进攻），构型翻转
- 硅上 SN2：通常也反转（背面进攻），但由于 Si 有 d 轨道可容纳5配位中间体，反转有时不完全
- **磷上 SN2**：可发生**保留**构型（Walden 保留）！机理：
  1. 亲核试剂从背面进攻P → 形成5配位三角双锥中间体
  2. 若离去基团从**同一面**离去（假旋转后）→ 构型保留
  3. 这在磷酰基转移反应中非常重要（如DNA水解）
- 立体化学判据：观察5配位中间体的Berry假旋转路径

**溶剂效应对 SN2-Si 速率的影响**：
| 溶剂类型 | 对 SN2-Si 影响 | 原因 |
|:---|:---|:---|
| 极性非质子（DMF, DMSO） | 显著加速 | 亲核试剂未被溶剂化，活性高 |
| 极性质子（MeOH, H₂O） | 减速 | 亲核试剂被氢键溶剂化，活性降低 |
| 非极性（THF, CH₂Cl₂） | 中等 | 亲核试剂部分去溶剂化 |
- 关键：SN2-Si 的过渡态电荷分散 → 极性非质子溶剂最有利
- 对比SN2-C：溶剂效应类似但更显著（Si的过渡态电荷更分散）

**酶催化中的 SN2-P 机理**：
- 碱性磷酸酶（AP）催化磷酸酯水解：
  1. Ser-OH 亲核进攻P（SN2-P）→ 形成磷酸化-Ser中间体
  2. 水进攻磷酸化-Ser → 释放磷酸盐
  3. Zn²⁺ 作为Lewis酸活化磷酸酯P-O键
- 蛋白激酶：ATP的γ-磷酸基团转移至Ser-OH（SN2-P，构型保留）
- 核酸酶：磷酸二酯键的水解通过SN2-P（Mg²⁺协助）
