---
title: Lewis结构式
aliases: [Lewis结构, 电子式, 路易斯结构式, Lewis dot structure, Lewis Structure, 电子点式, Lewis formula, 点电子式]
type: 知识点
...
> **关键区分**：有机 Lewis 结构式不标孤对电子，但标形式电荷。所以才会有"自由基正离子/负离子"的概念。有机机理中，只在电子推动的"起点/终点"处显式画出孤对（提示箭头来源）。

> **来源：ABOC §1.1 行 730**："有机 Lewis 结构式：不标孤对电子，但是标形式电荷。所以才会有'自由基正离子/负离子'的概念。"
...
> **ABOC 行 732-734**："对于第三周期及以后的元素，成偶数 (0除外) 根键时不用带上电荷。成双键时可以看做该元素单方面给电子另外一个元素接受的配位键。"
...
## 八、与其他知识点的联系
- 前置知识：[[共价键]]、[[原子结构]]（价电子概念）、[[八隅律]]、[[电负性]]
- 相关知识：[[VSEPR理论]]、[[价态-氧化态-形式电荷]]、[[共振论]]、[[Kekulé式]]（苯环表示法）、[[键线式]]（简化骨架）
- 应用知识：[[杂化轨道理论]]、[[σ键]]、[[π键]]、[[亲核体与亲电体]]（Lewis 酸碱等价）、[[反应机理表示法]]（电子推动箭头必须基于 Lewis 式）、[[碳正离子]]、[[卡宾]]、[[自由基]]（缺电子中间体的 Lewis 式表达）、[[VSEPR理论]]（Lewis 式 → 预测三维构型）
...
- **错**："B 在 BH₃ 中形式电荷是 +1" → B 的标准就是 3 根键（6 电子稳定），形式电荷 = 0
- **错**："有机 Lewis 式也要标所有孤对" → 有机机理不标孤对，只标电荷和单电子
- **错**："形式电荷 = 氧化数" → 形式电荷均分，氧化数全给电负性大的
- **错**："N⁺ 与 N⁻ 混淆" → N 成 4 根键 = N⁺；N 成 2 根键 = N⁻
- **错**："第三周期元素形式电荷漏算" → 如 SO₄²⁻ 中 S 的形式电荷 = +2（不是 0），需用公式计算
- **错**："键线式中忘记杂原子的孤对" → 机理书写时，O/N 的孤对往往是电子推动起点
- **错**："共振式中形式电荷不守恒" → 所有共振式的总电荷必须相同
...
- **NHC（氮杂环卡宾）的 Lewis 式**：看似 N 旁边有 “C:”（6 电子碳），但整个环有 6 π 电子 → 芳香性稳定。这是 Lewis 式”局部看不合理、整体看合理”的典型案例
- **CO 的 Lewis 式**：C≡O 中形式电荷 C⁻、O⁺，与电负性直觉相反。这是因为 O→C 的配位键占主导
- **硫酸的 S=O 配位键本质**：若把 S=O 画成配位键（S→O），则 S 的形式电荷 = +2，O = -1，通过共振平均后 S = 0，每个 O = -1/2
- **超价分子的分子轨道解释**：第三周期元素的”d 轨道参与”在现代理论中受到质疑，更准确的描述是**极化函数**而非真正的 d 轨道占据。但对于竞赛级别，d 轨道解释仍被广泛使用
