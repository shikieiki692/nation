...
## 十五、补充例题：极稀溶液 pH + 高温中性 pH

### 15.1 例题：极稀强酸溶液的精确 pH

**题目**：计算 $1.0 \times 10^{-8}$ M HCl 溶液的 pH（298 K）。

**常见错误**：直接 $\mathrm{pH} = -\lg(10^{-8}) = 8.0$ → 错！酸溶液 pH > 7 不合理。

**正确解法**（电荷守恒法）：

1. 电荷守恒：$[\mathrm{H^+}] = [\mathrm{Cl^-}] + [\mathrm{OH^-}] = c_{\mathrm{HCl}} + K_w/[\mathrm{H^+}]$
2. 令 $x = [\mathrm{H^+}]$：$x = 10^{-8} + 10^{-14}/x$
3. 整理：$x^2 - 10^{-8}x - 10^{-14} = 0$
4. 求解：$x = (10^{-8} + \sqrt{10^{-16} + 4 \times 10^{-14}}) / 2 \approx 1.05 \times 10^{-7}$ M
5. $\mathrm{pH} = -\lg(1.05 \times 10^{-7}) \approx 6.98$

**教学要点**：
- 极稀酸碱必须联立 $K_w$ 求解，不能忽略水的自耦电离
- 阈值：$c < 10^{-6}$ M 时，水的自耦电离贡献不可忽略
- 结论：无论多稀的酸，pH 永远 < 7（但趋近 7）

### 15.2 高温下中性 pH 的计算

**题目**：水在 373 K（100°C）的离子积 $K_w = 5.5 \times 10^{-13}$。求中性水的 pH。

**解题**：
- 中性条件：$[\mathrm{H^+}] = [\mathrm{OH^-}]$
- $[\mathrm{H^+}] = \sqrt{K_w} = \sqrt{5.5 \times 10^{-13}} = 7.4 \times 10^{-7}$ M
- $\mathrm{pH} = -\lg(7.4 \times 10^{-7}) = 6.13$

**关键认识**：
- 高温下中性 pH < 7！（不是 pH = 7）
- "中性"= $[\mathrm{H^+}] = [\mathrm{OH^-}]$，不是 pH = 7
- $K_w$ 随温度升高而增大（水的自耦电离是吸热过程）

| 温度 (K) | $K_w$ | 中性 pH |
|:---:|:---:|:---:|
| 298 | $1.0 \times 10^{-14}$ | 7.00 |
| 323 | $5.5 \times 10^{-14}$ | 6.63 |
| 373 | $5.5 \times 10^{-13}$ | 6.13 |
| 573 | $5.1 \times 10^{-12}$ | 5.65 |

## 十六、待完善项
- [x] 补充极稀溶液 pH 计算的例题——已补充 §15.1
- [x] 补充高温下中性 pH 的计算示例——已补充 §15.2
