...
importance: 5
has_images: true
key_images: ["[[media/zgxg-vsepr-summary-table.jpg]]", "[[media/zgxg-hybridization-types.jpg]]"]
status: 已填充
stage: published
...
| 7 | 0 | AX₇ | 五角双锥 | 五角双锥 | 72°, 90° | IF₇, [ZrF₇]³⁻ |
| 7 | 1 | AX₆E | 五角双锥 | 变形八面体 | — | [XeF₆]（争议结构） |
| 8 | 0 | AX₈ | 四方反棱柱 | 四方反棱柱 | — | [Mo(CN)₈]⁴⁻, XeF₈²⁻ |
| 8 | 0 | AX₈ | 正十二面体（三角十二面体） | 正十二面体 | — | [Mo(H₂O)₈]³⁺（少见） |

> **高配位数说明（CN=7,8）**：这些构型在初赛中极少出现，但在决赛中可能涉及配合物结构推断。五角双锥是 CN=7 最常见构型；CN=8 以四方反棱柱为主，少数为三角十二面体。实际结构需考虑电子效应和配体大小。详见 [[晶体场理论]] §5.3 Dq 参数表。
...
Berry 假旋转（Berry pseudorotation）是三角双锥（TBP）构型分子在不破坏键的情况下，通过中间体实现轴向和平伏位置互换的动态过程。

**过程描述**：
1. 起始：三角双锥，两个轴向配体 L_ax，三个平伏配体 L_eq
2. 中间体：四方锥（SP），原平伏的两个配体向轴向弯曲
3. 终态：新的三角双锥，原轴向和平伏位置互换

**关键特征**：
- 活化能约 20-30 kJ/mol（室温下快速进行）
- NMR 中观测到等价信号（如 $\ce{PF5}$ 在室温下只显示一个 $^{31}P$ 信号）
- 低温 NMR 可"冻结"假旋转，分辨轴向和平伏位（$\ce{PF5}$ 在 -100°C 显示两组信号）

**竞赛应用**：$\ce{PF5}$、$\ce{Fe(CO)5}$ 等三角双锥分子的 NMR 谱图解释。

### 16.2 竞赛真题

**例题 1**：预测 $\ce{XeF4}$ 的几何构型和孤对电子位置。

**解答**：Xe 有 8 个价电子，4 个用于成键，剩 4 个 = 2 对孤对电子。价层电子对数 = 6（4 bond + 2 lone pair）。根据 VSEPR，$AX_4E_2$ → 平面正方形（孤对电子占据轴向位置以最小化排斥）。

**例题 2**：$\ce{IF5}$ 和 $\ce{BrF5}$ 的几何构型是什么？哪个分子的键角更接近 90°？

**解答**：两者都是 $AX_5E_1$ → 四方锥。$\ce{IF5}$ 中 I-F 键角约 81.9°（孤对电子压缩），$\ce{BrF5}$ 中 Br-F 键角约 84.8°。$\ce{BrF5}$ 更接近 90°，因为 Br 的孤对电子排斥较弱（原子更大，电子云更分散）。

**例题 3**：为什么 $\ce{SF4}$ 的轴向 F-S-F 键角（173°）小于 180°？

**解答**：$\ce{SF4}$ 为 $AX_4E_1$（变形四面体/跷跷板形）。孤对电子占据一个平伏位置，其排斥力大于成键电子对，将轴向 F 压向平伏 F，使轴向键角从 180° 减小到 173°。

## 十七、待完善项
- [x] 补充 Berry 假旋转的图示 — 用文字描述机制过程，配 NMR 证据（§十六.1）
- [x] 补充 2-3 道初赛/决赛真题 — 见§十六.2 三道例题
