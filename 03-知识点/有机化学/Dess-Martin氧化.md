...
## 十四、待完善项
- [x] 补充 IBX 在加热条件下的"二醇裂解"应用
- [x] 补充 IBX 与 DMP 的环境与工艺规模评估
- [x] 补充 SIBX、mIBX 等改良试剂的对比

**IBX 二醇裂解应用**：
IBX 在加热条件（60-80°C，DMSO）下可实现 1,2-二醇的氧化裂解（C-C键断裂），生成两分子羰基化合物：
R₁R₂C(OH)-C(OH)R₃R₄ → IBX/Δ → R₁R₂C=O + R₃R₄C=O
机理：IBX 先氧化一个羟基为酮 → 形成 α-羟基酮中间体 → IBX 进一步氧化裂解 C-C 键。选择性优于 NaIO₄（可在含硫/含烯底物中使用）。

**IBX 与 DMP 的环境与工艺评估**：
| 维度 | IBX | DMP |
|:---|:---|:---|
| 溶解性 | 不溶于DCM/THF，需DMSO | 溶于DCM/THF，适用面广 |
| 安全性 | 冲击敏感（需与KBr混合使用） | 较稳定但仍需谨慎 |
| 副产物 | 2-碘苯甲酸（水溶性，易分离） | 2-碘苯甲酸 + AcOH |
| 原子经济性 | 差（MW 312） | 更差（MW 368） |
| 工艺规模 | SIBX更安全可行 | 多用于小规模实验室 |
| 成本 | 较低（仅需IBX一步） | 较高（IBX + AcOH/Ac₂O） |

**SIBX、mIBX 等改良试剂对比**：
| 改良试剂 | 组成/特征 | 优势 | 局限 |
|:---|:---|:---|:---|
| SIBX | IBX + KBr + 三聚氰胺复合物 | 不冲击敏感、可称量、DCM中悬浮可用 | 活性略低于纯 IBX |
| mIBX | IBX 在离子液体中的复合物 | 溶解性改善 | 成本高 |
| o-IBX-NH₂ | 氨基取代 IBX | 可溶于有机溶剂 | 合成步骤多 |
| DMP-MeOH | DMP 在 MeOH 中原位生成 | 直接使用，无需分离 DMP | 仅限 MeOH 溶液态 |
