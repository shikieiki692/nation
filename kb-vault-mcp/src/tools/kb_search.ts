 ...
 */
async function walkFiles(root: string, filter?: (name: string) => boolean): Promise<string[]> {
 ...
 * 执行 glob 搜索
 */
async function globSearch(pattern: string, vaultRoot: string, scope?: string): Promise<string[]> {
  const searchPath = scope ? path.join(vaultRoot, scope) : vaultRoot;
