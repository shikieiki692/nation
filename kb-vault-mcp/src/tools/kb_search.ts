...
export interface CrossRefResult {
  target: string;
  referencedBy: Array<{ file: string; line: number; linkText: string }>;
}
...
async function grepSearch(
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `grep -rn ${caseFlag} ${excludePattern} "${query}" "${searchPath}" --include="*.md"`,
 ...
 * 执行 glob 搜索
 */
async function globSearch(pattern: string, vaultRoot: string): Promise<string[]> {
  try {
    const { stdout } = await execAsync(
      `find "${vaultRoot}" -name "${pattern}" -type f ! -path "*/.git/*" ! -path "*/.claude/*" ! -path "*/.trash/*" ! -path "*/node_modules/*"`,
      { timeout: 10000 }
    );
 ...
 * 从缓存加载反向索引
 */
async function loadReverseIndex(vaultRoot: string): Promise<Record<string, Array<{ file: string; line: number; linkText: string }>> | null> {
  try {
 ...
 * 检查索引是否过期
 */
async function isIndexStale(vaultRoot: string): Promise<boolean> {
  try {
    const indexPath = path.join(vaultRoot, '.kb/state/reverse_index.json');
    ...
    switch (type) {
      ...
      case 'crossref': {
        ...
        if (index && !stale) {
          ...
          const indexData = (index.index || {}) as unknown as Record<string, any[]>;
