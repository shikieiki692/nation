 ...
 * 查询文件的反向引用数
 */
async function getRefCount(filePath: string, vaultRoot: string): Promise<number> {
  try {
    ...
    const { stdout } = await execAsync(
      `grep -r "\\[\\[${fileName}\\]\\]" "${vaultRoot}" --include="*.md" -l --exclude-dir=.git --exclude-dir=.claude --exclude-dir=.trash | wc -l`,
      { timeout: 10000 }
    );
 ...
 * 创建文件快照
 */
async function createSnapshot(filePath: string, vaultRoot: string): Promise<string> {
 ...
 */
async function softDeprecate(filePath: string, supersededBy?: string): Promise<void> {
