 ...
 * 扫描全库构建反向索引
 */
async function buildReverseIndex(vaultRoot: string): Promise<{
  index: Record<string, Array<{ file: string; line: number; linkText: string }>>;
  totalFiles: number;
  totalLinks: number;
}> {
  const index: Record<string, Array<{ file: string; line: number; linkText: string }>> = {};
  let totalFiles = 0;
  let totalLinks = 0;
 ...
 * 保存索引到文件
 */
async function saveIndex(vaultRoot: string, index: Record<string, any>): Promise<void> {
 ...
 * 检查索引状态
 */
async function getIndexStatus(vaultRoot: string): Promise<{ exists: boolean; stale: boolean; generatedAt?: string }> {
  try {
    const indexPath = path.join(vaultRoot, INDEX_FILE);
 ...
 * 处理 kb_index 请求
 */
export async function handleKbIndex(
  args: {
    action: 'rebuild' | 'status';
  },
  vaultRoot: string
