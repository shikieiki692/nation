...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case 'kb_read':
        result = await handleKbRead(args as any, stateManager, VAULT_ROOT);
        break;
        
      case 'kb_write':
        result = await handleKbWrite(args as any, stateManager, VAULT_ROOT);
        await hookRuntime.onOperationComplete();
        break;
        
      case 'kb_edit':
        result = await handleKbEdit(args as any, stateManager, VAULT_ROOT);
        await hookRuntime.onOperationComplete();
        break;
        
      case 'kb_bash':
        result = await handleKbBash(args as any, stateManager, VAULT_ROOT);
        break;
        
      case 'kb_session':
        result = await handleKbSession(args as any, stateManager);
        if ((args as any).action === 'init' && result.success) {
          await hookRuntime.onSessionInit();
        }
        break;
        
      case 'kb_gate':
        result = await handleKbGate(args as any, stateManager, hookRuntime, orchestrator, VAULT_ROOT);
        break;
        
      case 'kb_dirty':
        result = await handleKbDirty(args as any, stateManager);
        break;
        
      case 'kb_move':
        result = await handleKbMove(args as any, stateManager, VAULT_ROOT);
        if (result.success) {
          await hookRuntime.onOperationComplete();
        }
        break;
      ...
      case 'kb_search':
        // 只读工具，不检查状态
        result = await handleKbSearch(args as any, VAULT_ROOT);
        break;
        
      case 'kb_delete':
        result = await handleKbDelete(args as any, stateManager, VAULT_ROOT);
        if (result.success) {
          await hookRuntime.onOperationComplete();
        }
        break;
      ...
      case 'kb_index':
        // 只读工具，不检查状态
        result = await handleKbIndex(args as any, VAULT_ROOT);
        break;
