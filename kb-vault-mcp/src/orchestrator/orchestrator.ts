...
export class Orchestrator {
  ...
  private childArchivedCallbacks: Map<string, Set<string>> = new Map(); // parentId → Set<archivedChildId>
  private awaitingTimeouts: Map<string, NodeJS.Timeout> = new Map(); // parentId → timeout handle
  ...
  awaitChildren(parentTaskId: string): boolean {
    ...
    // 如果没有子Task，不需要等待
    if (parentTask.children.length === 0) return false;
   ...
   * 所有根Task ARCHIVED → 触发
   */
  async mergeOnClose(): Promise<{
    success: boolean;
    closingReport?: {
      step1_dirtyModules: string[];
      step2_closingChecklist: Array<{ target: string; mandatory: boolean; reason: string }>;
    ...
    const allDirtyModules = new Set<string>();
   ...
   * 生成收工清单
   */
  private generateClosingChecklist(rootTasks: Task[]): Array<{ target: string; mandatory: boolean; reason: string }> {
    const checklist: Array<{ target: string; mandatory: boolean; reason: string }> = [];
    const allDirtyModules = new Set<string>();
