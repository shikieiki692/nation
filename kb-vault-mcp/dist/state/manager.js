...
export class StateManager {
    ...
    createTask(params) {
        ...
        const task = {
            ...
            persistence: params.persistence || 'FORMAL',
     ...
     * 标记脏模块
     */
    markDirtyModule(module, file, changeType) {
     ...
     * 更新读取注册表
     */
    updateReadRegistry(path, fingerprint) {
        if (this.session) {
     ...
     * 这是核心的隐式验证链
     */
    validateModification(operation, targetPath) {
     ...
     * 验证 intentType 约束
     */
    validateIntentConstraint(task, targetPath, operation) {
        const normalizedPath = targetPath.replace(/\\/g, '/');
        // USE 意图禁止写入 03-知识点/
     ...
     */
    async rotateCheckpoint(checkpointPath, lines) {
     ...
     * 计算文件指纹
     */
    async computeFingerprint(filePath, content) {
