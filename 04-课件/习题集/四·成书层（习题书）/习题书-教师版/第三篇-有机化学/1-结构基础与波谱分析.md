...
> **题量**：278 ｜ **难度**：d1=0 d2=34 d3=177 d4=50 d5=17
...
烷烃 $\mathrm{C_6H_{14}}$ 共有 5 个构造异构体，其结构与系统命名如下：

1. $\mathrm{CH_3CH_2CH_2CH_2CH_2CH_3}$：**己烷**（正己烷，Hexane）
2. $\mathrm{(CH_3)_2CHCH_2CH_2CH_3}$：**2-甲基戊烷**（2-Methylpentane）
3. $\mathrm{CH_3CH_2CH(CH_3)CH_2CH_3}$：**3-甲基戊烷**（3-Methylpentane）
4. $\mathrm{(CH_3)_3CCH_2CH_3}$：**2,2-二甲基丁烷**（2,2-Dimethylbutane）
5. $\mathrm{(CH_3)_2CHCH(CH_3)_2}$：**2,3-二甲基丁烷**（2,3-Dimethylbutane）
...
**6-2** 显然 D 为氯代亚硫酸酯中间体，接着由于位阻过大，Cl 没有直接进攻，而是进行了 $S_{\mathrm{N}}2$' 反应——当 $S_{\mathrm{N}}2$ 不容易，且 $S_{\mathrm{N}}2$' 比较容易时，发生 $S_{\mathrm{N}}2$'。
...
**10-2** 催化循环如下：先形成活性 P-I 中间体，发生 P 上的 $S_{\mathrm{N}}2$ 反应，接着得到 Ph₃P（循环图见原书）。
...
**2-1-2** 尝试画出正一价金与三键形成的轨道相互作用。金用它的空 d 轨道接受三键的 HOMO，再用自身 HOMO 轨道与三键结合。为什么该结合的效果比较好？类似的，有什么东西能够比金更好的与三键结合，从而实现对三键温和的加水反应？提示：直接用 $d_{xy}$ 轨道来分析。
...
$\mathrm{S_N2}$ 机理 → **Walden 翻转**（构型完全反转）。DMSO 是极性非质子溶剂，促进 $\mathrm{S_N2}$。$\mathrm{NaOH}$ 提供 $\mathrm{OH}^-$ 亲核试剂。底物为 $(R)$-2-溴丁烷 → 产物为 $(S)$-2-丁醇。
...
![[f92a4c54980381e6a1f57641aac265ff260d7ac58c9eabc554e95d177a245781.jpg]]
...
![[5e39270bcbe5615642d85eb75d09661ef80195aa30f924b333aafcc51b76aa82.jpg]]
...
>
> **原文 OA 错误已修正**：原卷答案印作 "a、b、c、**c**"——选项 (c) 重复出现，属于笔误。按题意，(e)"混合蒸气进入分馏柱后发生连续热交换"是分馏的**核心机理**（正是靠这种连续的热交换实现多次气化-冷凝），必须选入。(d)"分馏柱中必须有冷凝水"并非必需（分馏柱靠自然散热形成温度梯度）。故正确答案为 **a、b、c、e**。
...
## 1.249 Frustulosin（含炔抗生素）结构最终挑战
...
5. β-碎裂消除 → 释放 $\mathbf{B}^{\bullet*}$ → 得到产物烯烃
