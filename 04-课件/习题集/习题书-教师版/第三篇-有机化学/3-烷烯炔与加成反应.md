...
化合物 A 与 B 的相对分子质量均为 68，且具有相同的碳架。A 和 B 都能使溴的四氯化碳溶液褪色。A 与 $[\mathrm{Ag}(\mathrm{NH}_{3})_{2}]^{+}$ 溶液反应生成沉淀，A 经 $KMnO_{4}$ 热溶液氧化得 $CO_{2}$ 和 $Me_{2}CHCOOH$ 。B 不与银氨溶液反应，与 $1\ mol\ Br_{2}$ 反应可得六种产物。写出 A 与 B 的结构。
