...
$$
) P t | H _ {2} (p ^ {\Theta}) | H ^ {+} (10 ^ {- 3} m o l / d m ^ {3}) | | H ^ {+} (10 ^ {- 2} m o l / d m ^ {3}) | H _ {2} (p ^ {\Theta}) | P t (+)
$$
...
$$
\mathrm{I} _ {2} + 2 \mathrm{e} - 2 \mathrm{I} ^ {-} \quad E ^ {\ominus} = 0.54 \mathrm{V}
$$
...
$$
\begin{array}{l} \mathrm{Cr} _ {2} \mathrm{O} _ {7} ^ {2 -} + 14 \mathrm{H} ^ {+} + 6 \mathrm{e} - 2 \mathrm{Cr} ^ {3 +} + 7 \mathrm{H} _ {2} \mathrm{O} \\ \mathrm{CrO} _ {4} ^ {2 -} + 4 \mathrm{H} _ {2} \mathrm{O} + 3 \mathrm{e} - \mathrm{Cr} (\mathrm{OH}) _ {3} + 5 \mathrm{OH} ^ {-} \\ \mathrm{CrO} _ {2} ^ {-} + 2 \mathrm{H} _ {2} \mathrm{O} + 3 \mathrm{e} - \mathrm{Cr} + 4 \mathrm{OH} ^ {-} \\ \end{array}
$$
...
$$
E ^ {\ominus} = 0.80 \mathrm{V}, K _ {\mathrm{sp}} (\mathrm{AgI}) = 8.5 \times 10 ^ {- 17}
$$
...
$$
\begin{array}{r l} \mathbf {O} _ {2} + 4 \mathbf {H} ^ {+} + 4 \mathbf {e} & = 2 \mathbf {H} _ {2} \mathbf {O} \\ & E ^ {\ominus} = 1.23 \mathbf {V} \end{array}
$$
...
$$
\mathbf {C u} ^ {2 +} + 2 \mathrm{e} = \mathbf {C u} E ^ {\ominus} = 0.34 \mathrm{V}
$$
...
$$
\mathrm{BrO} _ {4} ^ {-} \xrightarrow {+ 1.85} \mathrm{BrO} _ {3} ^ {-} \xrightarrow {+ 1.45} \mathrm{HBrO} \xrightarrow {+ 1.60} \mathrm{Br} _ {2} ^ {+ 1.07} \mathrm{Br} ^ {-}
$$
...
$$
\mathrm{BrO} _ {4} ^ {-} \xrightarrow {+ 1.85} \mathrm{BrO} _ {3} ^ {-} \xrightarrow {+ 1.45} \mathrm{HBrO} \xrightarrow {+ 1.60} \mathrm{Br} _ {2} \xrightarrow {+ 1.07} \mathrm{Br} ^ {-}
$$
...
$$
\mathrm{H} _ {5} \mathrm{IO} _ {6} \xrightarrow [ ]{+ 1.60} \mathrm{IO} _ {3} ^ {-} \xrightarrow [ ]{+ 1.13} \mathrm{HIO} \xrightarrow [ ]{+ 1.44} \mathrm{I} _ {2} \xrightarrow [ + 0.99 ]{+ 0.54} \mathrm{I} ^ {-}
$$
...
$$
\mathrm{H} _ {3} \mathrm{IO} _ {6} ^ {2 -} \xrightarrow {+ 0.70} \mathrm{IO} _ {3} ^ {-} \xrightarrow {+ 0.15} \mathrm{IO} ^ {-} \xrightarrow {+ 0.43} \mathrm{I} _ {2} \xrightarrow {+ 0.54} \mathrm{I} ^ {-}
$$
...
| --- | --- | --- | --- | --- |
| 能斯特方程 | $\varphi = \varphi^{0} + (0.0591/n)log(氧化型/还原型)$ | 对数项系数计算/氧化还原型判断 | ★★★ |  |
| 离子热力学函数 | 规定H+(1M)的 $\Delta fG^{0} = 0$ 作为基准 | 相对值计算/与其他热力学函数关系 | ★★★ |  |
| 浓差电池 | 相同电极不同浓度产生的电动势 | 正负极判断/电动势计算 | ★★★ |  |
| 电势-pH图 | 横坐标pH值,纵坐标电极电势 | 斜率意义/氢线氧线划分 | ★★★★★ |  |
| 沉淀对电势影响 | Ksp导致离子浓度变化影响φ值 | AgCl/AgI体系计算 | ★★★ |  |
| 元素电势图 | 酸性(φA)和碱性(φB)两种图示 | 歧化反应判断/中间价态稳定性 | ★★★★★ |  |
| 电池类型 | 锌锰/铅酸/锂离子/燃料电池 | 电极反应书写/充放电区别 | ★★★★ |  |
| 超电势 | 实际分解电压与理论值差值 | 气体析出超电势显著 | ★★ |  |
| 典型电池 | 正极材料 | 负极材料 | 电解质 | 总反应 |
| ---- | ---- | ---- | ---- | ---- |
| 锌锰干电池 | 碳棒(MnO2) | 锌皮 | NH4Cl | Zn+2MnO2→ZnO+Mn2O3 |
| 铅蓄电池 | PbO2 | Pb | H2SO4 | PbO2+Pb+2H2SO4↔2PbSO4+2H2O |
| 锂离子电池 | LiCoO2 | 石墨嵌锂 | 有机电解液 | LixC6+Li1-xCoO2↔6C+LiCoO2 |
| 电极反应对比 | 酸性条件 | 碱性条件 |  |  |
| ---- | ---- | ---- |  |  |
| 氧电极 | O2+4H++4e-→2H2O | O2+2H2O+4e-→4OH- |  |  |
| 氢电极 | 2H++2e-→H2 | 2H2O+2e-→H2+2OH- |  |  |
