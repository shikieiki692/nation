...
☐ 推导钙半径：CaS的 $a = 5.68\text{Å}$ 明显增大，说明钙离子撑开硫离子，用 $a = 2(r_{Ca^{2+}} + r_{S^{2-}})$ 反推  
...
$$
r _ {C a ^ {* +}} + r _ {S ^ {*}} = r _ {C a ^ {* +}} + 1.84 = 5.68 / 2
$$
...
$$
r _ {C a ^ {+ +}} + r _ {O ^ {-}} = 1.00 + r _ {O ^ {-}} = 4.80 / 2 \quad r _ {O ^ {-}} = 1.40 \text{Å}
$$

$$
r _ {M n ^ {+ +}} + r _ {O ^ {-}} = r _ {M n ^ {+ +}} + 1.40 = 4.44 / 2 \quad r _ {M n ^ {+ +}} = 0.82 \text{Å}
$$

⑤如果MgO为(a)(b)，则有 $1.4142*4.21 = 5.95 \neq 1.40*4 = 5.60$ 因而MgO属(c)

$$
r _ {M g ^ {+ +}} + r _ {O ^ {-}} = r _ {M g ^ {+ +}} + 1.40 = 4.21 / 2
$$

半径影响规律：晶胞参数随正离子半径增大而增大，如比较MnS和CaS时， $r_{Ca^{2+}} = 1.00\text{Å}$ ， $r_{Ca^{2+}} + r_{S^{2-}} = 5.68 / 2$   
...
| 正离子配位多面体 | 直线型 | 三角形 | 正四面体 | 正八面体 | 立方体 | 最密堆积 |
| --- | --- | --- | --- | --- | --- | --- |
| 正离子配位数 $r_{\mathrm {a}}/r_{-}$ | 2 | 3 | 4(ZnS)0.155 | 6(NaCl)0.225 | 8(CsCl, CaF2)0.4140.732 | 121 |
...
|  | 组成比 ${n}_{ + } : {n}_{ - }$ | 配位数比 |  | 电价比 ${W}_{ + } : {W}_{ - }$ |
| --- | --- | --- | --- | --- |
|  | 组成比 ${n}_{ + } : {n}_{ - }$ | $C{N}_{ + }$ | $C{N}_{ - }$ | 电价比 ${W}_{ + } : {W}_{ - }$ |
| CsCl型 | 1:1 | 8 | 8 | 1:1 |
| NaCl型 | 1:1 | 6 | 6 | 1:1 |
| 六方ZnS型 | 1:1 | 4 | 4 | 2:2 |
| 立方ZnS型 | 1:1 | 4 | 4 | 2:2 |
| ${\mathrm{CaF}}_{2}$ 型 | 1:2 | 8 | 4 | 2 |
| 金红石型 | 1:2 | 6 | 3 | 4 |
...
| --- | --- | --- | --- |
| 氯化钠结构 | 立方晶系,立方面心点阵,6:6配位,面心立方最密堆积(CCP),100%占据八面体空隙 | 精细与点阵形式区分(立方晶系≠立方面心) | ★★★ |
| 立方硫化锌结构 | 立方晶系,两套等同点(Zn/S),四面体空隙50%占据,配位比4:4 | 坐标变换技巧(平移顶点后新坐标计算) | ★★★★ |
| 氟化钙结构 | 立方晶系,三套等同点(Ca/F),简单立方堆积,立方体空隙50%占据 | 顶点转换分析(以F为顶点时堆积方式变化) | ★★★★ |
| 六方硫化锌结构 | 六方晶系,四套等同点(Zn/S),ABAB堆积,四面体空隙50%占据 | 分数坐标特殊值(八分之奇数列) | ★★★★ |
| 金红石结构 | 四方晶系,变形八面体空隙,伪六方堆积,配位比6:3 | 非整数坐标计算(u≈0.3) | ★★★★ |
| 半径比规则 | 配位数与r+/r-关系:四面体(0.225-0.414)/八面体(0.414-0.732) | 临界值记忆(0.155/0.225/0.414/0.732) | ★★★★ |
| 离子极化效应 | 导致配位数降低(如AgI实际为ZnS型而非NaCl型) | 极化能力:小半径/高电荷阳离子更强 | ★★★ |
| 石墨结构 | 六方vs三方堆积,层间不完全重合,导电性差异 | ABAB与ABC堆积区分 | ★★★ |
| 金刚石结构 | A4型堆积,面心立方+50%四面体空隙填充 | 空间利用率34.01% | ★★★ |
| 电价规则 | $\Sigma$ (正离子电价/配位数)=负离子电价(如钙钛矿稳定性验证) | 配位数双重计算( $CaTiO_3$ 中O的配位环境) | ★★★★ |