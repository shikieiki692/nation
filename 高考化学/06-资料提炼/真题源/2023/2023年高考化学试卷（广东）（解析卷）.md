...
| C | 家务劳动:擦干已洗净的铁锅,以防生锈 | 铁丝在 $O_{2}$ 中燃烧生成 $Fe_{3}O_{4}$ |
| D | 学农活动:利用秸秆、厨余垃圾等生产沼气 | 沼气中含有的 $CH_4$ 可作燃料 |
...
A. $1\ \mathrm{mol}\ NH_{4}Cl$ 含有的共价键数目为 $5N_{A}$ B. $1\ \mathrm{mol}\ NaHCO_{3}$ 完全分解，得到的 $CO_{2}$ 分子数目为 $2N_{A}$ C. 体积为1 L 的 $1\ \mathrm{mol}\cdot \mathrm{L}^{-1}\ NaHCO_{3}$ 溶液中， $HCO_{3}^{-}$ 数目为 $N_{A}$ D. NaCl 和 $NH_{4}Cl$ 的混合物中含 $1\ \mathrm{mol}\ Cl^{-}$ ，则混合物中质子数为 $28N_{A}$

【详解】A．铵根中存在4个N-H共价键，1mol $NH_{4}Cl$ 含有的共价键数目为 $4N_{A}$ ，A错误；
B．碳酸氢钠受热分解生成碳酸钠、水和二氧化碳，1mol $NaHCO_{3}$ 完全分解，得到 $0.5 \, \mathrm{mol} CO_{2}$ 分子，B 错误；
...
| B | 装有 $NO_2$ 的密闭烧瓶冷却后颜色变浅 | $NO_2$ 转化为 $N_2O_4$ 的反应吸热 |
| C | 久置空气中的漂白粉遇盐酸产生 $CO_2$ | 漂白粉的有效成分是 $CaCO_3$ |
...
| ii | 0.20 mol·L-1CuSO4溶液100 mL | 0.56 g Fe粉 | a | c |
...
由 $Q=cpV_{总}\cdot\Delta T$ 可得 $Q=4.18\ J\cdot g^{-1}\cdot^{\circ}C^{-1}\times1.0\ g\cdot \mathrm{mL}^{-1}\times(50\mathrm{mL}+50\mathrm{mL})\times(T_{1}-T_{2})=418(T_{1}-T_{0})$ ，故答案为 $418(T_{1}-T_{0})$ ;
...
| 氢氧化物 | $Co(OH)_2$ | $Co(OH)_3$ | $Ni(OH)_2$ | $Al(OH)_3$ | $Mg(OH)_2$ |
| --- | --- | --- | --- | --- | --- |
| $K_{sp}$ | $5.9×10^{-15}$ | $1.6×10^{-44}$ | $5.5×10^{-16}$ | $1.3×10^{-33}$ | $5.6×10^{-12}$ |
...
$$
\begin{array}{r l} & 2 \mathrm{Co(OH)} _ {3} + 12 \mathrm{NH} _ {3} \cdot \mathrm{H} _ {2} \mathrm{O} + \mathrm{SO} _ {3} ^ {2 -} = 2 \left[ \mathrm{Co(NH} _ {3}) _ {6} \right] ^ {2 +} + \mathrm{SO} _ {4} ^ {2 -} + 13 \mathrm{H} _ {2} \mathrm{O} + 4 \mathrm{OH} ^ {-} \text {或} \\ & 2 \mathrm{Co(OH)} _ {3} + 8 \mathrm{NH} _ {3} \cdot \mathrm{H} _ {2} \mathrm{O} + 4 \mathrm{NH} _ {4} ^ {+} + \mathrm{SO} _ {3} ^ {2 -} = 2 \left[ \mathrm{Co(NH} _ {3}) _ {6} \right] ^ {2 +} + \mathrm{SO} _ {4} ^ {2 -} + 13 \mathrm{H} _ {2} \mathrm{O}. \end{array}
$$
...
用浓度分别为 $2.0 \, \mathrm{mol} \cdot \mathrm{L}^{-1}$ 、 $2.5 \, \mathrm{mol} \cdot \mathrm{L}^{-1}$ 、 $3.0 \, \mathrm{mol} \cdot \mathrm{L}^{-1}$ 的 $HNO_{3}$ 溶液进行了三组实验，得到 $\mathrm{c}\left(\left[\mathrm{FeR}_{3}\right]^{2+}\right)$ 随时间 t 的变化曲线如图。

![](2023年高考化学试卷（广东）（解析卷）_images/77ac0f55fefe8f75c89792eb5fc434be1e67afa018c043bbbd8308ce3ad0320c.jpg)

① $c(HNO_{3})=3.0\ \mathrm{mol}\cdot \mathrm{L}^{-1}$ 时，在 $0\sim1\ min$ 内， $\left[FeR_{3}\right]^{2+}$ 的平均消耗速率=\_\_\_\_。
...
$c_{0}(M)=0.51c_{0}(L)=5.1\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1};\quad c_{平}(ML)=c_{平}(L)=0.4c_{0}(L)=4.0\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1},$ 由 L 守恒可知 $c_{平}(ML_{2})=\frac{1\times10^{-5}\mathrm{mol}\cdot \mathrm{L}^{-1}\cdot(4\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1}+4\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1})}{2}=1.0\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1}$ ，则 $c_{转}(M)=4.0\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1}+1.0\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1}=5.0\times10^{-6}\mathrm{mol}\cdot \mathrm{L}^{-1};$ 则 M 的转化率为 $\frac{5.0\times10^{-6}}{5.1\times10^{-6}}\times100\%\approx98\%$
