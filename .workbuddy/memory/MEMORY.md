...
- **等宽段口径（2026-09-11 实测纠正，原任务卡有误）**：81 份活跃产物 `VerbatimChar` 段 818 = **真代码块（pStyle=SourceCode）158 + 正文夹行内反引号 660**（后者本就是正文、无围栏可拆）。**pandoc 把整个代码块塞进一个 `<w:p>`（块内换行用 `<w:br/>`）→ 1 代码块 = 1 段**。**活跃产物不存在 Courier 等宽渲染**（`styles.xml` 无任何等宽字体）。工具在 `.workbuddy/tmp/`：`verbatim_report.py`（产物侧按 w:pStyle 分两类；基线 `verbatim_baseline_pre.json`）、`verbatim_worklist.py`（源侧按 ``` 切块带行号 + 逐 stem 对账 + 未闭合围栏检测）。
  ...
  - **批次1–10 累计**：全库等宽段 818→**706**，真代码块 158→**46**（−71%），行内 660 未动；**源 46 块 ↔ 产物 46 段 0 偏差**。批次量 13/38/21/2/13 + **人工定型 24 块**（批次7–10，`manual_batch7/8/9/10.py`，含"围栏位置+块内原文"双断言，全部校验通过才统一落盘）。
  ...
  - **阶段5（ASCII 图→图片）普查：低产、须人工、建议另立卡**。主题级命中 87%，但**精确匹配实测约 2/46**（教学专用图；库图多是同主题的别的反应/构型：卡宾只有三重态加成、多碘只有 I₇⁻、催化循环只有 Monsanto）。工具 `img_hit_survey.py`/`img_hit_detail.py`。**保留的 46 段已不是"错位"问题**：字体修复后全为 `SimSun` 等宽网格（已核验），只剩"ASCII 观感 vs 真图片"的取舍。唯一确认的精确匹配：八面体场 d 轨道分裂图 → `5f17019ed…jpg`。已拆出独立卡 `00-首页/活跃任务/任务卡-2026-09-11-讲义ASCII图转图片.md`（含 46 块逐块清单）。
