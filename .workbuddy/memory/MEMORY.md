...
- **等宽段口径（2026-09-11 实测纠正，原任务卡有误）**：81 份活跃产物 `VerbatimChar` 段 818 = **真代码块（pStyle=SourceCode）158 + 正文夹行内反引号 660**（后者本就是正文、无围栏可拆）。**pandoc 把整个代码块塞进一个 `<w:p>`（块内换行用 `<w:br/>`）→ 1 代码块 = 1 段**。**活跃产物不存在 Courier 等宽渲染**（`styles.xml` 无任何等宽字体）。工具在 `.workbuddy/tmp/`：`verbatim_report.py`（产物侧按 w:pStyle 分两类；基线 `verbatim_baseline_pre.json`）、`verbatim_worklist.py`（源侧按 ``` 切块带行号 + 逐 stem 对账 + 未闭合围栏检测）。
  ...
  - **批次1–11 累计（收官）**：全库等宽段 818→**705**，真代码块 158→**45**（−71.5%），行内 660 未动；**源 45 块 ↔ 产物 45 段 0 偏差**。批次量 13/38/21/2/13 + **人工定型 25 块**（批次7–11，`manual_batch7/8/9/10/11.py`，含"围栏位置+块内原文"双断言，全部校验通过才统一落盘）。批次11 还把八面体场 d 轨道分裂图换成了图谱库已核验图片 `5f17019ed…jpg`（按 sha256 确认真实嵌入 docx）。
  - **关键教训：分型桶一个都不可信**。`文字型`/`反应式` 里混骨架式；`结构图/连线`/`几何/结构式` 里藏着「①②③ + `│ ▼`」的**催化循环步骤**与**框线决策流程图**（本质都是列表）。每轮"以为转完了"后把剩余块 dump 出来**逐块人眼再过一遍**都还能再取约 10 块。
  - **另一条教训：转表格前先查上方是否已有同内容表格**（实测 `有机化学基础.md` L160-165 在 L152-157 已有带 `\ce{}` 的三列表格，盲转既重复又丢 LaTeX，已回退）。「去围栏」≠「转表格」。
  ...
  - **阶段5（ASCII 图→图片）普查：低产、须人工、建议另立卡**。主题级命中 87%，但**精确匹配实测约 2/46**（教学专用图；库图多是同主题的别的反应/构型：卡宾只有三重态加成、多碘只有 I₇⁻、催化循环只有 Monsanto）。工具 `img_hit_survey.py`/`img_hit_detail.py`。**保留的 45 段已不是"错位"问题**：字体修复后全为 `SimSun` 等宽网格（已核验），只剩"ASCII 观感 vs 真图片"的取舍。唯一确认的精确匹配：八面体场 d 轨道分裂图 → `5f17019ed…jpg`。已拆出独立卡 `00-首页/活跃任务/任务卡-2026-09-11-讲义ASCII图转图片.md`（含 46 块逐块清单）。
