...
覆盖七组能力（均为 2026-09-21 格式线所加/所修）：
  ...
  ⑦ **箭头标注 `->[….]` 也要补下标**：`->[Zn/H2O]` → `Zn/H_{2}O`（此前带标点的一律原样返回）
...
反向用例（同等重要，防"按形状猜"）：
  ...
  - **`S_N2` / `S_N1` 的 `2`/`1` 是全尺寸**（反应类型记法），显式下标组 `_{…}` 之后的数字
    绝不能再被下标（`S_{N}2`，不是 `S_{N}_{2}`）；但同位素 `^{288}115` 的 `115` **要**下标
  - 中文标注（`->[Et2O 或 THF]`）走 `\\text{}`，逐字不变（属另一口径，见工单）
...
# ── ⑦ 正向：箭头标注（`->[…]`）也要补下标 ────────────────────
# 此前只在标注**无标点**时才走 `_parse_species`，带 `/` `,` `(` 的一律原样返回 ⇒ 丢下标。
check("标注·Zn/H2O", conv("A ->[Zn/H2O] B"), "$A " + BS + "xrightarrow{Zn/H_{2}O} B$")
check("标注·多组分", conv("A ->[H2O2, NaOH] B"), "$A " + BS + "xrightarrow{H_{2}O_{2}, NaOH} B$")
check("标注·配体", conv("A ->[Pd(PPh3)4] B"), "$A " + BS + "xrightarrow{Pd(PPh_{3})_{4}} B$")
check("标注·带序号", conv("A ->[1) Hg(OOCCH3)_2, H2O] B"),
      "$A " + BS + "xrightarrow{1) Hg(OOCCH_{3})_{2}, H_{2}O} B$")
check("标注·电荷并组", conv("A ->[H3O^+] B"), "$A " + BS + "xrightarrow{H_{3}O^{+}} B$")
