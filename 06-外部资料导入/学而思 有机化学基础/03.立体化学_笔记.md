...
| --- | --- | --- |
| Y | 1,3-Diaxial strain |  |
| Y | (kJ/mol) | (kcal/mol) |
| F | 0.5 | 0.12 |
| Cl, Br | 1.0 | 0.25 |
| OH | 2.1 | 0.5 |
| ${\mathrm{CH}}_{2}$ | 3.8 | 0.9 |
...
- 溶液浓度：浓度越高旋光度越大，计算公式中c为样品浓度 $(g/\mathrm{cm}^{3})$   
◦ 样品管长度：以分米(dm)为单位，长度越长旋光度越大  
○ 测定温度：温度会影响分子构象从而影响旋光度  
光源波长：常用钠光D线( $\lambda = 589\mathrm{nm}$ )
...
○ c: 样品浓度 $(g/\mathrm{cm}^{3})$

● 标准条件：浓度 $1g/\mathrm{cm}^{3}$ ，管长1dm时的旋光度  
...
$\bullet$ 浓度 $c = 1.20g / 7.50\mathrm{cm}^3 = 0.160g / \mathrm{cm}^3$   
● 管长l=5.00cm=0.500dm  
...
| --- | --- | --- | --- |
| 环烷烃构象分析 | 环丙烷平面构型不稳定(60°键角张力);环丁烷非平面构型(88°键角减少氢斥力);环己烷椅式构象最稳定(1,3-直立键排斥) | 环己烷a键/e键空间分布规律 | ★★★★ |
| 手性分子判定 | 分子与镜像不重合性质;四个不同取代基的碳中心;对称面/中心/反轴检测法 | 假手性碳判断(如内消旋化合物) | ★★★★☆ |
| 构型标记法 | R/S命名规则(次序规则→最小基团远离→顺时针R/逆时针S);Fischer投影式转换技巧 | 多手性中心化合物的系统命名(如2R,3S) | ★★★★★ |
| 旋光性原理 | 平面偏振光旋转现象;比旋光度计算公式: $[\alpha]=\alpha/(1\cdot c)$ ;外消旋体与内消旋体区别 | 旋光度测量影响因素(浓度/光程/温度) | ★★★☆ |
| 药物手性案例 | 沙利度胺事件(R构型治疗孕吐,S构型致畸);左旋多巴(有效)vs右旋多巴(无效) | 酶对特定构型的识别机制 | ★★★★ |
| 构象异构体 | 乙烷交叉式/重叠式(12kJ/mol能垒);环己烷椅式/船式(23kJ/mol能差) | 取代环己烷1,3-二直立键排斥 | ★★★★ |
| 不对称合成 | 化学拆分法(形成非对映异构体盐);手性助剂诱导(如Evans辅基) | 对映体过量率(ee%)计算 | ★★★★★ |
| 特殊手性体系 | 丙二烯型/联苯型/螺环型轴手性;氮/磷中心手性(能垒差异) | S4反轴分子(无手性碳但有手性) | ★★★★★☆ |
| 立体电子效应 | SN2反应的瓦尔登翻转;烯烃顺反加成立体选择性 | 消除反应的反式共平面要求 | ★★★★☆ |
| 构象动态平衡 | 环己烷构象翻转(a键/e键互换);受阻旋转(如邻位取代联苯) | 能垒测定方法(DNMR) | ★★★★ |