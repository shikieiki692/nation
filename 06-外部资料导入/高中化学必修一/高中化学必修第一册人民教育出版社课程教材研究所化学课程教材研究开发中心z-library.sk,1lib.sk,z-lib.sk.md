...
| 概念 | 电离(以NaCl的电离为例) | 离子反应(以 $Na_{2}SO_{4}$ 溶液与 $BaCl_{2}$ 溶液的反应为例) |
| --- | --- | --- |
| 宏观现象 | NaCl溶于水或受热熔化时能导电 | 生成白色沉淀 |
| 微观实质 | NaCl溶于水或受热熔化时发生电离,形成自由移动的 $Na^{+}$ 和 $Cl^{-}$ | $Ba^{2+}$ 与 $SO_{4}^{2-}$ 发生反应生成 $BaSO_{4}$ |
| 符号表征 | 电离方程式: $NaCl = Na^{+} + Cl^{-}$ | 离子方程式: $Ba^{2+} + SO_{4}^{2-} = BaSO_{4} \downarrow$ |
...
| 盐 | $Na_{2}CO_{3}$ | $NaHCO_{3}$ |
| --- | --- | --- |
| (1)观察 $Na_{2}CO_{3}$ 和 $NaHCO_{3}$ 的外观并进行描述 |  |  |
...
| --- | --- | --- |
| 1向盛有 $Na_{2}CO_{3}$ 溶液的试管中滴加澄清石灰水 |  |  |
| 2向盛有 $Na_{2}CO_{3}$ 溶液的试管中滴加 $CaCl_{2}$ 溶液 |  |  |
| 3向盛有 $Na_{2}CO_{3}$ 溶液的试管中滴加稀盐酸 | 开始无气泡产生,一段时间后产生气泡 | 1 $CO_{3}^{2-}+H^{+}=HCO_{3}^{-}$ 2____ |
...
| 有效成分的化学式 | NaCl | $Na_{2}CO_{3}$ | $Al(OH)_{3}$ | $Ca(ClO)_{2}$ |
...
| --- | --- | --- | --- | --- |
| $V[\text{NaOH(aq)}]/\text{mL}$ | 13.0 | 12.9 | 13.1 | 13.0 |
...
| --- | --- | --- | --- | --- |
| 1 | 在 $FeCl_{2}$ 溶液中加入锌片 | 溶液由浅绿色变为无色 | $Zn + Fe^{2+}=Zn^{2+} + Fe$ |  |
| 2 |  |  |  | $Fe^{2+}$ 具有还原性 |
| 3 | 在 $FeCl_{3}$ 溶液中加入足量铁粉 | 铁粉逐渐溶解,溶液由棕黄色变为浅绿色 |  | $Fe^{3+}$ 具有氧化性;Fe具有还原性 |
...
|  | 2Na | + | $2{\mathrm{H}}_{2}\mathrm{O}$ | == | 2NaOH | + | ${\mathrm{H}}_{2} \uparrow$ |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 化学计量数之比 | 2 | : | 2 | : | 2 | : | 1 |
| 扩大 ${6.02} \times {10}^{23}$ 倍 | $2 \times {6.02} \times {10}^{23}$ | : | $2 \times {6.02} \times {10}^{23}$ | : | $2 \times {6.02} \times {10}^{23}$ | : | $1 \times {6.02} \times {10}^{23}$ |
...
| 碱金属单质 | 颜色(常态) | $\frac{\text{密度}}{\left( {\mathrm{g} \cdot {\mathrm{cm}}^{-3}}\right) }$ | $\frac{\text{熔点}}{\mathrm{C}}$ | $\frac{\text{沸点}}{\mathrm{C}}$ |
| --- | --- | --- | --- | --- |
...
| 卤素 $^{\text{1}}$ 单质 | 颜色(常态) | 密度 | $\frac{\text{熔点}}{\text{°C}}$ | $\frac{\text{沸点}}{\text{°C}}$ |
| --- | --- | --- | --- | --- |
| $F_{2}$ | 淡黄绿色(气体) | 1.69 g/L(15°C) | -219.6 | -188.1 |
| $Cl_{2}$ | 黄绿色(气体) | 3.214 g/L(0°C) | -101 | -34.6 |
| $Br_{2}$ | 深红棕色(液体) | 3.119 g/cm $^{3}$ (20°C) | -7.2 | 58.78 |
| $I_{2}$ | 紫黑色(固体) | 4.93 g/cm $^{3}$ | 113.5 | 184.4 |
...
| --- | --- | --- | --- | --- |
| 1~2 | 1 | $1 \rightarrow 2$ | —— | +1——→0 |
| 3~10 |  |  | 0.152 nm——→0.071 nm大——→小 | +1——→+5-4——→-1——→0 |
| 11~18 |  |  |  |  |
...
| 非金属元素 | Si | P | S | Cl |
| --- | --- | --- | --- | --- |
| 最高价氧化物对应的水化物(含氰酸)的酸性强弱 | $H_{2}SiO_{3}$ (硅酸)弱酸 | $H_{3}PO_{4}$ (磷酸)中强酸 | $H_{2}SO_{4}$ (硫酸)强酸 | $HClO_{4}$ (高氯酸)强酸(酸性比 $H_{2}SO_{4}$ 强) |
