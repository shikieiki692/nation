...
| 类型 | 个数 | 类型 | 个数 |
| --- | --- | --- | --- |
| $Ma_{2}b_{2}cd$ | 8(6+2) | $M(AA)b_{2}c_{2}$ | 4(3+1) |
| $Ma_{2}b_{2}c_{2}$ | 6(5+1) | $M(AA)b_{2}cd$ | 6(4+2) |
| $Ma_{3}bcd$ | 5(4+1) | $M(AA)b_{3}c$ | 2(2+0) |
| $Ma_{3}b_{2}c$ | 3(3+0) | $M(AA)_{2}b_{2}$ | 3(2+1) |
| $Ma_{3}b_{3}$ | 2(2+0) | $M(AA)_{3}$ | 2(1+1) |
| $Ma_{4}bc$ | 2(2+0) | $M(AB)_{2}cd$ | 11(6+5) |
| $Ma_{4}b_{2}$ | 2(2+0) | $M(AB)_{3}$ | 4(2+2) |
| $M(AA)_{2}bc$ | 3(2+1) | $M(AB)_{2}c_{2}$ | 8(5+3) |
...
| 物质 | Cl | H | 正丙基 | 氧化数 | 价电子构型 | 配位数 | 杂化类型 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| A | 1 | 0 | 0 | +1 | $4d^{8}$ | 4 | $dsp^{2}$ |
| B | 1 | 0 | 0 | +1 | $4d^{8}$ | 3 | $sp^{2}$ |
| C | 1 | 2 | 0 | +3 | $4d^{6}$ | 5 | $dsp^{3}$ |
| D | 1 | 2 | 0 | +3 | $4d^{6}$ | 6 | $d^{2}sp^{3}$ |
| E | 1 | 1 | 1 | +3 | $4d^{6}$ | 5 | $dsp^{3}$ |
...
| 物质 | $N_{2}$ | $H_{2}$ | $NH_{3}$ |
| --- | --- | --- | --- |
| $c/(\mathrm{mol}\ L^{-1})$ | 0.11 | 0.14 | 0.75 |

现向平衡体系中注入 $4.0 \, \mathrm{mol} \, N_{2}$ 。试计算反应的平衡常数 $K_{c}$ 和反应商 $Q_{c}$ ，并由此判断反应将向什么方向进行。
...
| 物质 | $\Delta_{\text{f}} H_{\text{m}}^{\ominus} / (\text{kJ mol}^{-1})$ | $S_{\text{m}}^{\ominus} / (\text{J mol}^{-1} \text{ K}^{-1})$ |
| --- | --- | --- |
| $\text{CH}_{4}(\text{g})$ | -74.4 | 186.3 |
| $\text{H}_{2}\text{O}(\text{g})$ | -241.8 | 188.8 |
| $\text{H}_{2}(\text{g})$ | 0 | 130.7 |
| $\text{CO}(\text{g})$ | -110.5 | 197.7 |
| $\text{H}_{2}\text{O}(\text{l})$ | -285.8 | 69.9 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| $3d^{1}4s^{2}$ | $3d^{2}4s^{2}$ | $3d^{3}4s^{2}$ | $3d^{5}4s^{1}$ | $3d^{5}4s^{2}$ | $3d^{6}4s^{2}$ | $3d^{7}4s^{2}$ | $3d^{8}4s^{2}$ |
| Y | Zr | Nb | Mo | Tc* | Ru | Rh | Pd |
| $4d^{1}5s^{2}$ | $4d^{2}5s^{2}$ | $4d^{4}5s^{1}$ | $4d^{5}5s^{1}$ | $4d^{5}5s^{2}$ | $4d^{7}5s^{1}$ | $4d^{8}5s^{1}$ | $4d^{10}$ |
| La | Hf | Ta | W | Re | Os | Ir | Pt |
| $5d^{1}6s^{2}$ | $5d^{2}6s^{2}$ | $5d^{3}6s^{2}$ | $5d^{4}6s^{2}$ | $5d^{5}6s^{2}$ | $5d^{6}6s^{2}$ | $5d^{7}6s^{2}$ | $5d^{9}6s^{1}$ |
...
加入 $4.0 \, \mathrm{mol} \, N_{2}$ 后，体积变化到原体积的 5.0 倍，则
...
$Na_{2}CO_{3}$ ，溶液变为棕色并生成沉淀。用 $0.04826 \, \mathrm{mol} \, L^{-1}$ 的 $Na_{2}S_{2}O_{3}$ 溶液滴定，近终点时加入淀粉指示剂，至终点，消耗 $12.69 \, \mathrm{mL}$ 。
...
7-2 下图示出硼酸-硼酸盐体系在硼的总浓度为 $0.4 \, \mathrm{mol} \, L^{-1}$ 时，其存在形式与 pH 的关系。1、2、3、4 分别为 4 种多硼酸根离子存在的区域。推出 1、2、3、4 分别对应的多硼酸根离子的化学式。
...
配合物带有三个水分子,它们在加热条件下是最容易失去的,同时三个水分子的摩尔质量为 $54.0\ g\ \mathrm{mol}^{-1}$ ,与 $57.4\ g\ \mathrm{mol}^{-1}$ 比较接近,二者间3.4的差别可能是实验误差造成的。因此第一步失重失去的组分为 $3H_{2}O$ 。
...
已知晶体是由草酸和尿素两种分子组成的，可设晶胞中含有 x 个草酸分子，y 个尿素分子，草酸的摩尔质量为 $90.04 \, g \mathrm{mol}^{-1}$ ，尿素的摩尔质量为 $60.06 \, g \mathrm{mol}^{-1}$ ，得到不定方程：
...
1-5 溶出伏安法用于测定金属离子浓度。原理是将待测金属离子还原为单质沉积在电极表面的汞膜内而达到富集的效果。为了减少污染，可以用铋膜代替汞膜。利用电解将铋离子还原为单质铋，沉积在圆柱形电极的底面上均匀成膜。设电极直径为 3 mm，电流强度为 1 mA，电解 2 min，计算电极上沉积的铋膜厚度（已知铋密度为 $9.8 \, g \mathrm{cm}^{-3}$ ）。
...
| T/K | $k_{1}/s^{-1}$ | $k_{2}/s^{-1}$ | $k_{obs}/s^{-1}$ |
| --- | --- | --- | --- |
| 293.2 | $4.869\times 10^{-3}$ | $1.350\times 10^{-4}$ | $5.865\times 10^{-4}$ |
| 301.2 | $7.731\times 10^{-3}$ | $2.398\times 10^{-4}$ | $9.795\times 10^{-4}$ |
...
3-3 已知 $\beta$ -CPBA-FeP 体系反应的表观活化能 $E_{a,obs}=47.07\ \mathrm{kJ}\ \mathrm{mol}^{-1}$ 和机理 2 中 (v) 步的活化能 $E_{a,1}=42.43\ \mathrm{kJ}\ \mathrm{mol}^{-1}$ ，计算 (vii) 步的活化能 $E_{a,2}$ 。
...
| --- | --- | --- | --- | --- |
| $K_i^{\ominus}$ | $2.775\times 10^{5}$ | $1.012\times 10^{5}$ | $4.080\times 10^{4}$ | $1.458\times 10^{4}$ |
...
由于这道题目本身对于有效数字要求不高,因此官方答案中结果写做3位有效数字。笔者认为,结果按照有效数字规则,写为 $1\times10^{-4}\mathrm{cm}$ 更为妥当。对于计算题,建议中间过程不带数据计算,一来减少计算失误,二来减少中间结果由于有效数字修约带来的变化。
...
对于 D, 每个氧分别与一个 Ge 和一个 Bi 相连, 每个 $\mathrm{Ge(IV)}$ 都位于 $\left[GeO_{4}\right]$ 四面体中, 即 $GeO_{4}^{4-}$ 。它与 $Bi^{3+}$ 形成化合物, 可以保证每个氧分别与一个 Ge 和一个 Bi 相连, 因此化学式写为 $\mathrm{Bi}_{4}(\mathrm{GeO}_{4})_{3}$ 或 $Bi_{4}Ge_{3}O_{12}$ 。为了验证这个化学式是否正确，可以将它的摩尔质量代入晶体密度公式，求得 Z。如果 Z 为整数，说明我们推断得到的化学式合理。此化学式的摩尔质量为 $1245.83\ g\ \mathrm{mol}^{-1}$ 。
...
3-1～3-3 A 在题目中给出的性质有: ① 熔点比较高, 摩尔质量 $76.12 \, g \mathrm{mol}^{-1}$ ; ② 可以通过题目中所述的三种途径合成; ③ 在水溶液中能异构化。
...
| --- | --- | --- | --- | --- | --- |
| $H_2O$ | (一) | (一) | (一) | 久置缓慢歧化为紫色溶液+棕色沉淀 | 蓝色溶液 |
| $H_2SO_4$ | (一) | 绿色沉淀 | (一) | 紫色溶液+棕色沉淀 | 蓝色溶液 |
...
$k_{1}=1.0\times10^{7}\ \mathrm{mol}^{-1}\mathrm{dm}^{3}\ s^{-1},\ k_{2}=1.0\times10^{2}\ \mathrm{mol}^{-1}\mathrm{dm}^{3}\ s^{-1},pK_{a}=5.0$ ，反应决速步骤为中间体 $HES^{\neq}$ 和 $ES^{\neq}$ 之生成。
...
有部分同学认为反应后 Na 的存在形式是 $Na_{3}N$ ，而事实是 Na 和 $N_{2}$ 即使在高温下也不能生成 $Na_{3}N$ ——这是因为 Na 和氮气反应是消耗气体的反应，必然熵减， $\Delta G = \Delta H - T\Delta S$ ，所以即使 $\Delta H < 0$ ，也应该在低温下自发反应，而高温下则不反应——这反倒证明了在高温下 $Na_{3}N$ 会自发分解，所以产物写 $Na_{3}N$ 不是一个好的选择。实际上 $Na_{3}N$ 的生成焓为 $62 \sim 66 \mathrm{kJ} \mathrm{mol}^{-1}$ ，所以 Na 和 $N_{2}$ 不论什么时候都不会生成 $Na_{3}N$ 。 $^{[1]}$
...
我们发现,如果要计算稳定常数之比,需要知道 $\left[H_{2}P_{2}O_{7}^{2-}\right]$ ,但相对来讲这是不好求算的。有没有其他的方法?实际上题目给了我们很大的提示,反应(e)的标态没法求。如果直接用题目中反应(e)的条件是否可以呢?在题目所给条件中, $\left[H_{2}P_{2}O_{7}^{2-}\right]=0.40\ \mathrm{mol}\ L^{-1}$ , $\left[Mn(H_{2}P_{2}O_{7})_{3}^{3-}\right]=\left[Mn(H_{2}P_{2}O_{7})_{2}^{2-}\right]$ ,这两个条件正好完全对应上述Nernst方程中的未知项。因此将1.15 V这个电势代入后:
...
环戊二烯钠与氯化亚铁在四氢呋喃中反应，或环戊二烯与氯化亚铁在三乙胺存在下反应，可制得稳定的双环戊二烯基合铁(二茂铁)。163.9 K以上形成的晶体属于单斜晶系，晶胞参数 $a=1044.35\ \mathrm{pm}, b=757.24\ \mathrm{pm}, c=582.44\ \mathrm{pm}, \beta=120.958^\circ$ 。密度 $1.565\ g\ \mathrm{cm}^{-3}$ 。
...
$O_{2}$ 被消耗的唯一途径就是与 $CH_{4}$ 反应。由反应(2)知，用于消耗 $O_{2}$ 的 $CH_{4}$ 共需 2 mol，产物为 $4\ \mathrm{mol}\ H_{2} + 2\ \mathrm{mol}\ CO$ ；而产生的 CO 经反应(3)转化为 $2\ \mathrm{mol}\ CO_{2} + 2\ \mathrm{mol}\ H_{2}$ 。故总的转化过程为： $O_{2}$ 被 $CH_{4}$ 完全消耗，共生成 $6\ \mathrm{mol}\ H_{2}$ 。
...
| 状态 | 基态 | $\Sigma$ 激发态 | $\Delta$ 激发态 |
| --- | --- | --- | --- |
| HOMO轨道电子排布 | $\uparrow \uparrow$ | $\uparrow \downarrow$ | $\uparrow \downarrow$ |
| 光谱学记号 | $^{3}\Sigma_{g}^{-}$ | $^{1}\Sigma_{g}^{-}$ | $^{1}\Delta_{g}$ |
| 相对能量 | 0 eV | 1.61 eV | 0.95 eV |
...
故 $Cu_{2}S$ 含量: $2.15\ \mathrm{mmol} \times 159.2\ g\ \mathrm{mol}^{-1} = 0.342\ g$ ，质量分数: $0.342\ g \div 0.7520\ g = 45.5\%$ .

故 CuS 含量: $2.98 \, \mathrm{mmol} \times 95.61 \, g \mathrm{mol}^{-1} = 0.285 \, g$ ，质量分数: $0.285 \, g \div 0.7520 \, g = 37.9\%$ .
...
1-4 M在一定温度下与氧作用得到固体 G。G 属立方晶系，其正当晶胞如下图所示，密度为 $6.00 \, g \, \mathrm{cm}^{-3}$ 。通过计算给出 M-O 的距离。
...
5-6 已知 B 晶体的理论密度为 $6.49 \, g \, \mathrm{cm}^{-3}$ ，通过计算说明 X 是何种元素。
...
| t/°C | 600 | 795 | 840 | 880 | 896 | 910 |
| --- | --- | --- | --- | --- | --- | --- |
| p/p $^{\ominus}$ | 0.0132 | 0.210 | 0.421 | 0.763 | 1.00 | 1.32 |
...
3-1 若 0.010 mol ZnS 全部溶于 1 L 盐酸中，则 $\left[Zn^{2+}\right]=0.010\ \mathrm{mol}\ L^{-1}$ 。

$$
[ \mathrm{Zn} ^ {2 +} ] [ \mathrm{S} ^ {2 -} ] = K _ {\mathrm{sp}} (\mathrm{ZnS})
$$

得 $\left[S^{2-}\right]=2.5\times10^{-20}\ \mathrm{mol}\ L^{-1}$ ，其数量级很小。此时，可忽略 $S^{2-}$ 和 $HS^{-}$ 的浓度，令 $\left[H_{2}S\right]\approx0.010\ \mathrm{mol}\ L^{-1}$ 。
...
当铜完全析出时， $\left[H^{+}\right]=2\times0.0500\ \mathrm{mol}\ L^{-1}=0.100\ \mathrm{mol}\ L^{-1}$ 。
...
然后利用题目中给出的条件：“金属的比热与其相对原子质量的乘积近似为 $6 \, cal \, g^{-1} \, ^{\circ}C^{-1}$ ”，可得金属 M 的相对原子质量： $\frac{6 \times 4.18 \, J \, g^{-1} \, ^{\circ}C^{-1}}{0.126 \, J \, g^{-1} \, ^{\circ}C^{-1}} = 199$ ，所以 M 的摩尔质量为 $199 \, g \, \mathrm{mol}^{-1}$ 。
