...
$$
\frac {\left(\frac {2 x}{4.10 - 2 x}\right) ^ {2}}{\left(\frac {1.10 - x}{4.10 - 2 x}\right) \left(\frac {3.00 - 3 x}{4.10 - 2 x}\right) ^ {3}} = 8.33,
$$
...
电势作为一个绝对量进行测量也比较困难,因此需要引入参比。使用标准氢电极,即 $1 \, bar \, H_{2}$ 和 $1 \, \mathrm{mol}/\mathrm{L} \, H^{+}$ 作为0电势参考,其余电对的电极电势用电位差参比标准氢电极计算得到。
...
$$
n _ {12} E _ {12} + n _ {23} E _ {23} = n _ {13} E _ {13},
$$
...
硫代硫酸歧化反应的电动势为 0.600-0.400=0.200V，故其 Gibbs 自由能变为 $-2\times0.200\times96485=-38.6\mathrm{kJ}/\mathrm{mol}$ 。
...
$$
\frac {c ^ {2}}{1 - c} = 1.77 \times 10 ^ {- 5} \Rightarrow [ \mathrm{OH} ^ {-} ] = 4.20 \mathrm{mmol/L}, [ \mathrm{NH} _ {3} ] = 0.996 \mathrm{mol/L},
$$

因此氧气的电势降低到

$$
\varphi_ {\mathrm{red}} = 1.23 + \frac {R T}{4 F} \ln \frac {K _ {\mathrm{w}}}{[ \mathrm{OH} ^ {-} ]} = 0.542 \mathrm{V},
$$

Co(Ⅲ)的电势则降低到

$$
\varphi_ {\mathrm{oxd}} = 1.84 + \frac {R T}{F} \ln \frac {\beta_ {6} \left(\mathrm{Co} \left(\mathrm{NH} _ {3}\right) _ {6} ^ {2 +}\right)}{\beta_ {6} \left(\mathrm{Co} \left(\mathrm{NH} _ {3}\right) _ {6} ^ {3 +}\right)} = 0.054 \mathrm{V} <   0.542 \mathrm{V},
$$
...
$$
K = \frac {k _ {1}}{k _ {- 1}} \Rightarrow k _ {1} = 120 \mathrm{s} ^ {- 1} \cdot \mathrm{kPa} ^ {- 1} 。
$$

注意到正向反应速率远高于逆向反应速率,可认为只有正向反应,所以时间为

$$
t \approx \frac {\ln 2}{120 \times 20} = 2.9 \times 10 ^ {- 4} \mathrm{s。}
$$
...
$$
{ } ^ { 147 } \mathrm{Sm} \longrightarrow { } ^ { 143 } \mathrm{Nd} + { } ^ { 4 } \mathrm{He} , k = 6.54 \times 10 ^ { - 12 } \text {年} ^ { - 1 }
$$
...
$$
\left\{ \begin{array}{l} \frac {0.111 a}{0.111 a + x} = \frac {0.280 b}{0.280 b + y} = a, \\ \frac {0.510 a - x}{a} = \frac {0.515 b - y}{b}. \end{array} \right.
$$
...
$$
\begin{array}{l} \mathrm{V} _ {2} \mathrm{O} _ {5} (\mathrm{s}) + \mathrm{SO} _ {2} (\mathrm{g}) \rightleftharpoons 2 \mathrm{VO} _ {2} (\mathrm{s}) + \mathrm{SO} _ {3} (\mathrm{g}) K _ {1} = 0.01186, \\ 2 \mathrm{VOSO} _ {4} (\mathrm{s}) \rightleftharpoons \mathrm{V} _ {2} \mathrm{O} _ {5} (\mathrm{s}) + \mathrm{SO} _ {2} (\mathrm{g}) + \mathrm{SO} _ {3} (\mathrm{g}) K _ {2} = 9.174 \times 10 ^ {- 5}, \\ 2 \mathrm{SO} _ {3} (\mathrm{g}) \rightleftharpoons 2 \mathrm{SO} _ {2} (\mathrm{g}) + \mathrm{O} _ {2} (\mathrm{g}) K _ {3} = 3.621 \times 10 ^ {- 4} \\ \end{array}
$$
...
【习题 8.33】计算化学研究表明,反应 $H_{2}S + SO_{2}$ 的焓变 $\Delta H = 16\mathrm{kJ}/\mathrm{mol}$ 。反应过程中,先产生中间体 $H_{2}S_{2}O_{2}$ 。其中一种中间体转化为产物的焓变为 -14kJ/mol,由原料转化为中间体的活化能为 136kJ/mol。画出反应过程中的势能图,并计算由中间体 $H_{2}S_{2}O_{2}$ 转化为原料的反应的活化能与焓变。
...
<table><tr><td>物质</td><td>Ag(s)</td><td> $H_2S(g)$ </td><td> $Ag_2S(s)$ </td><td> $H_2(g)$ </td></tr><tr><td> $\Delta_fH_m/(\mathrm{kJ}\cdot\mathrm{mol}^{-1})$ </td><td>-20.6</td><td>-32.6</td><td></td><td></td></tr><tr><td> $S_m/[J·(\mathrm{mol}\cdot\mathrm{K})^{-1}]$ </td><td>42.6</td><td>205.8</td><td>144</td><td>130.7</td></tr></table>
...
<table><tr><td>热力学数据</td><td> $C_3H_8(l)$ </td><td> $C_3H_8(g)$ </td><td> $O_2(g)$ </td><td> $CO_2(g)$ </td><td> $H_2O(l)$ </td><td> $H_2O(g)$ </td></tr><tr><td> $\Delta_fH_m/(\mathrm{kJ}\cdot\mathrm{mol}^{-1})$ </td><td>-120.9</td><td>-103.9</td><td></td><td>-393.5</td><td>-285.8</td><td>-241.8</td></tr><tr><td> $S_m/[J·(\mathrm{mol}\cdot\mathrm{K})^{-1}]$ </td><td>195.2</td><td>269.9</td><td>205.0</td><td>213.7</td><td>70.0</td><td>188.7</td></tr></table>
...
<table><tr><td> $c/(\mathrm{mol} \cdot \mathrm{L}^{-1})$ </td><td>0.00100</td><td>0.00200</td><td>0.00300</td><td>0.00500</td></tr><tr><td>A</td><td>0.56</td><td>1.08</td><td>1.60</td><td>2.64</td></tr></table>
...
为测定上述反应的平衡常数 $K_{d}$ ，在氮气保护下，将 0.3586g DBAA 溶于 26.9234g 苯中，测得溶液的凝固点下降 $\Delta T_{f}=0.176K$ 。已知苯的凝固点下降常数 $K_{f}=5.12\mathrm{K}\cdot \mathrm{kg}\cdot \mathrm{mol}^{-1}$ ，该溶液的体积为 V=31.7mL，请据此计算该解离反应的平衡常数 $K_{d}$ 和三聚体的解离度 $\alpha$ 。

解 由凝固点下降公式, DBAA 及其多聚体的平均摩尔质量为

$$
M = m _ {\mathrm{B}} K _ {\mathrm{f}} / m _ {\mathrm{A}} \Delta T _ {\mathrm{f}} = 0.3586 \times 5.12 / (26.9234 \times 0.176) = 387.5 \mathrm{g} / \mathrm{mol} 。
$$
...
故平衡常数为： $K_{d}=\frac{(3\alpha c)^{3}}{(1-\alpha)c}=1.32\times10^{-4}L^{2}/\mathrm{mol}^{2}$
...
$$
\mathrm{pH} = \frac {1}{2} \left(\mathrm{pK} _ {\mathrm{a} _ {1}} + \mathrm{pK} _ {\mathrm{a} _ {2}}\right) = 0.5 \times (6.38 + 10.33) = 8.36 。
$$
...
$$
\left\{ \begin{array}{l} {\left[ \mathrm{Cl} ^ {-} \right] = 0.01378 \mathrm{mol/L},} \\ {\left[ \mathrm{TI} ^ {-} \right] = 6.455 \times 10 ^ {- 6} \mathrm{mol/L},} \\ {\left[ \mathrm{TI} ^ {+} \right] = 0.01379 \mathrm{mol/L},} \\ {\left[ \mathrm{Ag} ^ {+} \right] = 1.317 \times 10 ^ {- 11} \mathrm{mol/L}.} \end{array} \right.
$$
...
$$
\varphi_ {(\mathrm{Ag} ^ {+} / \mathrm{Ag})} = 0.622 - \frac {R T}{F} \ln 0.00100 \Rightarrow \varphi_ {(\mathrm{Ag} ^ {+} / \mathrm{Ag})} = 0.799 \mathrm{V。}
$$
...
所以在 pH=8 的情况下， $\left[NH_{3}\right]=5.319\times10^{-3}\mathrm{mol}/\mathrm{L},\left[Ag^{+}\right]=2.044\times10^{-6}\mathrm{mol}/\mathrm{L}$ ；在 pH>12 的情况下， $\left[NH_{3}\right]=0.1000\mathrm{mol}/\mathrm{L},\left[Ag^{+}\right]=5.931\times10^{-9}\mathrm{mol}/\mathrm{L}$ 。
...
算得 $s=1.0\times10^{-15}\mathrm{mol}/\mathrm{L}$ 。
...
$$
K = \frac {K _ {\mathrm{sp}} K _ {\mathrm{w}}}{K _ {\mathrm{a2}}} = 1.824 \times 10 ^ {- 15} \Rightarrow \mathrm{s} = K ^ {\frac {1}{3}} = 1.2 \times 10 ^ {- 5} \mathrm{mol/L。}
$$
...
【例题 9.13】将 $3.0 \, mg \, PbSO_{4}$ 放在 $50.0 \, \mathrm{mL}$ 水中，以摩尔比 1:9 加入醋酸/醋酸钠。设溶液体积保持不变。数据： $HOAc, pK_{a} = 4.75; Pb^{2+}-OAc^{-}, lg\beta_{2} = 3.3, lg\beta_{3} = 3.45; PbSO_{4}, K_{sp} = 1.6 \times 10^{-8}$
...
$$
c _ {0} = \left[ \mathrm{SO} _ {4} ^ {2 -} \right] = 1.978 \times 10 ^ {- 4} \mathrm{mol} / \mathrm{L} \Rightarrow \left[ \mathrm{Pb} ^ {2 +} \right] = 8.087 \times 10 ^ {- 5} \mathrm{mol} / \mathrm{L}
$$
...
由上式解得 $x=0.02457\,\mathrm{mol}/\mathrm{L}$ 。然后列出其他守恒式，略掉氢氧根得

$$
\left\{ \begin{array}{l} \left[ \mathrm{Na} ^ {+} \right] + 2 \left[ \mathrm{Pb} ^ {2 +} \right] + y = x + \left[ \mathrm{Pb} ^ {2 +} \right] \beta_ {3} x ^ {3} + 2 \left[ \mathrm{SO} _ {4} ^ {2 -} \right], \\ \frac {10}{9} \left[ \mathrm{Na} ^ {+} \right] = x + \left[ \mathrm{Pb} ^ {2 +} \right] \left(3 \beta_ {3} x ^ {3} + 2 \beta_ {2} x ^ {2}\right) + \frac {x y}{K _ {\mathrm{a}}} 。 \end{array} \right.
$$

现在上面的方程属二元一次方程,解得

$$
y = 1.989 \times 10 ^ {- 6}, [ \mathrm{Na} ^ {+} ] = 0.02479 \mathrm{mol} / \mathrm{L}, \mathrm{pH} = 5.70 \Rightarrow m = 0.102 \mathrm{g。}
$$
...
$$
[ \mathrm{I} _ {2} ] _ {\mathrm{w}} = \frac {[ \mathrm{I} _ {2} ] _ {\mathrm{o}}}{K _ {\mathrm{D}}} = 0.0306 \mathrm{mol/L。}
$$

利用物料守恒

$$
\left\{ \begin{array}{l} {\left[ \mathrm{I} ^ {-} \right] + 2 \left[ \mathrm{I} _ {2} \right] _ {\mathrm{o}} + 2 \left[ \mathrm{I} _ {2} \right] _ {\mathrm{w}} + \left[ \mathrm{I} _ {3} ^ {-} \right] = 0.00485 \times 2 + 0.100,} \\ {\left[ \mathrm{I} _ {3} ^ {-} \right] + \left[ \mathrm{I} _ {2} \right] _ {\mathrm{w}} + \left[ \mathrm{I} _ {2} \right] _ {\mathrm{o}} = 0.00485,} \end{array} \right.
$$
...
滴定到酚酞恰好褪色时,体系是乙酸钠和乙酰水杨酸一钠盐的混合物,根据始终态钠离子守恒 $C_{10}H_{10}O_{4}\sim2NaOH$ ,消耗NaOH $0.05000\times0.1020-0.05050\times2\times0.02500=0.002575\mathrm{mol}$ ,故乙酰水杨酸的质量分数为 $0.002575/2\times180.16/0.250=92.8\%$ .
...
$$
\left\{ \begin{array}{l} 2 (x + y) = \frac {0.2513}{143.35}, \\ 2 (74.55 x + 58.44 y) = 0.1208, \end{array} \right.
$$
...
$$
\omega = \frac {0.002509 \times 1.5 \times 12.01}{0.90 \times 0.58 \times 0.1500} = 5.8 \%.
$$
...
$$
\mathrm{Be} \sim 2 \mathrm{OH} ^ {-} \sim 2 \mathrm{H} ^ {+}, \omega (\mathrm{Be}) = 0.02987 \times 0.09879 / 2 \times 9.014 \times 8 / 3
$$
...
$$
\begin{array}{l} [ \mathrm{H} ^ {+} ] = 10 ^ {- 6.80}, [ \mathrm{OH} ^ {-} ] = 10 ^ {- 7.20}, \\ [ \mathrm{CH} _ {3} \mathrm{NH} _ {3} ^ {+} ] = 0.9998 \times 2 c, [ \mathrm{pyH} ^ {+} ] = 0.0262 c. \\ \end{array}
$$
...
注:单位均为 $\mathrm{cm}^{-1} \cdot \mathrm{L} \cdot \mathrm{mol}^{-1}$ 。
...
解 1L 溶液中银离子的浓度为 $4 \times 0.225 / (1.8 \times 10^{4} \times 1) = 5.0 \times 10^{-5} \, \text{mol/L}$ ，故原溶液中有银离子 $5.0 \times 10^{-2} \, \mathrm{mol}/\mathrm{L}$ ，溶度积为 0.0025。
...
【习题 9.43】某人配制并标定了 $0.1024 \, \mathrm{mol}/\mathrm{L} \, CuCl_{2}$ 和 $0.1192 \, \mathrm{mol}/\mathrm{L} \, NiCl_{2}$ 溶液。首先他确定了两种溶液在 $1.000 \, \mathrm{cm}$ 石英比色皿中在不同波长下的吸光度：
