...
3. 周报写入 `09-审计报告/周巡检-YYYY-MM-DD.md`，**仅报异常增量**；三件套对比基线：
   - diag_remaining：`no_answer` 必须保持 0；**题目计数（2026-09-21 更新）＝ 组卷池 6,153**（`type: 题目` 非 deprecated @04-题库 **6,112** ＋ `05-真题库` `type: 真题` **41**）。旧基线 4,182（diag 口径 4,119＋63）／grep 4,186 **均已作废**——⚠️ **diag 工具若仍报 4,xxx，是先期口径未同步，须按 frontmatter 实重复核后再判漂移**；权威口径 [[04-题库/题库按用途分类说明]] v1.2.3 §一。
...
| 题目计数 | **6,153**（2026-09-21 frontmatter 实测：`type: 题目` 非 deprecated @04-题库 **6,112**（6,166 − dep 54）＋ `type: 真题` **41** @05-真题库。**旧基线 4,186（grep）／4,182（diag）均已作废**，见上方注） |
