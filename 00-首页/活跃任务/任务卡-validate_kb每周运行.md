...
evidence:
  ...
  - "2026-08-10: 08-10 精选重组归档移动后合规补跑（--full 6237 文件）——0 error / 1926 warning / 7406 info / 断链 1885 / 图片缺失 3。精选重组断链旧→新路径平移数量不变，未引入体系性断链；新增 ~68 断链为 08-09 晚间真题三方审计 knowledge_points 补全产物（目标 KP 缺失，已入断链内容建设backlog）；图片缺失 +1 为 04-课件/习题集/一·随堂层（载体I·配套讲义章末练习）/第一轮结构化学-晶体结构练习题组 mineru 图引用（08-10 07:59 修改）。"
...
2. 对比基线（0 error / 1,641 warning，2026-09-01 深夜终检 full 实测；枚举 0，图片缺失 0，正文断链 45 + frontmatter 1,590 存量按指示暂不处理）
   > **2026-09-21 注**：最近一次全量终检为 **09-17（受检 7,874 文件 / 0 error / 202 warning）**，周检应以该次为基线；本节 09-01 的 6,410/1,641 已过期，仅留作历史。
3. 周报写入 `09-审计报告/周巡检-YYYY-MM-DD.md`，**仅报异常增量**；三件套对比基线：
   - diag_remaining：`no_answer` 必须保持 0；**题目计数（2026-09-22 更新）＝ 组卷池 6,234**（`type: 题目` 非 deprecated @04-题库 **6,193** ＋ `05-真题库` `type: 真题` **41**）。旧基线 4,182（diag 口径 4,119＋63）／grep 4,186 **均已作废**——⚠️ **diag 工具若仍报 4,xxx，是先期口径未同步，须按 frontmatter 实重复核后再判漂移**；权威口径 [[04-题库/题库按用途分类说明]] v1.2.4 §一。
...
| 题目计数 | **6,234**（2026-09-22 frontmatter 实测：`type: 题目` 非 deprecated @04-题库 **6,193**（6,247 − dep 54）＋ `type: 真题` **41** @05-真题库。**旧基线 4,186（grep）／4,182（diag）均已作废**，见上方注） |
