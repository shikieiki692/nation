---
title: "习题22.24-2224222522262m"
type: 题目
fidelity: 原书逐字
submodule: Ch22-铁系元素和铂系元素
exam_stage: 初赛
subject: 元素化学
difficulty: 3
teaching_level: 巩固
tags: [化竞, 无机化学例题与习题, 铁系元素和铂系元素]
created: 2026-08-25
updated: 2026-08-25
source: 无机化学例题与习题（徐佳宁等）第22章
source_file: "[[无机化学 习题集]]"
status: 已补全答案
pack: 章节练习
subject_module: 元素与分析
knowledge_points: ["[[铁系元素]]", "[[铂系元素]]"]
---
# 习题22.24 2224222522262m

22.24 $\mathrm{Fe_3O_4 + 8HI = 3FeI_2 + I_2 + 4H_2O}$ 22.25 $2\mathrm{FeSO}_4\xrightarrow{\triangle}\mathrm{Fe}_2\mathrm{O}_3 + \mathrm{SO}_2\uparrow +\mathrm{SO}_3\uparrow$ 22.26 $4\mathrm{Fe(OH)}_2 + \mathrm{O}_2 + 2\mathrm{H}_2\mathrm{O} = 4\mathrm{Fe(OH)}_3$

$$
2 \mathrm{Fe} + 3 \mathrm{H} _ {2} \mathrm{O} \xlongequal {\text {赤热}} \mathrm{Fe} _ {2} \mathrm{O} _ {3} + 3 \mathrm{H} _ {2} \uparrow
$$

$$
\mathrm{FeCl} _ {3} + 3 \mathrm{Na} _ {2} \mathrm{CO} _ {3} + 3 \mathrm{H} _ {2} \mathrm{O} = \mathrm{Fe(OH)} _ {3} \downarrow + 3 \mathrm{NaHCO} _ {3} + 3 \mathrm{NaCl}
$$

> **答案**：$\mathrm{Fe_3O_4 + 8HI = 3FeI_2 + I_2 + 4H_2O}$ 22.25 $2\mathrm{FeSO}_4\xrightarrow{\triangle}\mathrm{Fe}_2\mathrm{O}_3 + \mathrm{SO}_2\uparrow +\mathrm{SO}_3\uparrow$ 22.26 $4\mathrm{Fe(OH)}_2 + \mathrm{O}_2 + 2\mathrm{H}_2\mathrm{O} = 4\mathrm{Fe(OH)}_3$

$$
2 \mathrm{Fe} + 3 \mathrm{H} _ {2} \mathrm{O} \xlongequal {\text {赤热}} \mathrm{Fe} _ {2} \mathrm{O} _ {3} + 3 \mathrm{H} _ {2} \uparrow
$$

$$
\mathrm{FeCl} _ {3} + 3 \mathrm{Na} _ {2} \mathrm{CO} _ {3} + 3 \mathrm{H} _ {2} \mathrm{O} = \mathrm{Fe(OH)} _ {3} \downarrow + 3 \mathrm{NaHCO} _ {3} + 3 \mathrm{NaCl}
$$
