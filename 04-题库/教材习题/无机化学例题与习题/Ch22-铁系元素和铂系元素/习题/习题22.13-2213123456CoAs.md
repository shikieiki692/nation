---
title: "习题22.13-2213123456CoAs"
type: 题目
fidelity: 原书逐字
submodule: Ch22-铁系元素和铂系元素
exam_stage: 初赛
subject: 元素化学
difficulty: 3
teaching_level: 巩固
tags: [化竞, 无机化学例题与习题, 铁系元素和铂系元素]
created: 2026-08-25
updated: 2026-08-25
source: 无机化学例题与习题（徐佳宁等）第22章
source_file: "[[无机化学 习题集]]"
status: 已补全答案
pack: 章节练习
subject_module: 元素与分析
knowledge_points: ["[[铁系元素]]", "[[铂系元素]]"]
---
# 习题22.13 2213123456CoAs

22.13 (1) $\mathrm{Fe_2O_3}$ ; (2) $\mathrm{Fe_3O_4}$ ; (3) $2\mathrm{Fe}_2\mathrm{O}_3\cdot 3\mathrm{H}_2\mathrm{O}$ ; (4) $\mathrm{FeCO_3}$ ; (5) $\mathrm{CoAs_2}$ (6) CoAsS (7) $\mathrm{Co_2S_4}$ ; (8) $\mathrm{K_3[Fe(CN)_6]}$ ; (9) $\mathrm{K_4[Fe(CN)_6]\cdot 3H_2O}$ ;

(10) $\mathrm{FeSO_4\cdot 7H_2O}$ ; (11) $(\mathrm{NH}_4)_2\mathrm{SO}_4\cdot \mathrm{FeSO}_4\cdot 6\mathrm{H}_2\mathrm{O}$ ;

$$
\mathrm{NH} _ {4} \mathrm{Fe} (\mathrm{SO} _ {4}) _ {2} \cdot 1 2 \mathrm{H} _ {2} \mathrm{O}
$$

(13) $\mathrm{Fe(C_5H_5)_2}$ ; (14) $\mathrm{K[Pt(C_2H_4)Cl_3]}$ ;

(15) $cis - \text{Pt(NH}_3\text{)}_2\text{Cl}_2$ .

> **答案**：(1) $\mathrm{Fe_2O_3}$ ; (2) $\mathrm{Fe_3O_4}$ ; (3) $2\mathrm{Fe}_2\mathrm{O}_3\cdot 3\mathrm{H}_2\mathrm{O}$ ; (4) $\mathrm{FeCO_3}$ ; (5) $\mathrm{CoAs_2}$ (6) CoAsS (7) $\mathrm{Co_2S_4}$ ; (8) $\mathrm{K_3[Fe(CN)_6]}$ ; (9) $\mathrm{K_4[Fe(CN)_6]\cdot 3H_2O}$ ;

(10) $\mathrm{FeSO_4\cdot 7H_2O}$ ; (11) $(\mathrm{NH}_4)_2\mathrm{SO}_4\cdot \mathrm{FeSO}_4\cdot 6\mathrm{H}_2\mathrm{O}$ ;

$$
\mathrm{NH} _ {4} \mathrm{Fe} (\mathrm{SO} _ {4}) _ {2} \cdot 1 2 \mathrm{H} _ {2} \mathrm{O}
$$

(13) $\mathrm{Fe(C_5H_5)_2}$ ; (14) $\mathrm{K[Pt(C_2H_4)Cl_3]}$ ;

(15) $cis - \text{Pt(NH}_3\text{)}_2\text{Cl}_2$ .
