---
title: 题-262-Clayden-Ch6-P1-硼氢化钠还原醛酮机理
type: 题目
submodule: 羰基亲核加成
exam_stage: 初赛
subject: 有机化学
difficulty: 2
teaching_level: 巩固
syllabus_codes: ["21"]
knowledge_points: ["[[羰基亲核加成]]", "[[硼氢化钠还原]]"]
tags: [化竞, Clayden, 有机化学, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch6-P1]
source: Clayden Organic Chemistry 2nd Ed. Chapter 6 Problem 1
cross_references: ["[[题-265-Clayden-Ch6-P4-NaBH4还原二羰基选择性]]", "[[题-268-Clayden-Ch6-P8-NaBH4还原氯醛水合物机理]]", "[[题-269-Clayden-Ch6-P10-Grignard加成+NaBH4选择性还原]]"]
module: 有机化学
status: 已填充
---
# 题-262: NaBH₄/LiAlH₄还原醛酮机理

## 题目

Draw mechanisms for these reactions:

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/90028763c8698bcdf7acc75303dce16a17068b9487ca3eae39cc41db9d4c0e9a.jpg]]

**原文题目**：Draw mechanisms for the NaBH₄ reduction and the LiAlH₄ reduction of the given aldehyde/ketone substrates.

## 参考答案

**Answer (English)**: Draw out the BH₄⁻ and AlH₄⁻ anions, with the carbonyl compound positioned so that one of the hydrogens can be transferred to the carbonyl group, and then transfer the hydrogen from B or Al to C. A proton transfer is needed to make the alcohol: from the solvent in the first case and during the work-up with water in the second.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/76e73a40a537cb05ee91f50219b65b15a0ed683d3c6e2d09da43b5a461e3a1d0.jpg]]

**中文解析**：

关键步骤：
1. **H⁻转移**：BH₄⁻或AlH₄⁻中的H⁻作为亲核试剂，进攻羰基碳（Bürgi-Dunitz角约107°），形成C-H键，同时C=O双键变为C-O单键
2. **质子化**：生成的烷氧基负离子需要质子化才能得到醇。NaBH₄还原时从溶剂（MeOH/EtOH）获取质子；LiAlH₄还原时需要水处理（work-up）后才能质子化
3. **区别**：NaBH₄较温和，可在醇溶剂中使用；LiAlH₄更活泼，必须在无水条件下操作

> **注意**：LiAlH₄虽然更常用，但NaBH₄更实用——本反应说明LiAlH₄也能还原醛，只是通常没必要用它。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[羰基亲核加成]] | H⁻作为亲核试剂进攻C=O的核心机理 | 直接 |
| [[硼氢化钠还原]] | NaBH₄和LiAlH₄作为还原剂的区别和使用条件 | 直接 |
| [[醛酮]] | 醛和酮作为亲电底物被还原 | 间接 |
| [[Bürgi-Dunitz角]] | 亲核进攻羰基碳的最优角度 | 间接 |

## 解题思路

1. **读题定位**：题目要求画机理——识别两个反应都是羰基的亲核加成（H⁻进攻C=O），底物分别是醛和酮
2. **🔑 关键转换**：将BH₄⁻/AlH₄⁻视为H⁻供体，H⁻以Bürgi-Dunitz角进攻羰基碳→形成C-H键+氧负离子→质子化得醇
3. **验证**：检查产物是否为伯醇（醛→伯醇）或仲醇（酮→仲醇），H的数量是否正确

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 忘记画质子化步骤 | 只画了H⁻转移就停了 | H⁻转移后必须有质子化才能得到中性醇产物 | 还原产物为什么不是醇盐？ |
| 将BH₄⁻写成BH₃ | 混淆硼烷和硼氢化物 | BH₄⁻是四面体阴离子（4个H），BH₃是Lewis酸 | NaBH₄中B的氧化态是多少？ |
| 画H⁻从AlH₄⁻直接攻击 | 没有考虑空间排列 | 应画出Al-H键与C=O的空间关系，H从H-Al-C方向转移 | LiAlH₄为什么比NaBH₄活泼？ |