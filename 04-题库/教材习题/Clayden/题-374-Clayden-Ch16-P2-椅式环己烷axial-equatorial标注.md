---
title: 题-374-Clayden-Ch16-P2-椅式环己烷axial-equatorial标注
type: 题目
fidelity: 原书逐字
submodule: 构象分析
exam_stage: 初赛
...
updated: 2026-07-25
aliases: [Clayden-Ch16-P2]
source: Clayden Organic Chemistry 2nd Ed. Chapter 16 Problem 2
cross_references: ["[[题-456-Clayden-Ch28-P2-单基团C-C切断练习]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]", "[[题-455-Clayden-Ch28-P1-单基团C-X切断练习]]", "[[题-291-Clayden-Ch14-P1-五个分子手性判断]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
---
# 题-374: 椅式环己烷axial/equatorial标注
...
本题考察椅式环己烷构象图的绘制能力以及取代基轴向/赤道位的判断。

关键要点：
1. **绘制规范**：椅式构象要求环的对边平行，轴向键竖直向上或向下，平伏键与相邻环键之一平行
2. **大基团优先占赤道位**：t-Bu基团的A值极大（>5 kcal/mol），几乎完全占据赤道位，成为锁定构象的"锚定基团"
3. **立体化学关系**：1,2-反式取代基可以都处于赤道位；1,3-顺式取代基也可以都处于赤道位
4. **最后一个分子**：t-Bu锁定在赤道位后，Me和OH的位置被强制为轴向，这是因为它们与t-Bu的相对构型决定的

> **注意**：画构象图时最常见错误是环的"爬楼梯"（各边不平行），应先准确画环再加取代基。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[构象分析]] | 椅式构象中取代基位置的判断与绘制 | 直接 |
| [[直立键与平伏键]] | axial/equatorial键的几何定义与稳定性差异 | 直接 |
| [[A值]] | t-Bu等大基团对构象平衡的锁定效应 | 间接 |
| [[Newman投影式]] | 沿C-C键观察的构象分析方法 | 间接 |

## 解题思路

1. **读题定位**：题目要求画构象图并标注每个取代基为axial或equatorial
2. **🔑 关键转换**：先确定大基团（t-Bu）的位置——锁定赤道位→由此推导其余取代基的相对构型→确定各取代基为axial还是equatorial
3. **验证**：检查轴向键是否竖直、平伏键是否平行于相邻环键；检查取代基间的顺反关系是否正确

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 环画成"爬楼梯"形状 | 没有确保对边平行 | 椅式环的三组对边必须严格平行 | 椅式环己烷有几个对称面？ |
| 将t-Bu画在轴向 | 未考虑A值对构象平衡的影响 | t-Bu的A值极大（>5 kcal/mol），几乎100%处于赤道位 | 什么基团能强迫t-Bu处于轴向？ |
| 轴向/平伏键方向画反 | 对椅式环中各碳上键的方向不熟悉 | 轴向键在C1/C3/C5向上，C2/C4/C6向下（或反之），交替排列 | 同一个碳上的轴向和平伏键之间的夹角是多少？ |