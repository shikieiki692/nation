---
title: 题-427-Clayden-Ch23-P4-多步反应中的保护策略
type: 题目
fidelity: 原书逐字
submodule: 化学选择性与保护基
exam_stage: 初赛
source_subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[保护基]]"]
tags: [化竞, Clayden, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch23-P4]
source: Clayden Organic Chemistry 2nd Ed. Chapter 23 Problem 4
cross_references: ["[[题-477-Clayden-Ch27-P1-分子内硫叶立德共轭加成环丙烷化]]", "[[题-478-Clayden-Ch27-P2-硫叶立德化学区域和立体化学]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
source_category: 教材课后习题
---
# 题-427: 多步反应中的保护策略

## 题目

How would you carry out these reactions? In some cases more than one step may be required.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/761f29022a584420b3d84783191c29b0e2672ce65c382d0389ec3a779b6bd111.jpg]]

## 参考答案

**Answer (English)**: Every step is straightforward except the final reduction where a less reactive ester must be reduced in the presence of a more reactive ketone. Protection is the answer and an acetal is suitable:

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/12c0d9c222532523654023f4d393073bfa5db53b93490053270953fdf8f2ec2b.jpg]]

The final product was used to make an analogue of thromboxane, a human blood clotting agent, by M. Hayashi and group, Tetrahedron Lett., 1979, 3661.

**中文解析**：

关键要点：
1. **核心难题**：需要还原酯基而保留酮基——但酮比酯更活泼，直接还原会先还原酮
2. **保护策略**：将酮保护为缩醛（对还原条件稳定），然后选择性还原酯，最后脱保护恢复酮
3. **缩醛作为保护基**：缩醛在碱性和还原条件下稳定，只在酸性条件下水解——完美匹配本题需求

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[保护基]] | 缩醛保护酮基防止被还原 | 直接 |
| [[化学选择性]] | 酯vs酮的选择性还原 | 直接 |
| [[逆合成分析]] | 从目标产物逆推保护策略 | 间接 |
| [[还原反应]] | LiAlH₄还原酯的条件和选择性 | 间接 |

## 解题思路

1. **读题定位**：题目要求设计多步合成路线——核心是识别选择性问题并设计保护策略
2. **🔑 关键转换**：识别目标：还原酯保留酮→酮更活泼需保护→缩醛保护→还原酯→酸性脱保护
3. **验证**：检查保护/脱保护条件是否与还原条件兼容，产物结构是否正确

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 直接还原不保护 | 没有识别选择性问题 | 酮比酯更活泼，直接还原会先还原酮 | LiAlH₄还原酮和酯的速率哪个更快？ |
| 保护基选择不当 | 不了解不同保护基的兼容性 | 缩醛在碱性和还原条件下稳定，酸性脱保护 | 为什么缩醛能耐受LiAlH₄还原条件？ |
| 脱保护条件不兼容 | 没有考虑脱保护对产物的影响 | 酸性水解脱保护，需确认不影响其他官能团 | 缩醛在什么条件下会水解？ |