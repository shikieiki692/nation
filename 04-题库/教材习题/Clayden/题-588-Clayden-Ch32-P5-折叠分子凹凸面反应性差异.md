---
title: 题-588-Clayden-Ch32-P5-折叠分子凹凸面反应性差异
type: 题目
fidelity: 原书逐字
submodule: 立体选择性
exam_stage: 初赛
source_subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[立体化学]]"]
tags: [化竞, Clayden, 有机化学, 立体选择性]
updated: 2026-07-25
aliases: [Clayden-Ch32-P5]
source: Clayden Organic Chemistry 2nd Ed. Chapter 32 Problem 5
cross_references: ["[[题-432-Clayden-Ch24-P1-氨基醇制备中区域选择性试剂选择]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]", "[[题-291-Clayden-Ch14-P1-五个分子手性判断]]", "[[题-433-Clayden-Ch24-P2-不饱和羰基直接共轭加成区域选择性]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
---
# 题-588: 折叠分子凹凸面反应性差异

## 题目

Why is one of these esters more reactive than the other?

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/64ee3ae5aa763625d9c9bf613284ef95b88b3720f55ad5225c00b1b4bbb1c3c3.jpg]]

## 参考答案

**Answer (English)**: The molecule is folded along the ring junction with one of the esters inside the fold (on the concave side) and one out in the open (on the convex side). In the rate-determining step of ester hydrolysis, the attack of the hydroxide ion on the carbonyl group, the forming tetrahedral intermediate is larger than the starting ester. This means that the ester on the outside, which has more room to 'expand', reacts faster than the ester on the inside.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/d375ea69ece70c1a2bccc17225ed9a109ea3f513b49d6a9e25ee3c22989ec748.jpg]]

**中文解析**：

关键步骤：
1. **分子折叠**：该分子沿环连接处折叠，形成一个酯基在折叠内部（凹面，concave side），另一个在外部开放处（凸面，convex side）的空间结构
2. **酯水解决速步**：在酯水解的速率决定步骤中，OH⁻进攻羰基碳形成四面体中间体。四面体中间体比原始酯基更大
3. **空间效应**：凸面的酯基有更多空间来"膨胀"形成四面体中间体，因此反应更快；凹面的酯基因空间受限，形成四面体中间体困难，反应更慢

> **核心概念**：折叠分子的凹面和凸面具有截然不同的反应性。四面体中间体比起始原料更大的反应（如酯水解）会优先发生在凸面一侧，因为那里有更多的空间容纳膨胀的中间体。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[立体化学]] | 折叠分子的凹/凸面反应性差异 | 直接 |
| [[构象分析]] | 分子折叠对空间可及性的影响 | 直接 |
| [[空间位阻]] | 四面体中间体膨胀导致的空间需求 | 直接 |

## 解题思路

1. **读题定位**：比较两个酯基的反应活性——识别分子为折叠结构，两个酯基在不同面上
2. **关键转换**：分子折叠→一个酯在凹面，一个在凸面→酯水解形成四面体中间体（更大）→凸面酯有更多空间→凸面酯反应更快
3. **验证**：检查水解产物，凸面酯水解更快而凹面酯几乎不反应

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 认为两个酯活性相同 | 没有考虑分子折叠 | 分子折叠使两个酯在空间上处于完全不同的环境 | 什么类型的反应会优先在凹面发生？ |
| 只考虑电子效应 | 忽略了空间因素 | 这里主要是空间位阻效应，不是电子效应 | 四面体中间体比原料大多少？ |
| 认为凹面酯反应更快 | 混淆了概念 | 凹面空间受限，四面体中间体难以形成，反应更慢 | 如果反应是SN2，哪个面更有利？ |