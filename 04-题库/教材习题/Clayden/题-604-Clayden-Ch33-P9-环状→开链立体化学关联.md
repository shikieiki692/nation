---
title: 题-604-Clayden-Ch33-P9-环状→开链立体化学关联
type: 题目
fidelity: 原书逐字
submodule: 非对映选择性
exam_stage: 初赛
source_subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[立体化学]]"]
tags: [化竞, Clayden, 有机化学, 非对映选择性]
updated: 2026-07-25
aliases: [Clayden-Ch33-P9]
source: Clayden Organic Chemistry 2nd Ed. Chapter 33 Problem 9
cross_references: ["[[题-433-Clayden-Ch24-P2-不饱和羰基直接共轭加成区域选择性]]", "[[题-291-Clayden-Ch14-P1-五个分子手性判断]]", "[[题-432-Clayden-Ch24-P1-氨基醇制备中区域选择性试剂选择]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
source_category: 教材课后习题
---
# 题-604: 环状→开链立体化学关联

## 题目

How could this cyclic compound be used to produce the open-chain compound with correct relative stereochemistry?

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/4385168fd175935e8ef6184debc787ca3ef2090b8db805519b14b1592e563d87.jpg]]

## 参考答案

**Answer (English)**: We should first discover which atoms in the cyclic compound provide which atoms in the product. Numbering the atoms is the easiest way and it shows little change except that C9 has gone and C8 has become an aldehyde.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/c4ecd361ae97a7cbc5a6a7ebf2e16391a71b3f526a0a588de21a13854d843bd4.jpg]]

We need to hydrolyse the ester and the acetal and oxidize the 1,2-diol to cleave the C–C bond between the two OH groups. The stereochemistry at C3 and C7 is unchanged and neither is threatened by any of the reaction conditions.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/bae40e3d012be209e05522091f67b366927a0c16a194bc49e120ebbfa2fd9876.jpg]]

**中文解析**：

关键步骤：
1. **原子映射**：首先确定环状化合物中的哪些原子对应开链产物中的哪些原子——通过编号可以发现变化很小，只是C9消失，C8变成了醛
2. **水解和氧化裂解**：需要水解酯和缩醛，并氧化1,2-二醇以断裂两个OH之间的C-C键（如NaIO₄氧化裂解）
3. **立体化学保持**：C3和C7的立体化学在所有反应条件下都不受影响——酯水解、缩醛水解和二醇氧化都不会触及这两个手性中心

> **核心概念**：环状化合物是控制开链分子相对立体化学的强大工具——因为环的构象固定了取代基的空间关系。通过选择性地断裂环中的特定键，可以将环状的立体化学信息"翻译"到开链产物中。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[立体化学]] | 环状到开链的立体化学保持 | 直接 |
| [[非对映选择性]] | 利用环状中间体控制开链立体化学 | 直接 |
| [[构象分析]] | 环的固定构象提供立体化学信息 | 间接 |

## 解题思路

1. **读题定位**：从环状化合物制备开链产物并保持立体化学——识别底物为含酯/缩醛/二醇的多官能团环状化合物
2. **关键转换**：编号映射原子→水解酯+缩醛→NaIO₄氧化裂解1,2-二醇→C3和C7立体化学不变
3. **验证**：检查开链产物的C3和C7立体化学是否与环状原料一致

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 不知道断哪个键 | 未做原子映射 | 先编号确定哪些原子对应产物中的哪些原子 | 如何快速映射环状→开链的原子对应关系？ |
| 氧化裂解时破坏手性中心 | 担心氧化会消旋化 | NaIO₄只裂解1,2-二醇的C-C键，不影响其他手性中心 | NaIO₄氧化的机理是什么？ |
| 忘记水解酯和缩醛 | 只关注氧化裂解 | 需要先水解酯和缩醛，暴露出1,2-二醇才能氧化 | 缩醛水解的条件是什么？ |