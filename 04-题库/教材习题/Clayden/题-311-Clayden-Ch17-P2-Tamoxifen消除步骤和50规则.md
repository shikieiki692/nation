---
title: 题-311-Clayden-Ch17-P2-Tamoxifen消除步骤和50规则
type: 题目
fidelity: 原书逐字
submodule: 消除反应
exam_stage: 初赛
source_subject: 有机化学
difficulty: 3
question_type: [机理]
teaching_level: 拓展
syllabus_codes: ["3.2"]
knowledge_points: ["[[E1反应]]"]
tags: [化竞, Clayden, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch17-P2]
source: Clayden Organic Chemistry 2nd Ed. Chapter 17 Problem 2
cross_references: ["[[题-299-Clayden-Ch15-P1-SN1与SN2机理判断]]", "[[题-310-Clayden-Ch17-P1-两个消除反应机理]]", "[[题-295-Clayden-Ch14-P5-反应产物手性和对映体纯度]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
---
# 题-311: Tamoxifen消除步骤和50规则

## 题目

Give a mechanism for the elimination reaction in the formation of tamoxifen, a breast cancer drug, and comment on the roughly 50:50 mixture of geometrical isomers (cis- and trans-alkenes).

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/437d2b3fa331671def6ebad51928d920f7db2ef9eb9b3a8e0780e8037b36943f.jpg]]

**原文题目**：

为他莫昔芬（一种乳腺癌药物）的形成中的消除反应提出机理，并解释大约50:50的几何异构体混合物（顺式-和反式-烯烃）。

## 参考答案

**Answer (English)**: The tertiary alcohol leaving group, the acid catalyst, and the 50:50 mixture all suggest E1 rather than E2. There is only one proton that can be lost and, as there is very little difference between the isomeric alkenes, equilibration probably gives the 50:50 mixture.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/c64a90abb6edfde420f1c0d1f24ed0b839b0a1f21a2a3410c5f3c9f3e798881e.jpg]]

**中文解析**：

**E1消除机理分析**：

| 判断依据 | 说明 |
|---------|------|
| 三级醇离去基团 | 三级碳正离子稳定→支持E1 |
| 酸催化条件 | 酸催化下三级醇易形成碳正离子→E1 |
| 50:50混合物 | 产物平衡→E1特征（非E2的立体专一性） |

**为什么是E1而不是E2？**

1. **三级醇**：OH是差的离去基团，但三级碳正离子稳定
2. **酸催化**：酸使OH质子化，变成H₂O（好的离去基团）
3. **只有一个β-H**：只有一个质子可以被消除
4. **50:50混合物**：说明存在平衡，E1消除后烯烃可以异构化

**E1机理步骤**：

| 步骤 | 过程 | 速率 |
|------|------|------|
| 1 | OH质子化 | 快（可逆） |
| 2 | H₂O离去→碳正离子 | 慢（速率决定步骤） |
| 3 | β-H消除→烯烃 | 快 |

**50:50混合物的原因**：

- E1消除后，产物烯烃可以在酸催化下异构化
- 顺式和反式烯烃的能量差异很小
- 平衡倾向于50:50混合物（热力学控制）
- 这是E1消除的典型特征

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| E1反应 | E1消除的机理和特征 | 直接 |
| [[消除反应]] | E1与E2的区分 | 直接 |
| 烯烃异构化 | 产物平衡和50:50混合物 | 间接 |

## 解题思路

1. **读题定位**：Tamoxifen合成中的消除反应，50:50混合物
2. **🔑 关键转换**：三级醇+酸催化→E1；50:50混合物→烯烃异构化平衡
3. **验证**：检查碳正离子稳定性，确认E1的合理性

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 认为这是E2消除 | 没有考虑50:50混合物的含义 | E2是立体专一性的，不会给出50:50混合物 | 什么条件下E2会给出50:50混合物？ |
| 忽略酸催化作用 | 没有理解三级醇的活化 | 酸使OH质子化，变成好的离去基团 | 为什么碱不能活化三级醇？ |
| 不理解50:50混合物 | 没有考虑烯烃异构化 | E1产物可以在酸催化下异构化 | 什么因素决定异构化平衡的位置？ |