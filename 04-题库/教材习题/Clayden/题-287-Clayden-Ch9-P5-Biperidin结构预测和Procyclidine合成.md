---
title: 题-287-Clayden-Ch9-P5-Biperidin结构预测和Procyclidine合成
type: 题目
fidelity: 原书逐字
submodule: 有机金属试剂
exam_stage: 初赛
subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[Grignard试剂]]", "[[有机锂试剂]]"]
tags: [化竞, Clayden, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch9-P5]
source: Clayden Organic Chemistry 2nd Ed. Chapter 9 Problem 5
cross_references: ["[[题-283-Clayden-Ch9-P1-有机金属加成羰基的机理]]", "[[题-285-Clayden-Ch9-P3-Fenarimol替代合成路线]]"]
module: 有机化学
status: 已填充
---
# 题-287: Biperidin 结构预测和 Procyclidine 合成

## 题目

The antispasmodic drug biperidin is made by the Grignard addition reaction shown here. What is the structure of the drug? Do not be put off by the apparent complexity of the structure: just use the chemistry of Chapter 9.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/46b559c3b62ec893d7deb56ee0a4043e4ebbe4b43e20c5f8d46cfc98831d23b6.jpg]]

How would you suggest that the drug procyclidine should be made?

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/f1536b4fc5e5759c9e38195b8bcd765b8c61708685033cecd106981e618fac16.jpg]]

**原文题目**：抗痉挛药物 Biperidin 通过所示的 Grignard 加成反应制备。该药物的结构是什么？不要被表面的复杂性吓倒——只需运用第9章的化学。你建议如何合成药物 Procyclidine？

## 参考答案

**Answer (English)**: A Grignard reagent must be formed from the alkyl bromide and this must add to the ketone. Aqueous acidic work-up gives a tertiary alcohol and that is biperidin. To get procyclidine, we must change both the alkyl halide and the ketone but the reaction is very similar.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/005d74277f134b5c93873997484b67ca0dab5c53442cde92785b543facb916e3.jpg]]
![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/75d6db79d23f83a7447d780fa8ec49d3365cbea7c021c83bfc3809c417a7b07b.jpg]]

**中文解析**：

关键分析：
1. **Biperidin 结构推导**：烷基溴化物（含二苯甲基哌啶结构）先转化为 Grignard 试剂，然后对酮（二苯甲酮衍生物）进行亲核加成。酸性水处理后得到叔醇——这就是 Biperidin 的结构
2. **反应本质**：尽管分子看起来很复杂，但核心反应就是一个标准的 Grignard 加成：R-MgBr + R'COR'' → 三级醇
3. **Procyclidine 合成**：将烷基卤化物和酮同时替换——使用不同的烷基溴化物（含环己基哌啶结构）和不同的酮（环己基苯基酮），反应模式完全相同：Grignard 加成 → 叔醇

> **核心洞察**：不要被分子的复杂外表迷惑，将其拆解为"Grignard 试剂来源"和"酮底物"两部分即可。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[Grignard试剂]] | Grignard 试剂的制备（RBr → RMgBr）及对酮的加成 | 直接 |
| [[有机锂试剂]] | 有机锂替代 Grignard 的可能性 | 直接 |
| [[C-C键形成]] | 通过 Grignard 加成构建复杂药物分子的叔醇 | 间接 |

## 解题思路

1. **读题定位**：两个子问题——(a) 从反应式推导 Biperidin 结构；(b) 为 Procyclidine 设计类似合成。核心反应都是 Grignard + 酮 → 叔醇
2. **🔑 关键转换**：将复杂分子拆分为 Grignard 试剂（来自烷基溴化物）和酮底物两部分。酸性水处理（work-up）后得到叔醇。Procyclidine 只需更换两个组分
3. **验证**：检查 Biperidin 中新形成的 C-C 键位置（烷基与酮碳之间），确认叔醇的三个取代基分别来自 Grignard 试剂和酮的两侧

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 被复杂结构吓倒，不知从何下手 | 未识别核心反应类型 | 将分子拆分为"Grignard 来源"和"酮底物"两部分 | 复杂药物合成题的一般策略是什么？ |
| 忘记酸性水处理步骤 | Grignard 加成后中间体是烷氧基盐 | 必须有 H₃O⁺ work-up 才能得到中性叔醇 | 如果不做酸性水处理会得到什么？ |
| 误认为 Procyclidine 只需更换一个组分 | 未仔细对比两个药物结构 | Procyclidine 的烷基卤化物和酮都与 Biperidin 不同 | 如何快速判断哪些部分需要改变？ |