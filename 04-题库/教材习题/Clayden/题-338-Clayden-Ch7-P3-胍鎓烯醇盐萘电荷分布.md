...
updated: 2026-07-25
aliases: [Clayden-Ch7-P3]
source: Clayden Organic Chemistry 2nd Ed. Chapter 7 Problem 3
...
| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[共轭效应]] | 电荷在共轭体系中的离域分布 | 直接 |
| [[离域效应]] | 正/负电荷通过共振分散到多个原子上 | 直接 |
| [[共振论]] | 用弯曲箭头画出等价/不等价共振结构 | 间接 |

## 解题思路

1. **读题定位**：题目要求用两种方式表示共轭——(a)弯曲箭头连接共振结构，(b)虚线+部分电荷表示电荷分布
2. **🔑 关键转换**：对于每个分子，先确定电荷的起始位置，然后用弯曲箭头将电子（及其携带的电荷）移到相邻原子上。胍鎓：正电荷在N间轮转；烯醇盐：负电荷在O和C间移动；萘：π键在外环移动
3. **验证**：检查画出的共振结构是否遵循八隅体规则（尤其注意N不能有5个键），电荷守恒（净电荷不变）

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 胍鎓中给N画5个键 | 画共振结构时忘记N只能有3个键+1个孤对 | N最多形成4个键（如NH₄⁺），在胍鎓中N有3个σ键+正电荷 | N在胍鎓中的形式电荷是多少？ |
| 烯醇盐中将负电荷只画在C上 | 只想到"碳负离子"而忽略O的电负性 | 负电荷主要在O上（O更电负），只有少量在C上，这正是烯醇盐O-烷基化vs C-烷基化的基础 | 烯醇盐的亲核性主要来自C还是O？ |
| 萘的共振结构画成两个独立苯环 | 忽略了两个环是共用边的 | 萘的π电子在整个外环离域（10个π电子），而非两个独立的苯环 | 萘的1,4-位和1,5-位的反应活性有何不同？ |