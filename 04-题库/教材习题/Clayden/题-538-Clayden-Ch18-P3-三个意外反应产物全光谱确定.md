...
updated: 2026-07-25
aliases: [Clayden-Ch18-P3]
source: Clayden Organic Chemistry 2nd Ed. Chapter 18 Problem 3
...
| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[波谱综合解析]] | 综合 MS/IR/NMR 三种光谱推导三个未知结构 | 直接 |
| [[NMR谱学]] | 化学位移估算区分 OMe (3–3.5 ppm) 与 SMe (2.5–2.8 ppm) | 直接 |
| [[IR光谱]] | 酯 (1745)、酮 (1710)、酸衍生物的 C=O 频率区分 | 直接 |
| [[质谱]] | 奇/偶分子离子判断 N 原子数；S 的同位素模式 | 间接 |

## 解题思路

1. **读题定位**：三个反应各给出光谱数据，要求"不要猜应该发生什么"——纯靠数据说话
2. **🔑 关键转换**：A 由 t-Bu + OMe + 酯羰基直接拼装；B 通过 CH₂ 的异常化学位移(3.24 ppm)锁定双羰基夹心结构；C 通过 δH 2.8/δC 45 判断为 SMe 而非 OMe，从而确定含 S 分子式
3. **验证**：检查每个结构的分子式、不饱和度、各信号归属是否自洽

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 将 C 的 δH 2.8 归为 OMe | 未记住 OMe 典型位移在 3–3.5 ppm | SMe 典型位移 2.4–2.8 ppm，与实测吻合 | OMe 和 SMe 在 ¹³C NMR 中各在什么位置？ |
| 试图用 C₅H₁₀O₃ 解释 C | 忽略了 S 与 O 的质量差异（32 vs 16） | m/z 相同但结构完全不同，必须综合其他数据判断 | 如何从质谱区分 C₅H₁₀O₃ 和 C₅H₁₀OS？ |
| 认为 B 中 CH₂ (3.24 ppm) 与 O 相连 | 未数清 O 原子数 | 已用完所有 O（酯 + 酮），该 CH₂ 只能在两个 C=O 之间 | 两个羰基之间的 CH₂ 位移为什么偏高？ |