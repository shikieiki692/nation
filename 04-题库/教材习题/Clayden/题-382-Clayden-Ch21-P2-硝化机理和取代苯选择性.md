---
title: 题-382-Clayden-Ch21-P2-硝化机理和取代苯选择性
type: 题目
fidelity: 原书逐字
submodule: 芳香亲电取代
exam_stage: 初赛
...
updated: 2026-07-25
aliases: [Clayden-Ch21-P2]
source: Clayden Organic Chemistry 2nd Ed. Chapter 21 Problem 2
...
本题考察硝化反应机理和取代基定位效应的综合应用。

关键要点：
1. **NO₂⁺的生成**：HNO₃ + H₂SO₄ → NO₂⁺ + HSO₄⁻ + H₂O（浓硫酸作为脱水剂和酸催化剂）
2. **苯甲酸**：含吸电子基（-COOH），间位定位，硝化产物为间硝基苯甲酸
3. **多甲基取代苯**：所有烷基都是弱给电子基，所有位置都被活化。但由于空间位阻，硝化发生在远离烷基的位置
4. **竞争性定位基**：当环上有两个邻对位定位基时，较强的定位基决定产物。含孤对电子的基团（N、O）比简单烷基的定位能力强得多
5. **对甲基苯胺**：N的定位能力强于甲基，产物为邻硝基（相对于N）
6. **对甲基苯甲醚**：O的定位能力强于甲基，且空间位阻使对位略优于邻位

> **定位基强度排序**：含孤对电子的基团（-NH₂、-OH、-OR）> 烷基 > 卤素 > 吸电子基（-NO₂、-COOH等）
...
| [[芳香亲电取代]] | 硝化反应的完整机理 | 直接 |
| [[硝化反应]] | NO₂⁺的生成与亲电进攻 | 直接 |
| [[定位效应]] | 多取代苯中定位基的竞争与优先级 | 间接 |
| [[活化基]] | 给电子基对芳香环的活化作用 | 间接 |

## 解题思路

1. **读题定位**：题目要求写出硝化机理，并预测四个取代苯的硝化产物
2. **🔑 关键转换**：先写出NO₂⁺生成机理→对每个底物分析取代基的定位效应→当有竞争时比较定位基强度→考虑空间位阻的影响
3. **验证**：检查产物中硝化位置是否符合定位规则；检查产物数量是否为单一产物

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 忽略定位基强度比较 | 不知道当两个定位基竞争时如何判断 | 含孤对电子的基团（N、O）定位能力强于烷基 | -NH₂和-OH哪个定位能力更强？ |
| 忽略空间位阻效应 | 只考虑电子效应 | 大基团附近的空间位阻会导致硝化发生在对位而非邻位 | 为什么对甲基苯甲醚的硝化主要在对位？ |
| 将吸电子基当作邻对位定位基 | 定位基分类不清 | 吸电子基（-COOH、-NO₂等）是间位定位基 | 如何快速判断一个基团是活化还是钝化？ |