---
title: 题-296-Clayden-Ch14-P6-碱性条件下外消旋化
type: 题目
submodule: 立体化学
exam_stage: 初赛
subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["36"]
knowledge_points: ["[[立体化学]]", "[[对映异构]]"]
tags: [化竞, Clayden, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch14-P6]
source: Clayden Organic Chemistry 2nd Ed. Chapter 14 Problem 6
cross_references: ["[[题-295-Clayden-Ch14-P5-反应产物手性和对映体纯度]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]"]
module: 有机化学
status: 已填充
---
# 题-296: 碱性条件下外消旋化

## 题目

This compound racemizes in base. Why is that?

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/56abb8e1193c3322c530e99e8963f375c51df87aa6ac83391dcb7fbee72dbe6b.jpg]]

**原文题目**：该化合物在碱性条件下外消旋化。为什么？

## 参考答案

**Answer (English)**: Ester exchange in base goes through a symmetrical (achiral) tetrahedral intermediate with a plane of symmetry. Loss of the right hand leaving group gives one enantiomer and loss of the left gives the other.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/796f2c7292ceca343ce45bd478936f32c61af141695fb99057e6518ba22fcff5.jpg]]

**中文解析**：

这个手性酯在碱性条件下通过**酯交换机理**外消旋化：

1. **OH⁻进攻**：碱（OH⁻）进攻酯的羰基碳，形成四面体中间体
2. **对称中间体**：这个四面体中间体是**对称的（非手性的）**——有对称面
3. **消除方向随机**：
   - 从右侧消除→得到一种对映体
   - 从左侧消除→得到另一种对映体
4. **外消旋化**：两种消除概率相等→得到外消旋混合物

关键洞察：**近对称分子的危险性**——起始物料看起来是单一对映体，但通过碱性条件下的酯交换，经过对称中间体，最终外消旋化。

> 这是工作手性化合物时的重要警告：如果分子"几乎对称"，碱性条件可能导致外消旋化！

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[立体化学]] | 外消旋化的机理 | 直接 |
| [[对映异构]] | 对映体纯度的保持和丧失 | 直接 |
| [[酯的水解]] | 酯交换作为外消旋化途径 | 间接 |
| [[平衡常数]] | 对称中间体的形成驱动外消旋化 | 间接 |

## 解题思路

1. **读题定位**：手性酯在碱中外消旋化→需要找到经过对称中间体的机理
2. **🔑 关键转换**：酯交换→四面体中间体（对称的！）→随机消除→外消旋化
3. **验证**：检查中间体是否真的有对称面——两个酯基相同→有对称面→非手性

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 认为碱直接拔氢导致外消旋化 | 没有考虑实际机理 | 碱进攻羰基→酯交换→对称中间体→外消旋化 | 为什么α-H不能被碱直接拔掉？ |
| 没有识别对称中间体 | 没有画出完整机理 | 四面体中间体的两个酯基相同→有对称面 | 如何设计实验验证这个机理？ |
| 认为外消旋化只通过碳负离子 | 过度概括 | 外消旋化可以通过多种途径：碳负离子、对称中间体、可逆反应等 | 哪些手性中心容易被碱外消旋化？ |