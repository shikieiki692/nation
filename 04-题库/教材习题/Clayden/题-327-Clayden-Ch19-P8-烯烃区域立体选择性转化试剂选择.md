---
title: 题-327-Clayden-Ch19-P8-烯烃区域立体选择性转化试剂选择
type: 题目
fidelity: 原书逐字
submodule: 烯烃的亲电加成
exam_stage: 初赛
subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["2.3", "2.4"]
knowledge_points: ["[[亲电加成]]"]
tags: [化竞, Clayden, 有机化学]
updated: 2026-07-25
aliases: [Clayden-Ch19-P8]
source: Clayden Organic Chemistry 2nd Ed. Chapter 19 Problem 8
cross_references: ["[[题-320-Clayden-Ch19-P1-HCl对三个烯烃加成方向]]", "[[题-323-Clayden-Ch19-P4-环戊二烯单环氧化选择性]]", "[[题-392-Clayden-Ch22-P2-共轭加成本质和不发生的情况]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
---
# 题-327: 烯烃区域/立体选择性转化试剂选择

## 题目

For each of the following transformations, select the appropriate reagent(s) and explain the regiochemistry and stereochemistry:

1. Alkene → Markovnikov alcohol
2. Alkene → anti-Markovnikov alcohol
3. Alkene → syn-diol
4. Alkene → trans-diol

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/2ec1a53272da729cc332177e427f6dfb21d9c0bf9583b1f63e1c02f44e77ce04.jpg]]

**原文题目**：

对于下列每种转化，选择合适的试剂并解释区域选择性和立体化学：

1. 烯烃 → Markovnikov 醇
2. 烯烃 → 反 Markovnikov 醇
3. 烯烃 → 顺式邻二醇
4. 烯烃 → 反式邻二醇

## 参考答案

**Answer (English)**:

1. **Markovnikov alcohol**: **Acid-catalyzed hydration** (H₃O⁺, e.g., dilute H₂SO₄ in H₂O) or **oxymercuration–demercuration** (Hg(OAc)₂/H₂O, then NaBH₄). Protonation gives the more stable carbocation → water captures → Markovnikov product. Oxymercuration avoids carbocation rearrangements.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/756ab715caf52f1efed25d7388f6634c0982929daa7dec2da4b0c6103ffd750e.jpg]]

2. **Anti-Markovnikov alcohol**: **Hydroboration–oxidation** (1) BH₃·THF (or 9-BBN, disiamylborane for terminal alkenes), (2) H₂O₂, NaOH. Boron adds to the less substituted carbon (sterics + electronics) → oxidation replaces B with OH → anti-Markovnikov. Syn addition.

3. **Syn-diol**: **OsO₄** (catalytic, with NMO as co-oxidant, or stoichiometric). OsO₄ performs a [3+2] cycloaddition with the alkene → cyclic osmate ester → hydrolysis gives syn-diol. Stereochemistry: both OH groups add to the same face.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/830e6cfa3c6e4157b746173b5adae210d69c0044263f4fbb105248838b450a1d.jpg]]

4. **Trans-diol**: **m-CPBA (epoxidation) → H₃O⁺ (acid-catalyzed ring opening)**. m-CPBA gives the epoxide → acid protonates the epoxide oxygen → water attacks from the opposite face (inversion) → trans-diol. Anti addition overall.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_1-199_images/9f3f6f708322722e35d879e3e84b89e15b5d01495ccea3a3c40e6f33c837cfb7.jpg]]

rotate about

**中文解析**：

| 转化 | 试剂 | 区域选择性 | 立体化学 |
|------|------|-----------|----------|
| Markovnikov 醇 | H₃O⁺ 或 oxymercuration-demercuration | OH 加在取代度高的碳 | 可能重排（酸催化）或无重排（汞化） |
| 反 Markovnikov 醇 | ① BH₃·THF ② H₂O₂/NaOH | OH 加在取代度低的碳 | syn 加成 |
| 顺式邻二醇 | OsO₄ (cat.) + NMO | 两个 OH 加在同一面 | syn 加成 |
| 反式邻二醇 | ① m-CPBA ② H₃O⁺ | 两个 OH 加在对面 | 反式（anti）加成 |

1. **酸催化水合**：H⁺ 质子化双键 → 碳阳离子 → H₂O 捕获 → Markovnikov 醇。但可能有碳阳离子重排。Oxymercuration 经汞鎓离子避免重排。
2. **硼氢化-氧化**：硼烷对烯烃加成时，B 原子加在取代度低的碳上（空间位阻+电子效应），H 加在取代度高的碳上。氧化步骤将 B 原子原位替换为 OH，保持构型。整体 anti-Markovnikov、syn 加成。
3. **OsO₄**：形成 [3+2] 环加成中间体（环状锇酸酯），两个 O 同时加在烯烃同一面 → syn 邻二醇。
4. **m-CPBA + 水解**：m-CPBA 将烯烃环氧化 → 酸催化开环时水从环氧背面进攻 → 两个 OH 反式排列。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[亲电加成]] | 各种烯烃加成反应的机理和试剂选择 | 直接 |
| [[区域选择性]] | Markovnikov vs anti-Markovnikov 产物的试剂控制 | 直接 |
| [[烯烃]] | 烯烃作为合成中间体的多样化转化策略 | 间接 |

## 解题思路

1. **读题定位**：四种烯烃转化，每种需要不同的区域/立体选择性——匹配试剂
2. **🔑 关键转换**：Markovnikov → 酸催化/Oxymercuration；反 Markovnikov → 硼氢化；syn → OsO₄；anti → 环氧化+水解
3. **验证**：检查每种试剂的区域选择性和立体化学是否匹配目标转化

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 混淆 OsO₄ 和 m-CPBA 的立体化学 | 记忆模糊 | OsO₄ → syn（同面）；m-CPBA+H₂O → anti（反面） | 为什么OsO₄只能给出syn产物？ |
| 硼氢化用 BH₃ 处理端烯 | 端烯活性太高易多次加成 | 端烯应用位阻大的硼烷（9-BBN或disiamylborane） | 为什么位阻大的硼烷可以停在单加成？ |
| 酸催化水合和 oxymercuration 混用 | 不了解两者区别 | 酸催化可能重排；oxymercuration经汞鎓离子无重排 | 什么情况下酸催化水合会出现重排？ |