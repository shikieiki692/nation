---
title: 题-482-Clayden-Ch27-P6-昆虫信息素Wittig立体化学
type: 题目
fidelity: 原书逐字
submodule: 硅硅磷化学
exam_stage: 初赛
subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[Wittig反应]]"]
tags: [化竞, Clayden, 有机化学, 硫化学]
updated: 2026-07-25
aliases: [Clayden-Ch27-P6]
source: Clayden Organic Chemistry 2nd Ed. Chapter 27 Problem 6
cross_references: ["[[题-424-Clayden-Ch23-P1-溴代醛选择性转化为两个产物]]", "[[题-425-Clayden-Ch23-P2-内酯选择性开环]]"]
module: 有机化学
status: 已填充
---
# 题-482: 昆虫信息素Wittig立体化学

## 题目

A single geometrical isomer of an insect pheromone was prepared in the following way. Which isomer is formed and why?

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/b4e0aa0f63b690642ee7d715412cd0aa1c571b6296b0b72e15526e107c959bf8.jpg]]

**原文题目**：A single geometrical isomer of an insect pheromone was prepared in the following way. Which isomer is formed and why?

## 参考答案

**Answer (English)**: The first Wittig with a stabilized ylid gives the E-enal A. The second, with an unstabilized ylid, gives a Z-alkene so the final product is an E,Z-diene.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/dfcbd4abd0f6002650dbda14191c50130c6d6edb71ee301d36c50538574ac094.jpg]]

**中文解析**：

关键步骤：
1. **第一个Wittig反应**：稳定化叶立德（醛基共轭稳定）→ E-烯醛（A）
   - 稳定化叶立德热力学控制 → E选择性
2. **第二个Wittig反应**：非稳定化叶立德 → Z-烯烃
   - 非稳定化叶立德动力学控制 → Z选择性
3. **最终产物**：E,Z-二烯（昆虫信息素）

> **核心要点**：通过选择不同类型的Wittig叶立德（稳定化vs非稳定化），可以在同一个分子中分别控制两个双键的E/Z构型。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| Wittig反应 | 两次Wittig反应的立体化学控制 | 直接 |
| [[烯烃立体选择性]] | 稳定化→E，非稳定化→Z的规则应用 | 直接 |
| [[逆合成分析]] | 信息素合成中的立体化学策略 | 间接 |

## 解题思路

1. **读题定位**：两次Wittig反应合成信息素→需判断每个双键的构型
2. **关键转换**：第一次（稳定化叶立德）→E；第二次（非稳定化叶立德）→Z；总产物E,Z-二烯
3. **验证**：检查两次Wittig的底物类型是否与产物构型一致

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 两次都画成E或都画成Z | 没区分叶立德类型 | 第一次稳定化→E，第二次非稳定化→Z | 如何从底物判断叶立德是否稳定化？ |
| 混淆反应顺序 | 没仔细读合成路线 | 先做E（稳定化），再做Z（非稳定化） | 为什么这个顺序是合理的？ |
| 忽略"single isomer" | 没注意题目要求 | 题目说得到单一异构体→两个双键都有立体选择性 | 信息素的立体化学为什么重要？ |