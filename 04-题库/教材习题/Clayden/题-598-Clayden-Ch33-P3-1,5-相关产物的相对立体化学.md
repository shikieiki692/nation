---
title: 题-598-Clayden-Ch33-P3-1,5-相关产物的相对立体化学
type: 题目
fidelity: 原书逐字
submodule: 非对映选择性
exam_stage: 初赛
source_subject: 有机化学
difficulty: 2
teaching_level: 巩固
syllabus_codes: ["21"]
knowledge_points: ["[[立体化学]]"]
tags: [化竞, Clayden, 有机化学, 非对映选择性]
updated: 2026-07-25
aliases: [Clayden-Ch33-P3]
source: Clayden Organic Chemistry 2nd Ed. Chapter 33 Problem 3
cross_references: ["[[题-433-Clayden-Ch24-P2-不饱和羰基直接共轭加成区域选择性]]", "[[题-432-Clayden-Ch24-P1-氨基醇制备中区域选择性试剂选择]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]", "[[题-291-Clayden-Ch14-P1-五个分子手性判断]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
source_category: 教材课后习题
source_grade: C
---
# 题-598: 1,5-相关产物的相对立体化学

## 题目

How is the relative stereochemistry of this product controlled? Why was this method chosen?

![[f9fde3011df5caf5ecc4bf0316062b4ef5d8b1980181f43e3987ccc9bea17335.jpg]]

## 参考答案

**Answer (English)**: The relationship between the two chiral centres in the product is 1,5 and that is too remote for any realistic control. The only plan is to disconnect between the two centres and add a removable anion-stabilizing group to one side and a leaving group to the other. The starting materials must of course be single enantiomers—then only one diastereoisomer can be formed.

![[286a256f6b34953f2aea918e829126f1b4b07bcee1a888deba606ad5aeeddcbc.jpg]]

**中文解析**：

关键步骤：
1. **1,5-关系的挑战**：产物中两个手性中心的相对关系是1,5-相关的——距离太远，无法通过任何现有的非对映选择性方法直接控制
2. **逆合成分析**：唯一的策略是在两个手性中心之间断开，在一侧引入可移除的阴离子稳定基团（如酯基），另一侧引入离去基团（如溴）
3. **光学纯起始原料**：关键在于使用单一对映体的起始原料——当两个片段都是光学纯的，$S_{\mathrm{N}}2$取代只会产生一种非对映异构体

> **核心概念**：对于距离较远的手性中心（1,5-或更远），直接的立体化学控制不可行。策略是通过逆合成断开，分别控制每个手性中心的绝对构型，然后通过$S_{\mathrm{N}}2$等立体专一性反应连接。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[立体化学]] | 1,5-相关手性中心的控制策略 | 直接 |
| [[非对映选择性]] | 光学纯原料+$S_{\mathrm{N}}2$→单一对映体 | 直接 |
| [[手性中心]] | 远距离手性中心的逆合成断开 | 间接 |

## 解题思路

1. **读题定位**：解释相对立体化学的控制——识别产物为1,5-相关的二醇衍生物
2. **关键转换**：1,5-关系太远→无法直接控制→逆合成断开→一侧阴离子稳定基团+一侧离去基团→光学纯原料+$S_{\mathrm{N}}2$→单一非对映异构体
3. **验证**：检查两个起始原料是否为单一对映体，连接后是否只有一种产物

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 试图用一个反应控制两个中心 | 1,5-关系太远，无法一步控制 | 必须分开控制，然后连接 | 多远的手性中心可以一步控制？ |
| 忘记需要光学纯原料 | 以为可以外消旋体反应 | 必须使用光学纯起始原料，否则得到混合物 | 如果用外消旋体会得到几种产物？ |
| 混淆阴离子稳定基团和离去基团 | 未理解逆合成逻辑 | 一侧需要阴离子稳定基团（如酯），另一侧需要离去基团（如Br） | 为什么需要阴离子稳定基团？ |