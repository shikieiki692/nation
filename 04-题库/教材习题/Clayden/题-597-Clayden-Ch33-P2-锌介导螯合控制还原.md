---
title: 题-597-Clayden-Ch33-P2-锌介导螯合控制还原
type: 题目
fidelity: 原书逐字
submodule: 非对映选择性
exam_stage: 初赛
source_subject: 有机化学
difficulty: 3
teaching_level: 拓展
syllabus_codes: ["21"]
knowledge_points: ["[[非对映选择性]]"]
tags: [化竞, Clayden, 有机化学, 非对映选择性]
updated: 2026-07-25
aliases: [Clayden-Ch33-P2]
source: Clayden Organic Chemistry 2nd Ed. Chapter 33 Problem 2
cross_references: ["[[题-432-Clayden-Ch24-P1-氨基醇制备中区域选择性试剂选择]]", "[[题-292-Clayden-Ch14-P2-旋光值区分]]", "[[题-291-Clayden-Ch14-P1-五个分子手性判断]]", "[[题-433-Clayden-Ch24-P2-不饱和羰基直接共轭加成区域选择性]]"]
module: 有机化学
status: 已填充
subject_module: 有机化学
pack: 章节练习
source_category: 教材课后习题
source_grade: B
---
# 题-597: 锌介导螯合控制还原

## 题目

Explain the stereochemistry shown in this sequence of reactions.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/2d2e63179ef58eee1fd2cc7b5bfd28b6813a21dcddcbb42fad0c0462a4925807.jpg]]

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/715bb26b2ba58ae0dae45241b86661f598f7b53befedcf26f4df4eb5f387c337.jpg]]

## 参考答案

**Answer (English)**: In both reductions the zinc atom is coordinated to the oxygen of the nearer functional group (CO₂Bn in the first and OH in the second) and the oxygen of the ketone being reduced. This fixes the conformation of the molecule and the borohydride ion attacks from the less hindered side. Anti stereochemistry results in both cases.

![[06-外部资料导入/clayden 有机习题/课后习题答案及解析clayden-SolutionsManualtoAccompanyOrganicChemistry_200-399_images/b32ed5116ab692ac209695512ba9b8d1142a3608df9ec442379a7dc35548ce95.jpg]]

**中文解析**：

关键步骤：
1. **螯合控制**：在两次还原中，锌原子都与较近官能团的氧（第一次是CO₂Bn，第二次是OH）和被还原酮的氧配位。这种螯合作用固定了分子的构象
2. **BH₄⁻从位阻较小面进攻**：构象被固定后，硼氢化物从位阻较小的一侧进攻酮羰基
3. **anti立体化学**：两次还原都得到anti产物——新形成的OH与原有手性中心的取代基处于反式关系

> **核心概念**：螯合控制是开链化合物立体化学控制的重要方法。金属离子（如Zn²⁺、Mg²⁺、Ti⁴⁺）通过同时与两个氧配位，将开链分子固定在特定构象中，使亲核试剂只能从一个面进攻。

## 知识点映射

| 关联 KP | 考查角度 | 直接/间接 |
|---|---|:---:|
| [[非对映选择性]] | 螯合控制实现anti选择性 | 直接 |
| [[立体选择性]] | 金属配位固定构象控制反应面 | 直接 |
| [[螯合控制]] | Zn²⁺螯合的过渡态结构 | 直接 |

## 解题思路

1. **读题定位**：解释多步还原的立体化学——识别两次还原都是酮的还原，且都有邻近含氧官能团
2. **关键转换**：Zn²⁺螯合两个O（邻近官能团+酮羰基）→固定构象→BH₄⁻从位阻较小面进攻→anti产物
3. **验证**：检查两次还原的产物都是anti构型，且Zn²⁺的螯合可以画出合理的六元环过渡态

## 易错分析

| 错误 | 原因 | 纠正 | 课堂提问 |
|------|------|------|----------|
| 画出syn产物 | 未理解螯合控制 | 螯合控制通常给出anti产物（BH₄⁻从位阻较小面进攻） | 螯合控制vs非螯合控制的区别是什么？ |
| 忘记Zn²⁺配位 | 只考虑电子效应 | Zn²⁺同时配位两个氧是控制立体化学的关键 | 为什么Zn²⁺而不是Na⁺有螯合控制？ |
| 混淆两次还原的配位基团 | 未仔细分析底物 | 第一次配位CO₂Bn，第二次配位OH | 如果没有邻近官能团会怎样？ |