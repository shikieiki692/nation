---
title: "题-HChO配-40-四苯基卟啉的络合与姜泰勒畸变"
type: 题目
source: HChO化学竞赛·第21届培训班综合卷 第4题（7分）
source_file: "[[bdwp资源/03配合物总结]]"
submodule: 配合物专题
module: 配位化学
subject_module: 结构化学
exam_stage: 初赛
difficulty: 4
teaching_level: 竞赛
fidelity: 原书逐字
knowledge_points: ["[[配合物]]", "[[过渡元素]]", "[[分子结构]]"]
pack: 模块习题集
question_type: [简答, 选择]
tags: [化竞, HChO, 配合物专题, 卟啉, 姜泰勒效应]
created: 2026-09-22
updated: 2026-09-22
status: 待审核
source_category: 竞赛导向·竞赛教辅
source_grade: A

---

# HChO配-40 四苯基卟啉的络合与姜-泰勒畸变（7 分）

> 来源：HChO 第21届培训班综合卷 第4题；讲义题型三练习

## 题目

四苯基卟啉（TPP），是一种人工合成的配体，对一些金属有很好的络合作用。

H₂TPP —（1) KAuCl₄, HOAc/KOAc；2) KPF₆, H₂O/CH₂Cl₂）→ AuTPP⁺PF₆⁻ —（reductant）→ AuTPP

4-1 以下哪种化合物是 TPP?

![[4970eb28c239b14e0e779b22f51f54a44dfce0c7ea6132d7eec6a2ba4acfcb03.jpg]]。

4-2 指出 AuTPP⁺PF₆⁻ 中 Au 的杂化方式。

4-3 如下四种还原剂中，哪一种无法用于还原 AuTPP⁺PF₆⁻ 到 AuTPP?

![[fc7ec1d5e87138c6d319d25d1eaa71d67ecd7d9765d38606c5df0ed897e8e578.jpg]]。

4-4 实验事实表明，AuTPP 中其中两根 Au-N 键比另两根 Au-N 键短 7%，简述产生这一现象的原因。

## 解答

4-1 第二种。（2 分）

4-2 dsp² 杂化。（1.5 分）

4-3 Mg 不能（会取代 Au 形成 Mg 的卟啉络合物，且还原性太强，会出单质）。（1.5 分）

4-4 Au(II) 的价电子构型为 5d⁹6s⁰，最后 1 个电子的电子云在平面四方形场中是不对称的，因此产生了姜-泰勒畸变，成为拉长平面四方。（2 分，答出 d 电子云畸变即可得分，答难以形成 dsp² 杂化、仅能形成 sp 杂化也得分）

![[c55dc5903b92a60e755e893aa2302010165aa15bcc4a5e633e31dccd0ca72918.jpg]]

![[5f95703b15204fe52c608c4171d0aa8826afa9d0cbd6ec976c3e7f4e3f60f105.jpg]]
![[09e299bc490ffa88f62a350bd67408c7da5c3c3f92e3476bd8f709e13befae2d.jpg]]

## 解析要点

- 4-3 的判据是"还原剂不能比 Au 的卟啉更活泼"：Mg 会直接与卟啉配位把 Au 置换出来，且自身还原性过强会析出单质，所以它根本不适合。
- 4-4 是姜-泰勒效应的标准应用：d⁹ 构型在平面四方场中简并轨道被不对称占据，导致两条键被拉长、另两条相对缩短（"拉长平面四方"），题给"两根短 7%"正是该结论的实验体现。
- 由此题可见卟啉作为四齿螯合配体的刚性平面场是最适合讨论配体场畸变的模型体系之一。
