---
title: "题-040-结构化学基础-原子结构-习题2.18"
type: 题目
submodule: 结构化学基础
source: 周公度 段连运《结构化学基础（第5版）习题解析》第2章
source_file: "07-资料提炼/习题提炼/习题-结构化学基础-第2章-原子的结构和性质.md"
exam_stage: 初赛
subject: 结构化学
difficulty: 4
teaching_level: 竞赛拔高
syllabus_codes: ["9"]
knowledge_points: ["[[电离能]]", "[[电子结合能]]", "[[He原子]]", "[[屏蔽常数]]", "[[H-离子]]"]
tags: [化竞, 结构化学基础, 原子结构]
module: 原子结构
created: 2026-07-27
updated: 2026-07-27
status: 已填充
---

# 题-040：原子结构 习题2.18

> **来源**：周公度 段连运《结构化学基础（第5版）习题解析》第2章
> **难度**：⭐⭐⭐⭐
> **教学层级**：竞赛拔高

---

## 题目

已知 He 原子的第一电离能 $I_{1}=24.59\ \mathrm{eV}$ ，试计算：

(1) 第二电离能, 1s 的单电子原子轨道能和电子结合能;

(2) 基态能量；

(3) 在 1s 轨道中两个电子的互斥能；

(4) 屏蔽常数；

(5) 根据(4)所得结果求 $\mathrm{H}^{-}$ 的基态能量。

---

## 参考答案

解

(1) He 原子的第二电离能 $I_{2}$ 是下一电离过程所需要的最低能量, 即

$$
\begin{array}{r l} & {\mathrm {He^ {+} (g)\longrightarrow He^ {2 + } (g)+ e^ {-}}} \\ & {I _ {2} = \Delta E = E _ {\mathrm {He^ {2 + }}} - E _ {\mathrm {He^ {+}}}} \\ & {= 0 - E _ {\mathrm {He^ {+}}} = - E _ {\mathrm {He^ {+}}}} \end{array}
$$

$\mathrm{He}^{+}$ 是单电子"原子"， $E_{He^{+}}$ 可按单电子原子能级公式计算，因而

$$
I _ {2} = - E _ {\mathrm{He} ^ {+}} = - \left(- 1 3. 5 9 5 \mathrm{eV} \times \frac {2 ^ {2}}{1 ^ {2}}\right) = 5 4. 3 8 \mathrm{eV}
$$

1s 的单电子原子轨道能为

$$
- \frac {(2 4 . 6 + 5 4 . 4) \mathrm{eV}}{2} = - 3 9. 5 \mathrm{eV}
$$

电子结合能为-24.59eV。

(2) 从原子的电离能的定义出发, 按下述步骤推求 He 原子基态的能量:

$$
\mathrm{He(g)} \longrightarrow \mathrm {He^ {+} (g)+ e^ {-}} \quad I _ {1} = E _ {\mathrm {He^ {+}}} - E _ {\mathrm{He}}\tag{1}
$$

$$
\mathrm{He} ^ {+} (\mathrm{g}) \longrightarrow \mathrm{He} ^ {2 +} (\mathrm{g}) + \mathrm{e} ^ {-} I _ {2} = E _ {\mathrm{He} ^ {2 +}} - E _ {\mathrm{He} ^ {+}}\tag{2}
$$

由(1)式得 $E_{\mathrm{He}} = E_{\mathrm{He}^{+}} - I_1$

将(2)式代入,得

$$
\begin{array}{r l} E _ {\mathrm{He}} & = E _ {\mathrm{He} ^ {2 +}} - I _ {2} - I _ {1} = 0 - (I _ {1} + I _ {2}) \\ & = - (2 4. 5 9 \mathrm{eV} + 5 4. 3 8 \mathrm{eV}) = - 7 8. 9 7 \mathrm{eV} \end{array}
$$

推而广之, 含有 n 个电子的多电子原子 A, 其基态能量等于各级电离能之和的负值, 即

$$
E _ {\mathrm{A}} = - \sum_ {i = 1} ^ {n} I _ {i}
$$

(3) 用 $J(s, s)$ 表示 He 原子中两个 1s 电子的互斥能，则

$$
\begin{array}{r l} J (\mathrm{s}, \mathrm{s}) & = E _ {\mathrm{He}} - 2 E _ {\mathrm{He} ^ {+}} \\ & = - 7 8. 9 7 \mathrm{eV} - 2 \times (- 5 4. 3 8 \mathrm{eV}) = 2 9. 7 9 \mathrm{eV} \end{array}
$$

(4) 屏蔽常数

$$
\begin{array}{r l} \sigma & = 2 - \left[ \frac {E _ {\mathrm{He}}}{- 1 3 . 5 9 5 \mathrm{eV} \times 2} \right] ^ {\frac {1}{2}} \\ & = 2 - 1. 7 0 4 \approx 0. 3 \end{array}
$$

(5) $\mathrm{H}^{-}$ 是核电荷为 1 的二电子"原子"，其基组态为 $(1s)^{2}$ ，因而基态能量为

$$
\begin{array}{r l} E _ {\mathrm{H} ^ {-}} & = [ - 1 3. 5 9 5 \mathrm{eV} \times (1 - \sigma) ^ {2} ] \times 2 \\ & = [ - 1 3. 5 9 5 \mathrm{eV} \times (1 - 0. 3) ^ {2} ] \times 2 \\ & = - 1 3. 3 2 \mathrm{eV} \end{array}
$$

---
