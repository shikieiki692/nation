...
> 这道题的表面是"甲酸水热分解与 Ru 催化产氢"，本质是**以"快速预平衡＋决速步"的稳态逻辑为轴心的动力学-热力学联算题**——酸催化与自催化两条速率方程、由 k₁/k₂ 提取 pKa、气液分配修正平衡常数、van't Hoff 斜率取焓，最后落到 Ru 催化循环的配位化学。

---

## 知识点映射

- **直接 KP**：[[反应机理与近似]]、[[速率方程]]、[[van't Hoff方程]]
- **间接 KP**：[[酸碱平衡]]、[[平衡常数]]、气液分配系数（亨利定律型）
- **跨学科连接**：甲酸脱羧产氢是"液体氢载体"储氢路线的核心反应——Ru 催化循环（Laurenczy 体系）是绿色化学的真实研究前沿。

---

## 解题思路

1. **外加强酸走"预平衡质子化"**：H⁺ 过量时质子化甲酸 HC(OH)₂⁺ 由快速平衡决定（k′₊₁/k′₋₁），代入决速步得 r₁=k₁(HCOOH)(H⁺)——"预平衡＋决速步"是速率方程推导的标准两步走。
2. **自催化体系用 Ka 换 H⁺**：无外加酸时 [H⁺]≈√(Ka·[HCOOH])（电离度小），代入同一定速式 → r₂=k₁Ka^0.5[HCOOH]^1.5——1.5 级不是"新机理"，只是 Ka 的隐式贡献。
3. **pKa 从速率常数对提取**：k₂=k₁Ka^0.5 → pKa=2(lg k₁−lg k₂)，逐温度代入表格；pKa 随温度升高增大——高温水溶剂化变弱、酸性降低。
4. **K 与 Q 之间加"气液分配修正"**：Q=n(CO)/n(HCOOH)=K(V_g/V_aq·K_D+1)，而 V_g/V_aq=1/x_T−1 → K=x_T·Q/[(1−x_T)K_D+x_T]——每一步都把"可观测量"往"液相平衡量"折算。
5. **焓变从 lg~1/T 斜率读**：ΔH°_CO=−2.303R×(−3.0×10³)≈57 kJ/mol；ΔH°_D=−2.303R×(1.3×10³)≈−25 kJ/mol（溶解放热）；lg Q 的斜率组合 ΔH°_CO+βΔH°_D（β 由 K_D 与 x_T 表达）——van't Hoff 方程的三次复用。

---

## 变式拓展

- 若外加 HCl 浓度可变（非大过量），r₁ 的表达式如何修正为含 Ka 的完整形式？
- 3-3 若 CO 的 K_D 极大（几乎全在气相），K 的表达式退化成什么形式——物理直觉核对公式极限。
- 本题"由速率常数对提取热力学量（pKa）"与 38 决理-3"由零点能差提取 ΔH"同构——总结动力学数据反推热力学的三条路径。

---

## 易错分析

- 常见错误 1：3-1 无酸情形漏"[H⁺]≈√(Ka·[HCOOH])"的近似依据（电离度小），直接写 1.5 级结果丢过程分。
- 常见错误 2：3-2-2 只给数值不解释——pKa 随温度升高的原因是"高温下水溶剂化能力下降"，答案表要求解释 1 分。
- 常见错误 3：3-3 把 Q 直接当 K 用，或 V_g/V_aq 用错成 x_T（应为 1/x_T−1）——气液两相摩尔数加权是本题最容易翻车的代数段。
- 常见错误 4：3-4 忘了 K_D 对应的"反应方向"是 CO(aq)→CO(g)（溶解焓取负），斜率符号处理错导致 ΔH°_D 符号反；3-5 的 β 推导漏 (1−x_T)K_D 与 x_T 的权重结构。
