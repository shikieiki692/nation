...
> 这道题的表面是"乙醇在人体内的代谢路径"，本质是**以原电池设计计算平衡常数为前半场、以双底物酶促反应两种机理的速率方程推导与 Lineweaver-Burk 判别为后半场的热力学-动力学双主题综合题**——先"能不能反应"（ΔG/K），再"怎么反应、谁决速"（机理判别）。

---

## 知识点映射

- **直接 KP**：[[酶动力学]]、[[电化学]]（米氏方程、双倒数作图为纯文本概念，暂无独立 KP 页）
- **间接 KP**：生化标准态、[[速率方程]]、热力学平衡
- **跨学科连接**：生化标准态（[H⁺]=10⁻⁷ mol·L⁻¹）是生物化学与电化学的接口——pH 中性条件下电极电势整体平移；乙醇脱氢酶的双底物机理判别是酶学实验设计（固定一种底物作图族）的真实范式。

---

## 解题思路

1. **把氧化还原反应装配成电池**：乙醛/乙酸电对为负极（氧化）、RibO/Rib 为正极（还原），写电池表示式注意惰性电极 Pt 与双竖线盐桥；两极反应相加核对电子守恒。
2. **生化标准态求 K**：总反应不含 H⁺ → 生化标准态与化学标准态电动势相等，E°=−0.21−(−0.60)=0.39 V；再由 −RT ln K°=−zFE° 得 K°=1.5×10¹³，反应热力学上几乎进行到底。
3. **热力学"能行"与动力学"未行"分开答**：5-3 用 5-2 的巨大 K 说明热力学允许，未反应是动力学因素——乙醛氧化涉及 C–H 断裂活化能大，热力学结论不能代替速率。
4. **稳态近似推导双底物速率方程**：物料守恒＋米氏常数定义解出游离酶 [E]，代入决速步 r=k₂[ES₁S₂]；有序机理（A）只有一条结合路径，随机机理（B）有两条且利用 K_M1K_M12=K_M2K_M21 化简。
5. **双倒数图族判机理**：固定 [S₁] 对 1/[S₂] 作图——有序机理截距为常数 1/r_max（与 [S₁] 无关）；随机机理斜率、截距都随 1/[S₁] 线性变化。实验两组作图截距均非常数 → 随机顺序机理。

---

## 变式拓展

- 若把 5-2 中两个电极电势换成化学标准态数值（[H⁺]=1 mol·L⁻¹），K 的计算需要额外引入什么修正？（提示：两极反应各含几个 H⁺。）
- 对有序顺序机理，若把"决速步在最后一步"改为"第一步结合为决速步"，双倒数图族的截距/斜率行为会怎样变化？
- 本题判别方法与 37 决理-1 甲醛时钟的"准一级近似+图特征判机理"同构——试总结"从速率数据反推机理"的通用流程图。

---

## 易错分析

- 常见错误 1：电池表示式漏写双竖线（盐桥）或惰性电极 Pt，电极反应未配平 H₂O/H⁺（酸性水介质中氧化还原常漏水）。
- 常见错误 2：5-2 直接把 −0.21/−0.60 V 当化学标准态使用却不说明"总反应与 H⁺ 无关故 E°=E^⊕"——这一步论证是得分点。
- 常见错误 3：推导速率方程时用游离酶 [E] 而非总酶 [E]₀ 表达（官方明确不给分），或漏用 K_M1K_M12=K_M2K_M21 导致随机机理式子化简失败。
- 常见错误 4：5-6 只固定一个底物浓度作 4 条回归线（不足以判别机理，最多得 4 分）——判别必须两个方向都作图，证明截距均非常数。
