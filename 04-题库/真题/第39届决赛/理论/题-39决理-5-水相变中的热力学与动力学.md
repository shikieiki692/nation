...
若直接用298.15 K时的蒸发焓44.00 kJ mol⁻¹ 计算，
...
$\Delta G_{总}$ 的极大值点代表成核所需克服的最大能垒, 是判断晶核能否稳定生长的关键阈值, 其对应的液滴半径被称为临界成核半径 $r^{*}$ 。试推导临界成核半径 $r^{*}$ 的计算公式, 并计算298 K下水蒸气分压为7.92 kPa时的 $r^{*}$ (结果保留2位有效数字)。已知: 298.15 K时, 水的 $\sigma = 7.2 \times 10^{-2} \text{ N m}^{-1}$ , 密度为1.00 g cm⁻³ 。
...
> 这道题的表面是"雨极乞拉朋齐的降水物理"，本质是**一条从宏观到微观的热力学-动力学全链条**：基尔霍夫定律→沸点升高依数性→绝热抬升与露点判云→均相成核的 ΔG(r) 能垒→开尔文方程与扩散生长动力学——把"天上的云"还原成可推导的公式。

---

## 知识点映射

- **直接 KP**：[[相变热力学]]、[[溶液依数性]]、[[基尔霍夫定律]]
- **间接 KP**：[[绝热可逆过程]]、[[沸点]]、开尔文方程与成核理论
- **跨学科连接**：露点公式、绝热递减率与临界成核半径是气象学（云物理）与物理化学共用的一套语言——竞赛热力学可以直接解释"世界雨极"。

---

## 解题思路

1. **焓变先做温度校正**：Δ_vapH(373 K)=Δ_vapH(298 K)+ΔC_p·ΔT=40.87 kJ·mol⁻¹，再代 K_b,m=R(T_b*)²M_A/Δ_vapH 得 0.510 K·kg·mol⁻¹——直接用 298 K 焓值会得 0.474（官方只给一半分）。
2. **依数性套公式不忘 i**：1.24% NaCl → m_B=0.2148 mol·kg⁻¹，强电解质 i≈2 → ΔT_b=0.219 K。
3. **露点判云三步走**：体积分数→相对湿度 RH→代入露点公式与气温比较；海平面 18 ℃<25 ℃ 无云，抬升后 15 ℃>12.24 ℃ 成云，气温高于 0 ℃ 故降雨。
4. **绝热抬升联立两个微分方程**：dp=−(pMg/RT)dh 与绝热方程微分式联立 → ΔT=−12.76 K、T=285.39 K，再用 p^(1−γ)T^γ=常数得 p=0.859 bar；γ=C_p/(C_p−R)。
5. **成核能垒求极值**：ΔG_总=(4ρπr³RT/3M)ln(p_v,∞/p)+4πσr²，dΔG/dr=0 得 r*=2σM/(ρRT ln(p/p_v,∞))≈1.1 nm——热力学极值与扩散动力学（c_b−c_r=0）两条路径殊途同归；大液滴极限 r→∞ 时 dr/dt∝r⁻¹（负一级），积分得 r²=r₀²+kt。

---

## 变式拓展

- 把 5.3.2 的过饱和比 p/p_v,∞ 从 7.92/3.17 提高一倍，r* 变为多少？为什么过饱和度越高越容易成核？
- 5.3.5 的"负一级"生长在 t→∞ 时 r∝t^(1/2)——若改成表面反应控制（dr/dt=常数），r–t 关系与级数如何变？
- 本题与 35 决"制冷剂热力学"同属"相变热力学+实际过程"结构——总结"先校 C_p、再校焓熵、最后代公式"的温区换算流程。

---

## 易错分析

- 常见错误 1：5.1.1 忘用基尔霍夫定律把气化焓校正到沸点温度——298 K 数值代入得 0.474，官方明确只给一半分。
- 常见错误 2：5.1.2 漏掉强电解质 i≈2（NaCl→Na⁺+Cl⁻），结果差一半。
- 常见错误 3：5.2.2 绝热方程与静力学方程联立时 γ 定义写错（C_p/C_V vs C_p/(C_p−R)），或忘记用提示的微分公式而积分卡壳。
- 常见错误 4：5.3.1 体相项写成 ln(p/p_v,∞)（符号反）导致 ΔG_总 恒正、r* 无解；5.3.5 近似时忘记 exp(r*/r·ln…)→1 的合理性论证（r/r*≥1000）。
