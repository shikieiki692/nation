...
> 这道题的表面是"CO₂ 电解制 CO/甲酸两种工艺"，本质是**以 ΔG=−nFE 与 Nernst 方程为双主线、贯穿热力学（分解电压/温度系数/可逆反应热）与工程指标（法拉第效率/能量效率）的电化学定量题**——电势-pH 图把选择性判断变成几何问题。

---

## 知识点映射

- **直接 KP**：[[电化学]]、[[Nernst方程]]、[[电解池]]
- **间接 KP**：[[法拉第效率]]、[[电化学热力学]]（ΔG=−nFE）、van't Hoff 关系
- **跨学科连接**：CO₂ 电还原是碳中和技术的核心课题——电势-pH（Pourbaix）图是电催化、腐蚀科学与地球化学共用的语言。

---

## 解题思路

1. **分解电压从生成自由能算**：Δ_rG°=Δ_fG°(CO)−Δ_fG°(CO₂)=+257.2 kJ·mol⁻¹，E°=−ΔG/2F → 理论分解电压 1.333 V（注意取相反数）。
2. **温度外推用焓熵不变假设**：两个温度的 ΔG 线性相减得 Δ_rS°=+79.67 J·mol⁻¹·K⁻¹，可逆反应热 Q_rev=TΔS=7.97×10⁴ J·mol⁻¹——"分解电压随温度降低"正是熵项贡献。
3. **温度系数对 Nernst 项求导**：分压比与 T 无关 → (∂E/∂T)_p=ΔS°/2F−(R/2F)lnQ，代入非标分压得 −6.449×10⁻⁴ V·K⁻¹；封闭体系中 Q 随电解漂移 → 系数绝对值先减小后增大（选 E）。
4. **选择性用 φ-pH 直线竞争**：两条还原反应各写含 pH 的 Nernst 式（斜率差 2 倍是关键），令 φ₁<φ₂ 解得 pH>3.99；再与氢析出线 φ₃=−(RT ln10/F)pH 比较，pH>7.54 可同时避开 CO 与 H₂。
5. **工程指标按定义算**：法拉第效率 FE=2n_F/IΔt=83%；能量效率=产物实际储能 n·ΔG/实际电功 UIΔt=6.3%——甲酸电离（K_a）与浓度项都要进入 ΔG。

---

## 变式拓展

- 4.1.3 若在敞开恒压体系进行，温度系数绝对值的变化趋势与封闭体系有何不同？
- 把 4.2.2 的选择性判据推广到三产物（CO/HCOO⁻/H₂）共存体系，画出完整的 φ-pH 分区图并标出"甲酸选择窗口"。
- 本题与 37 决理-1 甲醛时钟同属"热力学判据＋动力学/工程修正"结构——总结电解水溶液中竞争反应的三层判断（热力学顺序→过电势→传质）。

---

## 易错分析

- 常见错误 1：4.1.1 分解电压忘取相反数（答 −1.333 V）或电子数按 1 而非 2 计。
- 常见错误 2：4.1.3 漏掉"分解电压=−E"的符号翻转，或 Nernst 项中 √p(O₂) 的 1/2 化学计量漏开方。
- 常见错误 3：4.2.2 两条 Nernst 式的 pH 斜率写成相同（反应 2 只含 1 个 H⁺ 却转移 2e⁻，斜率是反应 1 的一半）。
- 常见错误 4：4.3.2 能量效率分母用"法拉第效率×理论电压"混淆概念——必须是实际电功 W=UIt；甲酸产物浓度项与 K_a 也常被漏乘。
