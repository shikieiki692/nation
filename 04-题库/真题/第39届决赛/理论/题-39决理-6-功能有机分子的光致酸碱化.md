...
6.2 受 “光致酸化” 启发, 科学家进一步提出 “光致碱化” 体系, 利用结构如右图所示的芴基光敏分子 PBOH, 其水溶液在光照下可以释放出 HO⁻ , 同时形成 PB⁺ , pH 值在 7\~12 之间变化, 可实现 CO₂ 捕集和富集。
...
> 这道题的表面是"光控 pH 实现 CO₂ 富集"，本质是**以热力学循环（平衡常数串联）为前半场、以量子产率的光谱学测定与机理检验的动力学判据为后半场的光化学综合题**——从 K_a 与 K_0 的耦合，到比尔-朗伯定律定量光子账本，再到"机理预言与实验速率矛盾"的证伪逻辑。

---

## 知识点映射

- **直接 KP**：[[光致变色]]、[[光化学基础]]、[[酸碱平衡]]
- **间接 KP**：[[化学动力学]]、[[化学动力学深化]]、[[有机光化学]]
- **跨学科连接**：光致酸/碱化（photoacid）与 Ru(bpy)₃²⁺ 瞬态吸收光谱是人工光合作用与 CO₂ 捕集研究的真实工具——量子产率外标法是超快光谱实验室的标准操作。

---

## 解题思路

1. **平衡常数串联合并反应**：把"MC-H⁺ 去质子化 + HCO₃⁻/CO₂ 平衡 + MC↔SP 光平衡"写成一条总反应，[MC′]=[MC]+[SP]=[MC](1+K₀)，逐项代入 K_a 与 K_a1，得最佳条件 K_a(MC-H⁺)=K_a1(CO₂)/(1+K₀)。
2. **性能差异从"可逆性"找**：R₂=CN 使 SP 难以暗回复为 MC（选 B）——光稳态被"锁死"，无法循环而非不能光反应。
3. **共振式按正电荷落点选**：PB⁺ 最稳定共振式把正电荷放在苄位/芴环共轭最大处，写出骨架完整的共振极限。
4. **量子产率是"光子账本"**：吸收光子数 N_abs=N(1−10^−A)，生成物分子数由 ΔA=εcl 反推 → Φ=ΔA(λ₂)V/[ε(λ₂)lN(1−10^−A(λ₁))]；外标法用 Φ(Ru*)=1 消去 N，得 Φ(PB⁺)=ΔA₅₁₅·ε_Ru/(ΔA₄₄₈·ε_PB)=12%。
5. **机理检验用级数与速率对比**：无 CO₂ 时电荷守恒 [OH⁻]≈[PB⁺] → 二级速率 −k₂[PB⁺]²，积分得 1/[PB⁺]=1/[PB⁺]₀+k₂t；若机理为真，通 CO₂ 后 [OH⁻] 被压低几个数量级、速率应骤降——实验仅略降，机理不合理。

---

## 变式拓展

- 若把 6.1.1 的"最佳平衡常数=1"改为"光照/暗态循环中 CO₂ 吸收量最大"，K_a 的最优条件会向哪个方向移动？
- 6.3 外标法要求两溶液在 λ₁ 处吸光度相等——若不等，写出带 10^(−A) 修正的通用表达式。
- 本题"由速率-浓度关系证伪机理"与 37 决理-1 甲醛时钟"由 pH 曲线反推决速物种"同构——总结动力学判据的三类证据（级数、同位素效应、抑制剂）。

---

## 易错分析

- 常见错误 1：6.1.1 平衡常数合并时漏掉 [MC′]=[MC]+[SP] 的 (1+K₀) 因子，或把 K_a 与 K_a1 的分子分母位置颠倒。
- 常见错误 2：6.2.2 忘记"照射光子数 N 不等于吸收光子数"，直接用 N 作分母；或漏掉比尔-朗伯定律中 10^(−A) 的指数形式。
- 常见错误 3：6.3.1 忘用外标消去 N 与 V/l——把 ΔA 直接除以 ε 就当产率；6.3.2 激发波长选 448/515 nm（那是探测波长，且两点吸光度不等）。
- 常见错误 4：6.4.2 未用电荷守恒 [OH⁻]≈[PB⁺] 就写出二级速率式；6.4.3 只答"不合理"不给出"CO₂ 应使速率骤降而实验未骤降"的量化对比。
