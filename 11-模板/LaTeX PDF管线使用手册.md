...
MD 源文件                           ← 在 Obsidian 中编写
  ...
  │  \ce{...}                        ← 化学式
  │  *图 N 文字*                     ← 图注（编号连续）
  │  > [!tip] 理解要点 / > [!warning] 易错提醒   ← 教学 callout
  │  [[wikilink]]                    ← 内部链接
  ▼
...
写源稿时，先以 [[11-模板/MD通用规范（Word-PDF共同最优子集）]] 为准；需要看一整份讲义怎么组织，再参考 [[MD讲义格式参考（原子结构样板）]]。PDF 侧最常用的几条规则如下：
...
| 字母下标 | `$K_p$`、`$K_c$`、`$t_{2g}$`、`$e_g$` |
| 标准态符号 | `$\Delta_\mathrm{r} H^\theta$`、`$E^\theta$`，不用 `°` / `^\circ` |
| 章节标题 | `## 一、` / `### 1.1` / `#### ①` |
| callout | `> [!tip] 理解要点` / `> [!warning] 易错提醒` / `> [!example] 名师点拨` |
| 来源标注 | 不写 |
| `<center>` | 禁用 |
