...
\item
  能用盖斯定律、标准生成焓、键焓三种方法计算标准反应焓变 \(\Delta_{\1} \2^{\circ}\)
\item
...
\item
  能区分 ΔG 与 \(\Delta \1^{\circ}\) 的物理意义，会用 \(\Delta \1^{\circ}\) = −RT ln \(K^{\circ}\) 建立热力学与化学平衡的桥梁
\item
  知道 \(\Delta \1^{\circ}\) 与标准电动势 \(E^{\circ}\) 的关系(\(\Delta \1^{\circ}\) = −nFE°)，为电化学学习做铺垫
\end{itemize}
...
\tightlist
\item
  高炉炼铁过程中的主要反应 \(\mathrm{Fe_2O_3 + 3CO \to 2Fe + 3CO_2}\)，那能不能用类似方法炼铝？---热力学告诉我们：不能，Al\textsubscript{2}O\textsubscript{3} 的 ΔfH° 太负
\item
  NO 和 CO 都是汽车尾气中的有毒成分，它们能否相互反应生成无毒的 N\textsubscript{2} 和 CO\textsubscript{2}？---热力学告诉我们：能，\(\Delta \1^{\circ}\) \textless{} 0
\item
...
\textbf{本质}：焓 H 是状态函数 → ΔH 只取决于始态和终态。

\textbf{应用方法}：将目标反应拆分为若干已知 ΔH 的子反应，通过代数组合求算。

\textbf{例3(盖斯定律)}：已知 298 K 时：
- (1) C(s) + O\textsubscript{2}(g) = CO\textsubscript{2}(g)，ΔrH°\textsubscript{1} = −393.5 kJ/mol
- (2) H\textsubscript{2}(g) + ½O\textsubscript{2}(g) = H\textsubscript{2}O(l)，ΔrH°\textsubscript{2} = −285.8 kJ/mol
- (3) CH\textsubscript{4}(g) + 2O\textsubscript{2}(g) = CO\textsubscript{2}(g) + 2H\textsubscript{2}O(l)，ΔrH°\textsubscript{3} = −890.3 kJ/mol

求反应 C(s) + 2H\textsubscript{2}(g) = CH\textsubscript{4}(g) 的 ΔrH°。

解：目标反应 = (1) + 2×(2) − (3)

\[\Delta_r H^\circ = (-393.5) + 2 \times (-285.8) - (-890.3) = -74.8\ \mathrm{kJ/mol}\]

答案：\(\Delta_r H^\circ = -74.8\) kJ/mol(这正是 CH\textsubscript{4} 的标准生成焓)

\subsubsection{4.3 三种焓变计算方法}\label{ux4e09ux79cdux7113ux53d8ux8ba1ux7b97ux65b9ux6cd5}

\paragraph{(1) 标准生成焓 ΔfH° 法(最常用)}\label{ux6807ux51c6ux751fux6210ux7113-ux3b4fh-ux6cd5ux6700ux5e38ux7528}

在标准状态和指定温度(通常 298.15 K)下，由\textbf{稳定单质}生成 1 mol 化合物时的焓变。稳定单质的 ΔfH° = 0。

\[
\boxed{\Delta_r H^\circ = \sum n_i \Delta_f H^\circ(\text{生成物}) - \sum n_j \Delta_f H^\circ(\text{反应物})}
\]

注意：C 的稳定单质是\textbf{石墨}而非金刚石，\(\Delta_f H^\circ(\text{C, 金刚石}) = +1.9\) kJ/mol \(\neq\) 0。

\textbf{例4(生成焓法)}：计算反应 CH\textsubscript{4}(g) + Cl\textsubscript{2}(g) = CH\textsubscript{3}Cl(g) + HCl(g) 的 ΔrH°。

已知：\(\Delta_f H^\circ(\text{CH}_4) = -74.8\)，\(\Delta_f H^\circ(\text{CH}_3\text{Cl}) = -81.9\)，\(\Delta_f H^\circ(\text{HCl}) = -92.3\)，\(\Delta_f H^\circ(\text{Cl}_2) = 0\)(均为 kJ/mol)

\[\Delta_r H^\circ = [(-81.9) + (-92.3)] - [(-74.8) + 0] = -99.4\ \mathrm{kJ/mol}\]

\paragraph{(2) 标准燃烧焓 ΔcH° 法}\label{ux6807ux51c6ux71c3ux70e7ux7113-ux3b4ch-ux6cd5}

1 mol 物质完全燃烧的焓变。完全燃烧规定：C→CO\textsubscript{2}(g)，H→H\textsubscript{2}O(l)，N→N\textsubscript{2}(g)，S→SO\textsubscript{2}(g)，Cl→HCl(aq)。
...
\paragraph{(3) 键焓 ΔbH° 法(估算用)}\label{ux952eux7113-ux3b4bh-ux6cd5ux4f30ux7b97ux7528}

1 mol 气态物质断开化学键成为气态原子时的焓变。ΔrH° = Σ(反应物断键) − Σ(生成物成键)。
...
可以拆解为五个步骤：
1. Na(s) → Na(g) \textbf{升华热} \(\Delta_{\text{sub}}H\) = +107 kJ/mol
2. Na(g) → Na\textsuperscript{+}(g) + e\textsuperscript{-} \textbf{第一电离能} \(I_1\) = +496 kJ/mol
3. ½Cl\textsubscript{2}(g) → Cl(g) \textbf{解离能} \(\frac{1}{2}D\) = +122 kJ/mol
4. Cl(g) + e\textsuperscript{-} → Cl\textsuperscript{-}(g) \textbf{电子亲和能} \(E_{\text{ea}}\) = −349 kJ/mol
5. Na\textsuperscript{+}(g) + Cl\textsuperscript{-}(g) → NaCl(s) \textbf{晶格能} \(U\)

由盖斯定律：\(\Delta_f H^\circ = \Delta_{\text{sub}}H + I_1 + \frac{1}{2}D + E_{\text{ea}} + U\)
...
\textbf{热力学第三定律}：在 0 K 时，任何\textbf{完美晶体}的熵为零。由此可以计算物质在各温度下的\textbf{标准摩尔熵} \(S^\circ\)(规定熵)。
...
2. 焓增熵减 & + & − & 非自发 & 非自发 & 无 & 2CO → C + O\textsubscript{2}(极罕见) \\
3. 焓增熵增 & + & + & 非自发 & 自发 & \(T_{\text{转}} = \Delta H/\Delta S\) & CaCO\textsubscript{3} 分解 \\
4. 焓降熵减 & − & − & 自发 & 非自发 & \(T_{\text{转}} = \Delta H/\Delta S\) & NH\textsubscript{3}(g) + HCl(g) → NH\textsubscript{4}Cl(s) \\
\end{longtable}
}

\textbf{理解要点}：ΔG \textless{} 0 不意味着反应一定快---热力学只讨论''能不能''，不讨论''有多快''。例如 H\textsubscript{2} 与 O\textsubscript{2} 在室温下 ΔG 极负，但没有催化剂时反应极慢。

\subsubsection{\texorpdfstring{6.3 \(\Delta \1^{\circ}\) 与化学平衡常数}{6.3 \textbackslash Delta \textbackslash1\^{}\{\textbackslash circ\} 与化学平衡常数}}\label{delta-1circ-ux4e0eux5316ux5b66ux5e73ux8861ux5e38ux6570}

\[
\boxed{\Delta_r G^\circ = -RT \ln K^\theta}
\]

其中 \(K^{\circ}\) 为标准平衡常数(无量纲)。这就是连接热力学与化学平衡的\textbf{桥梁公式}。
...
\bottomrule\noalign{}
\endlastfoot
\(\Delta \1^{\circ}\) ≪ 0(如 \textless{} −40 kJ) & \(K^{\circ}\) ≫ 1 & 反应正向进行彻底 \\
\(\Delta \1^{\circ}\) ≈ 0 & \(K^{\circ}\) ≈ 1 & 反应物与产物共存 \\
\(\Delta \1^{\circ}\) ≫ 0(如 \textgreater{} +40 kJ) & \(K^{\circ}\) ≪ 1 & 反应几乎不发生 \\
\end{longtable}
}

\textbf{ΔG 与 \(\Delta \1^{\circ}\) 的关系}(非标准状态下)：
...
\textbf{核心辨析：\(\Delta \1^{\circ}\) \(\neq\) ΔG}
- \(\Delta \1^{\circ}\) 是标准状态下的值(所有气体 p° = 100 kPa，溶液 c° = 1 mol/L)，是\textbf{定值}
- ΔG 是任意状态下的值，\textbf{随浓度/分压变化}
- \(\Delta \1^{\circ}\) \textgreater{} 0 不代表反应在任何条件下都不能发生---调整 Q 可能使 ΔG \textless{} 0

\subsubsection{\texorpdfstring{6.4 \(\Delta \1^{\circ}\) 与标准电动势 \(E^{\circ}\)}{6.4 \textbackslash Delta \textbackslash1\^{}\{\textbackslash circ\} 与标准电动势 E\^{}\{\textbackslash circ\}}}\label{delta-1circ-ux4e0eux6807ux51c6ux7535ux52a8ux52bf-ecirc}
...
\textbf{单位陷阱预警}：R = 8.314 J/(mol·K)，而 \(\Delta \1^{\circ}\) 通常以 kJ/mol 为单位。计算时必须先统一单位！单位错误是热力学计算失败的第一大原因。
...
\begin{longtable}[]{@{}lcc@{}}
\toprule\noalign{}
物质 & ΔfH° (kJ/mol) & S° (J·mol\textsuperscript{-1}·K\textsuperscript{-1}) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\tightlist
\item
  计算 298 K 时的 \(\Delta_{\1} \2^{\circ}\)、\(\Delta_{\1} \2^{\circ}\)、\(\Delta_{\1} \2^{\circ}\)
\item
  估算标准压力下自发进行的最低温度
\end{enumerate}
...
\tightlist
\item
  \(\Delta_r H^\circ = [(-635.1) + (-393.5)] - (-1206.9) = +178.3\) kJ/mol
\end{enumerate}

\(\Delta_r S^\circ = (39.7 + 213.8) - 92.9 = +160.6\) J/(mol·K) = +0.1606 kJ/(mol·K)

\(\Delta_r G^\circ = 178.3 - 298 \times 0.1606 = +130.4\) kJ/mol \textgreater{} 0

298 K 时 \(\Delta \1^{\circ}\) \textgreater{} 0，反应非自发。
...
\tightlist
\item
  令 \(\Delta \1^{\circ}\) = 0：\(T_{\text{转}} = \frac{\Delta H^\circ}{\Delta S^\circ} = \frac{178.3}{0.1606} \approx 1110\) K ≈ 837°C
\end{enumerate}

答案：(1) \(\Delta_{\1} \2^{\circ}\) = +178.3 kJ/mol，\(\Delta_{\1} \2^{\circ}\) = +160.6 J/(mol·K)，\(\Delta_{\1} \2^{\circ}\) = +130.4 kJ/mol；(2) 约 1110 K

\textbf{题干}：298 K 时 \(\mathrm{N_2O_4(g) \rightleftharpoons 2NO_2(g)}\) 的 \(\Delta_r G^\circ = +4.73\) kJ/mol。求 \(K^{\circ}\)。

\textbf{解析}：

\[\ln K^\theta = \frac{-\Delta_r G^\circ}{RT} = \frac{-4.73 \times 10^3}{8.314 \times 298} = -1.91\]

\[K^\theta = e^{-1.91} = 0.148\]

答案：\(K^{\circ}\) = 0.148(\textless{} 1，说明 298 K 时 N\textsubscript{2}O\textsubscript{4} 更稳定)

\textbf{题干}：计算 CH\textsubscript{4}(g) + Cl\textsubscript{2}(g) = CH\textsubscript{3}Cl(g) + HCl(g) 的 \(\Delta_{\1} \2^{\circ}\)。
...
\def\labelenumi{(\arabic{enumi})}
\item
  生成焓法：\(\Delta_r H^\circ = -99.4\) kJ/mol(精确值)
\item
  键焓法(断键 − 成键)：断键 = 413 + 243 = 656 kJ；成键 = 339 + 431 = 770 kJ
\end{enumerate}

\[\Delta_r H^\circ = 656 - 770 = -114\ \mathrm{kJ/mol}\]
...
\begin{verbatim}
    ...
    ΔG = ΔH − TΔS
       ↙         ↘
  $\Delta \1^{\circ}$ = −nFE°    $\Delta \1^{\circ}$ = −RT ln $K^{\circ}$
\end{verbatim}
...
生成焓法 & \(\Delta_r H^\circ = \sum n_i \Delta_f H^\circ(\text{生}) - \sum\) & 稳定单质 ΔfH° = 0 \\
燃烧焓法 & \(\Delta_r H^\circ = \sum(\text{反应物}) - \sum(\text{生成物})\) & 方向与生成焓法相反 \\
键焓法 & \(\Delta_r H^\circ = \sum(\text{断键}) - \sum(\text{成键})\) & 估算值 \\
Born-Haber 循环 & 盖斯定律在离子晶体中的应用 & 可间接求晶格能 \\
熵 S & 混乱度的量度；\(S = k_B \ln \Omega\) & 稳定单质 S° \(\neq\) 0 \\
Gibbs 自由能 & \(G = H - TS\) & 恒温恒压自发性判据 \\
自发性 & \(\Delta G = \Delta H - T\Delta S\) & ΔG \textless{} 0 自发 \\
\(\Delta \1^{\circ}\)---\(K^{\circ}\) & \$\Delta\_G° = -RT\ln \(K^{\circ}\)\$ & \(K^{\circ}\) 无量纲；单位统一 \\
\(\Delta \1^{\circ}\)---\(E^{\circ}\) & \(\Delta_G° = -nFE°_{\text{cell}}\) & 连接热力学与电化学 \\
\end{longtable}
}
...
(a) NH\textsubscript{4}Cl(s) → NH\textsubscript{3}(g) + HCl(g)
(b) N\textsubscript{2}(g) + 3H\textsubscript{2}(g) → 2NH\textsubscript{3}(g)
(c) H\textsubscript{2}O(l) → H\textsubscript{2}O(g)
(d) Ag\textsuperscript{+}(aq) + Cl\textsuperscript{-}(aq) → AgCl(s)

\textbf{4.} 已知 298 K 时 ΔfH° 数据：HCl(g) = −92.3 kJ/mol，H\textsubscript{2}(g) = 0，Cl\textsubscript{2}(g) = 0。计算反应 H\textsubscript{2}(g) + Cl\textsubscript{2}(g) = 2HCl(g) 的 ΔrH°。
...
\textbf{8.} 已知某反应的 ΔrG° = −30.0 kJ/mol(298 K)，计算 \(K^{\circ}\)，并说明反应正向进行的程度。

\textbf{9.} 焓变计算：已知 ΔfH°(H\textsubscript{2}O, l) = −285.8 kJ/mol，ΔfH°(H\textsubscript{2}O, g) = −241.8 kJ/mol。计算 H\textsubscript{2}O(l) → H\textsubscript{2}O(g) 的蒸发焓，并说明其符号的物理意义。
...
求 C\textsubscript{2}H\textsubscript{5}OH(l) 的标准生成焓 ΔfH°。

\textbf{12.} \textbf{(ΔH 与 ΔU 的换算)} 反应 N\textsubscript{2}(g) + 3H\textsubscript{2}(g) = 2NH\textsubscript{3}(g) 在 298 K 时 ΔrH° = −92.2 kJ/mol。计算 ΔrU°，并解释两者差异的物理原因。

\textbf{13.} \textbf{(\(\Delta \1^{\circ}\) 与 \(K^{\circ}\) 的深度计算)} 已知 298 K 时：
- ½N\textsubscript{2}(g) + ½O\textsubscript{2}(g) = NO(g)，ΔrG° = +86.6 kJ/mol
- 2NO(g) + O\textsubscript{2}(g) = 2NO\textsubscript{2}(g)，ΔrG° = −71.2 kJ/mol

求反应 N\textsubscript{2}(g) + 2O\textsubscript{2}(g) = 2NO\textsubscript{2}(g) 的 ΔrG° 和 \(K^{\circ}\)。

\textbf{14.} \textbf{(键焓法的局限性)} 用键焓法计算 NH\textsubscript{3}(g) → NH\textsubscript{2}·(g) + H·(g) 所需能量(即 N−H 键的键焓)，并与文献值 389 kJ/mol 比较。已知 NH\textsubscript{3} 中三个 N−H 键的键焓不完全相同，第一、二、三步解离能分别为 435、397、339 kJ/mol。讨论''平均键焓''与''逐级解离能''的区别。
...
\endlastfoot
K(s) → K(g) & +89 \\
K(g) → K\textsuperscript{+}(g) + e\textsuperscript{-} & +419 \\
½Cl\textsubscript{2}(g) → Cl(g) & +122 \\
Cl(g) + e\textsuperscript{-} → Cl\textsuperscript{-}(g) & −349 \\
K(s) + ½Cl\textsubscript{2}(g) → KCl(s) & −437 \\
\end{longtable}
}

\textbf{16.} \textbf{(转变温度的实际意义)} 已知：
- CaCO\textsubscript{3}(s) = CaO(s) + CO\textsubscript{2}(g)，\(\Delta \1^{\circ}\) = +178 kJ/mol，\(\Delta \1^{\circ}\) = +161 J/(mol·K)
- MgCO\textsubscript{3}(s) = MgO(s) + CO\textsubscript{2}(g)，\(\Delta \1^{\circ}\) = +101 kJ/mol，\(\Delta \1^{\circ}\) = +175 J/(mol·K)

分别估算两者的分解温度，并解释为什么 MgCO\textsubscript{3} 比 CaCO\textsubscript{3} 更容易分解。

\textbf{17.} \textbf{(ΔG 的非标准状态计算)} 在 298 K，某反应的 ΔrG° = +20 kJ/mol。若反应物浓度均为 0.01 mol/L，产物浓度均为 1.0 mol/L，判断反应进行的方向。

\textbf{18.} \textbf{(量热实验设计)} 某同学想用保温杯式量热计测定锌与盐酸反应的焓变。已知：100 mL 1.0 mol/L HCl 加入过量锌粉，温度从 25.0°C 升至 38.2°C。溶液密度 ≈ 1.0 g/mL，比热容 ≈ 4.18 J/(g·K)，量热计常数 C = 50 J/K。
(a) 计算反应放出的热量 Q。
(b) 写出热化学方程式并计算 ΔrH°(以 HCl 为基准)。
(c) 与文献值 ΔfH°(ZnCl\textsubscript{2}, aq) = −485 kJ/mol 比较，讨论可能的误差来源。
...
\endlastfoot
Na(s) → Na(g) & +107 \\
Na(g) → Na\textsuperscript{+}(g) + e\textsuperscript{-} & +496 \\
½Br\textsubscript{2}(l) → Br(g) & +112 \\
Br(g) + e\textsuperscript{-} → Br\textsuperscript{-}(g) & −325 \\
Na(s) + ½Br\textsubscript{2}(l) → NaBr(s) & −361 \\
\end{longtable}
}
...
\tightlist
\item
  计算反应 Fe\textsubscript{2}O\textsubscript{3}(s) + 2Al(s) = Al\textsubscript{2}O\textsubscript{3}(s) + 2Fe(s) 的 ΔrH°、ΔrS° 和 298 K 时的 ΔrG°。
\item
  这就是铝热反应。解释为什么铝热反应一旦引发就极其剧烈(从热力学角度)。
\item
  从 ΔrG° 估算该反应在标准条件下的平衡常数 \(K^{\circ}\)，说明反应正向进行的程度。
\end{enumerate}
...
\textbf{22.} \textbf{(全国初赛)} 某反应在 298 K 时 ΔrH° = −120 kJ/mol，ΔrS° = −150 J/(mol·K)。
(a) 判断 298 K 时反应是否自发。
(b) 估算反应自发进行的温度上限。
...
\tightlist
\item
  计算 2H\textsubscript{2}O\textsubscript{2}(l) = 2H\textsubscript{2}O(l) + O\textsubscript{2}(g) 的 ΔrH°、ΔrS°、ΔrG°。
\item
  计算 298 K 时的 \(K^{\circ}\)。
\item
...
\textbf{24.} \textbf{(电化学桥接题)} 298 K 时，Zn \textbar{} Zn\textsuperscript{2+}(1 mol/L) \textbar\textbar{} Cu\textsuperscript{2+}(1 mol/L) \textbar{} Cu 电池的标准电动势 \(E^{\circ}\) = 1.10 V。
(a) 用 ΔrG° = −nFE° 计算该电池反应的 ΔrG°。
(b) 用 ΔrG° = −RT ln \(K^{\circ}\) 计算反应 Zn(s) + Cu\textsuperscript{2+}(aq) = Zn\textsuperscript{2+}(aq) + Cu(s) 的 \(K^{\circ}\)。
(c) 解释为什么该电池反应的 \(K^{\circ}\) 如此之大(\textgreater{} 10\textsuperscript{37})。

\textbf{25.} \textbf{(综合推断)} 某化合物 X 在加热时发生分解反应，产生两种气体。已知：
- 分解温度约为 450 K(此时 \(K^{\circ}\) ≈ 1)
- 分解反应是吸热反应
- 产物气体分子数多于反应物
...
\textbf{1.}
(a) 错误。放热(ΔH \textless{} 0) \(\neq\) 自发。自发性由 ΔG = ΔH − TΔS 决定。若 ΔS \textless{} 0 且温度足够高，ΔG \textgreater{} 0 可使放热反应非自发。
(b) 错误。稳定单质的 ΔfH° = 0，但 S° \(\neq\) 0。只有在 0 K 时完美晶体的熵才为零。
(c) 正确。这正是状态函数的定义特征。
(d) 错误。吸热 Q \textgreater{} 0 时，ΔH = Qp \textgreater{} 0 仅在恒压条件下成立。且 ΔH 与 Q 的取号规则不同。

\textbf{2.}
(a) H\textsubscript{2}(g) + ½O\textsubscript{2}(g) = H\textsubscript{2}O(g)，ΔrH° = −241.8 kJ/mol
(b) C(s,石墨) + O\textsubscript{2}(g) = CO\textsubscript{2}(g)，ΔrH° = −393.5 kJ/mol

\textbf{3.}
(a) ΔS \textgreater{} 0(固体→气体，混乱度大增)
(b) ΔS \textless{} 0(4 mol 气体→2 mol 气体，分子数减少)
(c) ΔS \textgreater{} 0(液体→气体)
(d) ΔS \textless{} 0(离子→固体沉淀，有序度增加)

\textbf{4.} ΔrH° = 2 × (−92.3) − {[}0 + 0{]} = −184.6 kJ/mol
...
\textbf{7.} Born-Haber 循环将离子晶体的生成过程拆解为升华→电离→解离→电子亲和→晶格形成等步骤。由盖斯定律，各步骤焓变之和等于生成焓。特别适用于间接求晶格能。

\textbf{8.} \(K^{\circ}\) = e\^{}(30000/(8.314×298)) = e\^{}12.1 ≈ 1.8 × 10\textsuperscript{5}。\(K^{\circ}\) 远大于 1，反应正向进行很彻底。

\textbf{9.} ΔrH° = −241.8 − (−285.8) = +44.0 kJ/mol。正值表示水的蒸发是吸热过程。

\textbf{10.}
(a) S°(NaCl, aq) \textgreater{} S°(NaCl, s)：溶解后离子分散到溶液中，混乱度增大。
(b) S°(石墨) \textgreater{} S°(金刚石)：石墨为层状结构，层间有自由电子运动，微观状态数更多。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{11.} 目标反应 = 2×(1) + 3×(2) − (3)
ΔfH° = 2×(−393.5) + 3×(−285.8) − (−1367) = −277.4 kJ/mol

\textbf{12.} Δn = 2 − 4 = −2，ΔrU° = ΔrH° − ΔnRT = −92.2 − (−2) × 8.314 × 10\textsuperscript{-3} × 298 = −87.2 kJ/mol。物理原因：反应使气体分子数减少，环境对体系做功。

\textbf{13.} ΔrG° = 2×(+86.6) + (−71.2) = +102.0 kJ/mol；\(K^{\circ}\) = e\^{}(-102000/2478) ≈ 1.2 × 10\textsuperscript{-18}。\(K^{\circ}\) 极小，N\textsubscript{2} 与 O\textsubscript{2} 直接生成 NO\textsubscript{2} 几乎不可能。
...
\textbf{16.} CaCO\textsubscript{3}：T\_转 ≈ 1106 K；MgCO\textsubscript{3}：T\_转 ≈ 577 K。MgCO\textsubscript{3} 更易分解，因为 MgO 晶格能更大(Mg\textsuperscript{2+} 半径更小)。
...
\textbf{18.}
(a) Q = (4.18×100×1.0+50)×(38.2−25.0) = 468×13.2 = 6178 J ≈ 6.18 kJ
(b) n(HCl) = 0.100 mol，ΔrH° = −6.18/0.100 = −61.8 kJ/mol
...
\textbf{20.}
(a) ΔrH° = −851.5 kJ/mol，ΔrS° = −31.9 J/(mol·K)，ΔrG° = −842.0 kJ/mol
(b) ΔrG° 极负，产物热力学稳定性远高于反应物。一旦引发，大量焓变以热能释放，温度急剧升高---典型的''热爆炸''式自催化过程。
(c) \(K^{\circ}\) ≈ 10\^{}147，反应正向极其彻底。

\textbf{21.}
(a) ΔrG° = +2.89 kJ/mol \textgreater{} 0
(b) ΔrG° \textgreater{} 0，石墨→金刚石非自发。石墨是热力学稳定相。
(c) 高压有利于向密度更大的相转化((∂G/∂p)\_T = V)。金刚石密度 \textgreater{} 石墨，施压有利于石墨→金刚石。

\textbf{22.}
(a) \(\Delta \1^{\circ}\) = −120 + 44.7 = −75.3 kJ/mol \textless{} 0，自发。
(b) T\_转 = 800 K。
(c) 降低温度；改变浓度/分压使 Q \textless{} K；耦合一个 ΔG 更负的反应。

\textbf{23.}
(a) ΔrH° = −196.0 kJ/mol，ΔrS° = +125.7 J/(mol·K)，ΔrG° = −233.5 kJ/mol
(b) \(K^{\circ}\) ≈ 10\^{}41
(c) 热力学上 \(K^{\circ}\) 极大。H\textsubscript{2}O\textsubscript{2} 溶液中微量杂质催化分解；纯 H\textsubscript{2}O\textsubscript{2} 缺乏催化活性位点。
(d) 分解反应活化能较高(O−O 键断裂需 \textasciitilde210 kJ/mol)，室温下分子动能不足以大量跨越能垒。

\textbf{24.}
(a) ΔrG° = −2 × 96485 × 1.10 = −212.3 kJ/mol
(b) \(K^{\circ}\) = e\^{}(212300/2478) ≈ 10\^{}37
...
(b) 450 K 时 \(K^{\circ}\) ≈ 1 即 \(\Delta \1^{\circ}\) ≈ 0。T \textless{} 450 K 时 T\(\Delta \1^{\circ}\) \textless{} \(\Delta \1^{\circ}\)，\(\Delta \1^{\circ}\) \textgreater{} 0；T \textgreater{} 450 K 时 \(\Delta \1^{\circ}\) \textless{} 0。
