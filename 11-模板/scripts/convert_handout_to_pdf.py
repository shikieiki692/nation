...
def preprint(source_md: str) -> str:
    ...
    # ── 1b. 转义正文里的 ASCII `~`（区间记号）——必须在第 355 行之前 ──────────
    # 🔴 2026-09-21 修（格式线）：本文件第 355-358 行**刻意**把 Unicode 上下标
    #    `₂`/`⁺` 转成 pandoc 语法 `~2~`/`^+^`，依赖 reader 的 `subscript`/`superscript`
    #    扩展。但 pandoc 这两个扩展要求定界符之间**不含空格**，而中文行内标点前后无空格
    #    ⇒ 源文里的**区间记号**同样会被吃掉，且是**静默改字**：
    #        `300~500°C（573~773 K）` → `300\textsubscript{500°C（573}773 K）`
    #        `400~430</td><td>430~470` → `400\textsubscript{430</td><td>430}470`
    #    实测受害面：非源层 220 处 / 135 份 md（其中学生讲义 16 份 = 本线输入）。
    #    修法：把源里的区间 `~` 转义成 `\~`（pandoc → `\textasciitilde`，仍是可见的 `~`），
    #    而本文件随后生成的上下标 `~2~` **不受影响**（它们在这一步之后才产生）。
    #    断言用「前后均不邻 `~`」，故 `~~删除线~~` 与 `~~~` 围栏**原样保留**（不去学它们）。
    #    另需跳过**行内代码**（`` `10~40 kJ/mol` ``）：在 `\texttt{}` 里 `\~` 会渲染成
    #    字面 `\~`（`\textbackslash{}\textasciitilde{}`），属自造污染。
    #    故用**单次原子替换**：同一正则里既匹配代码段（原样放回）也匹配裸 `~`（转义）。
    #    围栏代码块已被上面的 `_save_code` 摘走，此处无需再管。
    text = re.sub(r"`[^`\n]*`|(?<!~)~(?!~)",
                  lambda m: m.group(0) if m.group(0).startswith("`") else r"\~", text)
