...
\title{C:\Obsidion\妙妙屋\04-课件\学生讲义\溶液与相图}
\date{}
...
\textbf{上节衔接}：\左箭 热力学初步-超级充实版(自学完整)\textbar 热力学初步(Gibbs 自由能与相变)
\textbf{下节衔接}：\箭 2026-06-23-化学平衡\textbar 化学平衡(Q/K 分析方法)
...
\textbf{推导链}：\$\Delta p \propto x\_B \propto b \rightarrow \Delta T\_b \propto \Delta p \propto b \rightarrow \Delta T\_b = \(K_{\1}\) \cdot b\$
...
\textbf{融雪原理}：往雪上撒盐(NaCl)，盐溶于雪水形成溶液，凝固点降低(\$\Delta T\_f = \(K_{\1}\) \cdot b\$)，使雪在 0°C 以下也能融化。但融雪后路面温度会更低---``盐不是热的，雪融化是因为凝固点降低了''。
...
沸点升高 & \$\Delta T\_b = \(K_{\1}\) \cdot b\$ & 必须用 b(mol/kg)，不是 c \\
凝固点降低 & \$\Delta T\_f = \(K_{\1}\) \cdot b\$ & 优先选用 \(K_f\) 大的溶剂 \\
...
\textbf{8.} 渗透压法对大分子最有效：\(\Pi = cRT\)，即使 c 很小，\(\Pi\) 仍然可测。而 \$\Delta T\_b = \(K_{\1}\) \cdot b\$ 对大分子太小，难以精确测量。
...
\textbf{12.} \$\Delta T\_b = \(K_{\1}\) \cdot b = \(K_{\1}\) \cdot (0.650/M) / 0.0250 = 0.48\$
...
\textbf{23.} 非电解质理论 \$\Delta T\_f = \(K_{\1}\) \times b = 1.86 \times (1.00/300/0.100) = 0.062,\^{}\circ\mathrm{C}\$
