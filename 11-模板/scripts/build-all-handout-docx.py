...
# Pandoc 转换扩展
# ⚠️ `-superscript` 不是可有可无的：pandoc 的 markdown 默认**开启** `superscript`
#    （`^…^` → 上标），而本库大量正文写着「漏了 `$` 的 LaTeX 碎片」（`Mn^{2+}`、`MnO4^-`）。
#    当正文里出现 `^{…}`、而紧随其后的 `$…$` 数学域里也有 `^` 时，两个 `^` 会被
#    **跨 `$` 配成一对上标** ⇒ `$` 配对整体位移 ⇒ 数学域不再被识别 ⇒ Word 里出字面 `$`。
#    实测（2026-09-21）：`03-知识点/分析化学/氧化还原滴定.md` 即此因。
#    全库「数学域外的 `^`」共 1111 处，**无一处是有意的 `^词^` 上标**（逐条抽样核过），
#    故禁用该扩展既有根治效果、又无内容损失。
#
# ⚠️ 同理禁用 `-subscript`（`~…~` → 下标）：本库用 `~` 表**区间/约数**（`70~80℃`），
#    pandoc 会把两个短区间配成下标并**吞掉 `~`** ⇒ 静默内容损坏。
#    实测（2026-09-21）：「条件：70~80℃，0.5~1 M H+ 介质。」
#    产物文本竟变成「条件：7080℃，0.51 M H+ 介质。」（`~` 消失、"80℃，0.5" 变下标）。
#    全库数学域外 `~` 8100 处 / 1193 份；而**有意的 `~下标~` 写法 0 处**（按
#    `[字母]~短内容~` 全库扫描，0 命中）⇒ 禁用无内容损失。
#    影响是**单调**的：只可能把「被吞的下标文本」还原为字面 `~`，不可能新增 `$` 失效。
