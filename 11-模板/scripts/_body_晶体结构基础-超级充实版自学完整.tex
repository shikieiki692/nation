...
\begin{quote}
{[}认知冲突{]} \textbf{认知对比}：「晶体 vs 非晶体」---晶体像图书馆里的书(每本书在固定的书架上、固定的位置，取走一本，另一本也是同样的方向放在那里)，非晶体像垃圾场里的杂物(随意堆叠，没有长程周期)。普通玻璃是典型的非晶体---它没有固定的熔点，只是逐渐软化。
\end{quote}
...
\begin{quote}
{[}易错点{]} \textbf{易错点}：结构基元 ≠ 晶胞 ≠ 一个原子。结构基元是''内容''(含什么+怎么排)，点阵是''框架''(几何位置关系)。干冰的结构基元含4个CO\textsubscript{2}分子---因为4个分子的取向不同，取其中1个平移无法还原整个晶体。
\end{quote}
...
根据晶胞的\textbf{轴长 a、b、c} 和\textbf{轴角 \(\alpha\)、\(\beta\)、\(\gamma\)}，晶体可分为7大晶系。但注意：\textbf{晶系由特征对称元素决定，不是由晶胞参数数值决定的}。

\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「为什么晶体没有5次对称轴？」---数学证明表明，周期排列只能允许1/2/3/4/6次旋转轴。五边形不能无空隙地铺满平面→晶体中不可能出现5次对称。这就是''轴次定理''。
\end{quote}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-3a-bravais-lattice-primitive-body-face-centered.jpg}
\caption{图3 布拉维格子示意图 — 从左至右：简单(P) 体心(I) 面心(F) 底心(C)}
\end{figure}

\begin{quote}
{[}教师原话{]} \textbf{课堂原话}：``14种布拉维格子就像26个字母---用它们可以拼出所有晶体结构。第一轮不用全背，但要理解它的逻辑：晶系(7种)× 带心方式(最多4种)= 所有可能的几何组合。''
\end{quote}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/金属晶体A1A2A3对比.并列对比.png}
\caption{图12 金属晶体A1/A2/A3并列对比 — 三种基本堆积方式(fcc/bcc/hcp)的参数与实例一览}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/close-packed-layer.jpg}
\caption{图5 密置层 — 等径圆球最紧密排列，每个球周围有6个球紧密接触。三个球之间的凹陷处形成两种三角形空隙(顶点向上/向下)}
\end{figure}

\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「为什么ccp和hcp的堆积系数都是74.05\%，但晶系不同？」---两者的差异不在堆积密度，而在\textbf{堆积层序}：ccp是ABCABC\ldots(三层一循环，产生立方对称性)，hcp是ABAB\ldots(两层一循环，六方对称性)。\textbf{同样的密度，不同的排列方式}。
\end{quote}

\begin{quote}
{[}教师原话{]} \textbf{课堂原话}：``CCP=ABC, HCP=AB, 密度一样高，堆法各不同。第三层去了新位置是CCP(立方)，回到原位是HCP(六方)。''
\end{quote}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/密堆积-二维到三维.递进图.png}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-7-10-four-unit-cell-types-sc-bcc-fcc.jpg}
\caption{图6 四种晶胞对比 — 简单立方(左上)、体心立方(右上)、面心立方(右下)及原子接触方向}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-7-10-unit-cell-types-sc-bcc-fcc-comparison.jpg}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/空隙-四面体八面体.标注图.png}
\caption{图14 四面体空隙与八面体空隙的位置标注 — 空隙类型对比、计数公式与fcc应用实例}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-8a-close-packed-bilayer-tetrahedral-octahedral-voids.jpg}
\caption{图9 密置双层中的空隙 — 从双层俯视可见四面体空隙(T)和八面体空隙(O)的位置分布}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-8b-tetrahedral-void-structure.jpg}
\caption{图10 正四面体空隙(左)与正八面体空隙(右)— 四面体空隙由4个等径球围成，八面体空隙由6个等径球围成}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/ccp-stacking.jpg}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-11-three-ab-ionic-crystal-structures-nacl-cscl-zns.jpg}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/晶体结构-NaCl-CsCl-ZnS对比.关系图.png}
\caption{图12 三种AB型离子晶体结构对比 — 从左至右：NaCl型(6:6)、CsCl型(8:8)、ZnS型(4:4)}
\end{figure}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/nacl_crystal_structure.jpg}
\caption{图13 NaCl型结构 — 负离子面心立方密堆积，正离子填全部八面体空隙。注意Na和Cl各构成一套面心立方格子，两套互相穿插}
\end{figure}

\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「NaCl型结构 ≠ 面心立方格子！」---NaCl是两套面心立方格子(Na\textsuperscript{+}一套、Cl\textsuperscript{-}一套)互相穿插。面心立方格子要求八个顶点和六个面心是同种原子，不是这样交替的。
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/cscl_crystal_structure.jpg}
\caption{图14 CsCl型结构 — 负离子简单立方堆积，正离子填充立方体空隙}
\end{figure}

\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「CsCl是不是体心立方结构？」(发生率\textasciitilde50\%)---不是！CsCl是\textbf{简单立方}(两套简单立方格子互相穿插)。体心立方的顶点和体心是同种原子(如\(\alpha\)-Fe)，但CsCl的顶点是Cl\textsuperscript{-}、体心是Cs\textsuperscript{+}。\textbf{同种离子各自单看}，都是简单立方格子。
\end{quote}

\begin{quote}
{[}易错点{]} \textbf{易错点}：CsCl型的点阵类型是\textbf{简单立方P}(不是体心立方I)！因为顶点和体心是不同种类的原子。
\end{quote}
...
\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/zns_crystal_structure.jpg}
\caption{图15 立方ZnS型(闪锌矿)结构 — S面心立方密堆积，Zn填一半四面体空隙。注意Zn之间的位置关系—它们互相错开¼对角线}
\end{figure}
...
\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「半径比规则是几何必要条件，不是充分条件」---rc/ra 在范围内只说明几何上允许这个配位数，但实际结构还受离子极化、共价成分、温度压力等因素影响。\textbf{AgI 的 rc/ra ≈ 0.51(NaCl型范围)，但实际是ZnS型(4配位)}---因为Ag-I键有明显共价成分。
\end{quote}

\begin{quote}
{[}易错点{]} \textbf{易错点}：半径比规则给出的是\textbf{最小}半径比。若正离子半径比该临界值更小，正、负离子不能接触，结构不稳定。但半径比规则并非绝对---当离子极化显著时(共价成分增多)，该规则的适用性下降。
\end{quote}
...
\begin{quote}
{[}教师原话{]} \textbf{课堂原话}：``Pauling电价规则本质上是一本\textbf{能量账本}---每个正离子把自己'电力'平均分给所有邻居，所有邻居加起来必须刚好还完负离子的'债'。''
\end{quote}
...
- 6 个 Na\textsuperscript{+} 总贡献 = \(6 \times 1/6 = 1\) = Cl\textsuperscript{-} 的电价 → \textbf{结构合理} {[}完成{]}
...
- 4 个 Ca\textsuperscript{2+} 对 F\textsuperscript{-} 总贡献 = \(4 \times 1/4 = 1\) = F\textsuperscript{-} 的电价 → \textbf{结构合理} {[}完成{]}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-12-octahedral-coordination.jpg}
...
\begin{quote}
{[}认知冲突{]} \textbf{认知冲突}：「为什么AgI的半径比(0.523)落在NaCl型范围，实际却是ZnS型？」---Ag\textsuperscript{+}是18电子构型(4d\textsuperscript{10})，极化作用强；I\textsuperscript{-}半径大(220pm)，变形性大。Ag\textsuperscript{+}的强极化使电子云严重畸变→键的共价性增大→配位数从6降为4。
\end{quote}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-17-ion-polarization-ideal-vs-polarized.jpg}
...
\caption{图20 金刚石晶体结构(左)及其晶胞(右)— 每个C原子sp杂化，四面体配位。金刚石是典型的原子晶体(共价晶体)}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.45\textwidth,keepaspectratio]{media/13-14-co2-molecular-crystal-unit-cell.jpg}
\caption{图21 干冰(CO)分子晶体晶胞 — 面心立方排列的CO分子，分子之间以范德华力结合}
\end{figure}
...
hcp 中，密置双层中上层球落在下层三个球形成的凹陷处。三个下层球的球心构成正三角形，边长 \(2r\)。上层球球心到三个下层球球心的距离相等，构成正四面体。正四面体高 \(h = \frac{\sqrt{6}}{3} \times 2r\)。所以两层间的垂直距离 = 正四面体的高 = \(\frac{2\sqrt{6}}{3}r\)。一个 hcp 晶胞的 c 轴包含 2 层(即 AB 两个底面的垂直距离 × 2)：\(c = 2 \times \frac{2\sqrt{6}}{3}r = \frac{4\sqrt{6}}{3}r = 4\sqrt{\frac{2}{3}}r\)。而 \(a = 2r\)，所以 \(c/a = 2\sqrt{\frac{2}{3}} \approx 1.633\) {[}完成{]}
...
\item
  \textbf{CaO}：\(r_+/r_- = 100/140 = 0.714\)，0.414 \textless{} 0.714 \textless{} 0.732 → \textbf{NaCl型} {[}完成{]}
\end{enumerate}
...
6 个 Mg\textsuperscript{2+} 对 O\textsuperscript{2-} 总贡献 = \(6 \times 1/3 = 2\) = O\textsuperscript{2-} 的电价 → \textbf{结构合理} {[}完成{]}
...
\item
  MgO 为 NaCl 型，Mg\textsuperscript{2+} 被 6 个 O\textsuperscript{2-} 包围。每个 Mg\textsuperscript{2+} 分配静电键强度 = \(2/6 = 1/3\)，6 个总贡献 = \(6 \times 1/3 = 2\) = O\textsuperscript{2-} 的电价 → \textbf{结构合理} {[}完成{]}
\item
  \textbf{hcp 理想 c/a}：\(c/a = 2\sqrt{2/3} \approx 1.633\)。推导：正四面体高 \(h = \sqrt{6}/3 \times a\)，hcp 单层高 \(= 2h\)，\(c = 2 \times 2h = 4h\) → \(c/a = 2\sqrt{2/3}\)。
\item
  fcc 有 \textbf{4个八面体空隙} + \textbf{8个四面体空隙}。验证：每球周围有 6 个八面体空隙，每个由6个球共享 → \(6 \times 4 / 6 = 4\) {[}完成{]}。每球周围有 8 个四面体空隙，每个由4个球共享 → \(8 \times 4 / 4 = 8\) {[}完成{]}。
\item
