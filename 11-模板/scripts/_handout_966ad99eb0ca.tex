...
\begin{quote}
\textbf{三相点 \不等于 冰点}：三相点是物质固、液、气三相共存的温度(纯物质特有)，而冰点是溶液中固液两相平衡的温度。水的三相点 = 0.01°C，冰点 = 0.00°C(标准大气压下)。
\end{quote}
...
\[
\boxed{\Delta T_b = K_b \cdot b}
\]

其中 \(K_b\) 是沸点升高常数(溶剂特有)，b 是质量摩尔浓度。

\textbf{推导链}：\(\Delta p \propto x_B \propto b \rightarrow \Delta T_b \propto \Delta p \propto b \rightarrow \Delta T_b = K_b \cdot b\)

\textbf{常见溶剂的 \(K_b\) 和 \(K_f\)}：
...
\begin{quote}
\textbf{竞赛提示}：测定分子量时优先选用 \(K_f\) 大的溶剂(如环己烷 \(K_f\) = 20.0)，因为同样的 b 产生的 \(\Delta T_f\) 更大，测量更精确。
\end{quote}

\subsubsection{4.3 凝固点降低}\label{ux51ddux56faux70b9ux964dux4f4e}

\[
\boxed{\Delta T_f = K_f \cdot b}
\]

\begin{quote}
\textbf{融雪原理}：往雪上撒盐(NaCl)，盐溶于雪水形成溶液，凝固点降低(\(\Delta T_f = K_f \cdot b\))，使雪在 0°C 以下也能融化。但融雪后路面温度会更低---``盐不是热的，雪融化是因为凝固点降低了''。
\end{quote}
...
\textbf{渗透压} & \(\Pi = 2.26\) kPa & \textbf{最佳选择} \\
\end{longtable}
}

\begin{quote}
\textbf{结论}：对大分子(蛋白质、高分子)，渗透压是测定分子量的最佳方法---因为 \(\Pi = cRT\)，即使 c 很小，\(\Pi\) 仍然可测。
\end{quote}
...
\begin{quote}
\textbf{``可乐气泡''类比}：打开可乐瓶盖，压力骤降，CO\textsubscript{2} 的分压减小 \箭 溶解度降低 \箭 气泡逸出。
\end{quote}
...
\bottomrule\noalign{}
\endlastfoot
潜水 & 高压下 N\textsubscript{2} 溶入血液，减压过快 \箭 气泡形成 \箭 减压病 \\
...
\begin{quote}
\textbf{``少量多次''原则}：将总量的萃取剂分成多次使用，比一次性使用全部萃取剂效果更好。这是分配定律的数学推论。
\end{quote}
...
\begin{quote}
\textbf{三相点 \不等于 冰点}：三相点是纯水的固有性质(0.01°C)，与外压无关；冰点受大气压影响(标准大气压下 0.00°C)。
\end{quote}

\textbf{等压冷却过程}(25°C \箭 低温)：
...
\textbf{题干}：0.500 g 未知非电解质溶于 25.0 g 水中，溶液凝固点为 −0.260°C。求该物质的分子量。(\(K_f\) = 1.86 K·kg/mol)
...
\begin{quote}
本题用凝固点降低而非沸点升高，因为 \(K_f > K_b\)(水的 \(K_f\) = 1.86 \textgreater{} \(K_b\) = 0.512)，同样浓度下 \(\Delta T_f > \Delta T_b\)，测量更精确。
\end{quote}
...
van't Hoff 因子 & 电解质 i \textgreater{} 1 & 稀溶液 i \约等于 化学式中离子数 \\
\end{longtable}
}
...
\textbf{2.} 计算 0.50 m 蔗糖水溶液的沸点升高和凝固点降低。(\(K_b\) = 0.512, \(K_f\) = 1.86)
...
\textbf{9.} 计算 25°C 时，O\textsubscript{2} 在水中的溶解度(已知 \(K_H\) = 1.3 × 10\textsuperscript{-3} mol/(L·atm)，空气中 O\textsubscript{2} 分压 = 0.21 atm)。
...
\textbf{12.} \textbf{(依数性综合)} 将 0.650 g 某有机化合物溶于 25.0 g 苯中，溶液沸点升高 0.48°C。求该化合物的分子量。(苯 \(K_b\) = 2.53)

\textbf{13.} \textbf{(电解质依数性)} 计算 0.10 m MgCl\textsubscript{2} 溶液在 25°C 时的渗透压(假设完全电离)。实测值约为理论值的 83\%，解释原因。

\textbf{14.} \textbf{(Henry 定律应用)} 潜水员在 40 m 深(5 atm)呼吸空气 1 小时后快速上升到水面。已知 N\textsubscript{2} 在血液中的 \(K_H\) = 6.5 × 10\textsuperscript{-4} mol/(L·atm)。计算 37°C 时每升血液中释放的 N\textsubscript{2} 体积(假设血液中 N\textsubscript{2} 初始溶解量等于平衡溶解量)。
...
\textbf{24.} \textbf{(Henry 定律与潜水)} 潜水员在 60 m 深(7 atm)使用 He-O\textsubscript{2} 混合气(He:O\textsubscript{2} = 4:1)呼吸 2 小时后减压。已知 He 在 37°C 血液中的 \(K_H\) = 1.5 × 10\textsuperscript{-3} mol/(L·atm)。计算每升血液中 He 的溶解量，以及快速上升到水面时每升血液释放的 He 体积(标准状况下)。
...
\textbf{2.} \(\Delta T_b = 0.512 \times 0.50 = 0.256\)°C，沸点 = 100.256°C
\(\Delta T_f = 1.86 \times 0.50 = 0.930\)°C，凝固点 = −0.930°C

\textbf{3.} 理论值假设 i = 2(完全电离)，但实际 NaCl 在 0.10 m 时 i \约等于 1.87(离子间相互作用使有效粒子数减少)，所以实测值小于理论值。
...
\textbf{8.} 渗透压法对大分子最有效：\(\Pi = cRT\)，即使 c 很小，\(\Pi\) 仍然可测。而 \(\Delta T_b = K_b \cdot b\) 对大分子太小，难以精确测量。

\textbf{9.} \(c = K_H \times p = 1.3 \times 10^{-3} \times 0.21 = 2.7 \times 10^{-4}\ \mathrm{mol/L}\)。
...
\textbf{12.} \(\Delta T_b = K_b \cdot b = K_b \cdot (0.650/M) / 0.0250 = 0.48\)
...
\textbf{16.} 300°C \箭 液相区(降温)\箭 到达液相线 \箭 开始析出 Bi \箭 液相组成沿液相线变化 \箭 到达 144°C(共晶温度)\箭 剩余液相同时析出 Bi 和 Cd(共晶混合物)\箭 完全凝固 \箭 固相降温至 100°C。
...
\textbf{20.} Π = cRT \箭 c = Π/(RT) = 0.684/(8.314×298/1000) = 2.76×10\textsuperscript{-4} mol/L
...
Π\_浓缩 \约等于 5.8 MPa，实际需施加约 7.0 MPa

\textbf{22.} 水从低浓度(0.20 m，蒸气压高)向高浓度(0.50 m，蒸气压低)转移。烧杯 B(0.20 m)的水蒸发，烧杯 A(0.50 m)的水凝结。最终烧杯 A 体积增大。

\textbf{23.} 非电解质理论 \(\Delta T_f = K_f \times b = 1.86 \times (1.00/300/0.100) = 0.062\,^\circ\mathrm{C}\)
...
释放量 = 6.0×10\textsuperscript{-3} mol/L \箭 V = 0.134 L = 134 mL/L 血液
...
12x × 50 + x × 50 = 0.50 \箭 650x = 0.50 \箭 x = 7.7×10\textsuperscript{-4} g/mL
...
\item
  两次萃取(每次 25 mL 水)：
  第一次：x\textsubscript{1}(12×50 + 25) = 0.50 \箭 x\textsubscript{1} = 0.50/625 \箭 CCl\textsubscript{4} 剩余 = 12x\textsubscript{1}×50 = 0.48 g
