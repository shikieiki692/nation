...
> 属合理拒收）。
>
> **2026-09-20 D 项（去表格包裹·第三形态，`1c4bb456f`）**：授权域内**不适用 pipe table** 的
> HTML 表（评分卡 / 问答卡：`width9` 评分卡、`prose` 长答卡、`narrow1` 单列问答块）共
> **15 文件 / 16 表**，把表格包裹**拆掉**改为「**题干普通段 + 答案引用块（`> `）**」，
> 公式随段落恢复 inline 渲染。适用域：`04-课件/习题集` 第一轮·竞赛教材版 + `04-题库` 教材习题/真题。
>
> D 项转换器（`tmp/_dcv.py`）三条关键规则：
> 1. **行分类**：答案行 = 含「N分」得分标记 → 引用块；其余 = 题干段。
>    判据**只用得分标记**，不用「题号+$」（后者会误判含公式的题干行）。
> 2. **表内子表识别**：`<table>` 内嵌「表头 + ≥2 数据行」→ 转 **pipe table**，
>    **不得降级为裸文本**（首轮 120 行数据裸行缺陷的根因）。
> 3. **span 展开守恒**：`colspan`/`rowspan` 的文本**只在起始格出现一次**，其余空占位；
>    ⚠️ 误把文本复制到每个跨越格 → 评价卡文本在输出中重复 N 次（首轮实证）。
>    `rowspan` 复合评分卡（答案-数据-答案 sandwich）→ **整表拒收**（不可拆）。
>
> D 项四道核验（全通过）：标签守恒（`details`/`summary`/`img`/`![[`）异常 **0**；
> 内容守恒（HEAD 单元格文本去空白比对）缺失 **0**；渲染自证（现状残留含 `$` 表）**0**；
...
>
> **⭐ 并入等价性三重实证**：
> 1. **逐表等价**：从 D 项备份取 34 张原表，新旧转换器输出 **34/34 逐字节一致**；
> 2. **端到端等价**：D 项 16 文件从「转换前」备份用**并入后的 CLI** 重转，
>    产物与仓库现状 **16/16 逐字节一致**（行尾归一后）；
> 3. **幂等**：对已转换的 16 文件再跑 `--qa-cards` → 候选 **0**；域内 7146 文件扫描 → 候选 **0**。
>
> ⚠️ **并入必踩的判据适配（本次实证）**：四口径的「**宽度一致**」假设产物是表格
> （逐行 `|` 数一致），而 D 通道产物是「题干段 + 引用块」混合文本 → **假失败**，
> 反向验证时 2 表全被跳过（`处理文件 0`）。**正解**：
> ① 「宽度一致」**只对 pipe 通道生效**（按 `item["channel"]` 分派）；
> ② D 通道改用「**内容守恒**」（原表每个非空单元格文本须在产物中出现）；
> ③ 「行结构自洽」亦按通道分派（pipe 须有数据行；D 只须非空）。
>
> **2026-09-20 续轮（W8 · 宽度上限修正，`ed8bd441a`）**：原 `width > 8` 判据**误杀真数据表** ——
> `n l m ms` 量子数表（10 列）、水化焓/晶格能热力学表（9 列）、分子几何构型对照（9 列）、
> Hammett σ 取代基表（9 列）**全是规整数据表**，公式却因被拒收而仍不渲染。
> 判据放宽 **8 → 12**（宽度窄屏显示差 << 公式变 LaTeX 源码乱码）。转换 **7 表 / 7 文件**：
> 习题书-教师版·学生版（各 1 张，**同源双版须并转**）+ `04-题库/化学原理/15-09.md` +
> `04-题库/教材习题` 2 份 + `04-题库/真题/第37届决赛` 1 份。
> 其中 3 份为「**行尾混杂**」文件（CRLF 主体 + frontmatter 裸 LF），工具「四口径」的
> `行尾保持: False` 会跳过 → 改用**定向替换器**（只替换 `<table>` 段、按**紧邻前导换行**定 eol、
> 其余字节原样保留），CRLF/裸 LF 计数守恒。核验：渲染自证 **7/7**；js-yaml 受检 7 / 失败 0；
> validate_kb 受检 7 / 0E / 0W / 0I；`--ignore-cr-at-eol` 与普通 numstat 数字一致（真内容改动）。
> 全库域内复扫：残留含公式宽表 **0**；域外（`06-外部资料导入` / `mineru` / `clayden` /
> 中级无机化学等 OCR 源区）**89 处按既定策略不做**。
